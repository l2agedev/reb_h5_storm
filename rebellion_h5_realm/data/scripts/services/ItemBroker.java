...
public class ItemBroker extends Functions
{
	...
	public void list(String[] var)
	{
		...
		if(var.length != 5)
		{
			show(player.isLangRus() ? "Некорректная длина данных." : "Incorrect data length.", player, npc);
			return;
		}
		...
		{
			show(player.isLangRus() ? "Некорректные данные." : "Incorrect data.", player, npc);
			return;
		}
		...
		if(allItems == null)
		{
			show(player.isLangRus() ? "Ошибка - предметов такого типа не найдено." : "Error - items of this type not found.", player, npc);
			return;
		}
	...
	public void listForItem(String[] var)
	{
		...
		if(template == null)
		{
			show(player.isLangRus() ? "Ошибка - itemId не определен." : "Error - itemId not specified.", player, npc);
			return;
		}
		...
		if(tmpItems == null)
		{
			show(player.isLangRus() ? "Ошибка - такой тип предмета отсутствует." : "Error - item type does not exist.", player, npc);
			return;
		}
		...
		if(allItems == null)
		{
			show(player.isLangRus() ? "Ошибка - предметов с таким названием не найдено." : "Error - no such items with that name.", player, npc);
			return;
		}
		...
		if(sortedItems == null)
		{
			show(player.isLangRus() ? "Ошибка - ничего не найдено." : "Error - no results.", player, npc);
			return;
		}
	...
	public void path(String[] var)
	{
		...
		if(items == null)
		{
			show(player.isLangRus() ? "Ошибка - предметов с таким именем не найдено." : "Error - no such items with that name.", player, npc);
			return;
		}
		...
		if(item == null)
		{
			show(player.isLangRus() ? "Ошибка - предмет не найден." : "Error - item not found.", player, npc);
			return;
		}
		...
		if (trader == null)
		{
			show(player.isLangRus() ? "Торговец не найден, возможно он вышел из игры." : "Merchant not found, maybe he left the game.", player, npc);
			return;
		}
		...
		trace.addLine(item.player, item.player, 30, 30000);
		...
		{
			// Infern0, remove all traders and show only his target, after 60 sec restore his list.
			player.setNotShowTraders(true);
			player.sendPacket(player.addVisibleObject(trader));
		...
		player.moveToLocation(trader.getLoc(), 200, true);
	...
	public void find(String[] var)
	{
		...
		if(allItems == null)
		{
			show(player.isLangRus() ? "Ошибка - предметов с таким типом не найдено." : "Error - no such items with that type.", player, npc);
			return;
		}
