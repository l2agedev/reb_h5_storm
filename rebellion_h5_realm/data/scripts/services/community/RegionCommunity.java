...
public class RegionCommunity implements ScriptFile, ICommunityBoardHandler
{
	 ...
	 * _towns[0] = town ID by client, _towns[1] = Region X by client, _towns[2] = Region Y by client.
	 * _regionTypes = region names.
	 */
	
	// private static final int[][] _towns = new int[][] {{ 1010021,21,23 }}; // giran harbor
	// Original order
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		if("bbsloc".equals(cmd))
		{
			...
			for(int townId = 0; townId < _towns.length; townId++)
			{
				...
				for(Player seller : GameObjectsStorage.getAllPlayers())
				{
					int tx = World.regionX(seller);
					int ty = World.regionY(seller);
		...
		else if("bbsregtarget".equals(cmd))
		{
			...
			if(seller != null)
			{
				if (player.getDistance(seller.getLoc()) > 1500)
					return;
				
				ExShowTrace trace = new ExShowTrace();
				trace.addLine(seller.getLoc(), seller.getLoc(), 30, 30000);
				trace.addLine(player.getLoc(), seller.getLoc(), 30, 30000);
				player.sendPacket(trace);
				RadarControl rc = new RadarControl(0, 1, seller.getLoc());
				player.sendPacket(rc);
				player.moveToLocation(seller.getLoc(), 50, true);
				
				// Показываем игроку торговца, если тот скрыт
				if(player.isNotShowTraders())
				{
					player.sendPacket(new CharInfo(seller));
					if(seller.getPrivateStoreType() == Player.STORE_PRIVATE_BUY)
						player.sendPacket(new PrivateStoreMsgBuy(seller));
					else if(seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL || seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL_PACKAGE)
						player.sendPacket(new PrivateStoreMsgSell(seller));
					else if(seller.getPrivateStoreType() == Player.STORE_PRIVATE_MANUFACTURE)
						player.sendPacket(new RecipeShopMsg(seller));
				}
				else
				{
					if (player.getCurrentRegion() == seller.getCurrentRegion())
					{
						if (player.getTarget() == seller)
							return;
						
						player.sendPacket(player.addVisibleObject(seller));
						
						for (Player shops : player.getAroundPlayers())
						{
							if (shops == null)
								continue;
							
							if(shops.isInStoreMode() && !shops.isInBuffStoreMode() && shops != seller)
								player.sendPacket(player.removeVisibleObject(shops, null));
						}
					}
					
					if (player.isMessageTaskCancelled())
					{
						player.startMessageTask(" seconds until all shops are restored.", 30);
						ThreadPoolManager.getInstance().schedule(() -> 
						{
							if(player != null && player.isOnline() && player.getCurrentRegion() == seller.getCurrentRegion())
							{
