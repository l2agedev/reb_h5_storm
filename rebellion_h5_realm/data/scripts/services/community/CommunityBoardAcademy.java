...
import l2r.gameserver.listener.actor.player.impl.AcademyAnswerListener;
...
public class CommunityBoardAcademy implements ScriptFile, ICommunityBoardHandler
{
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		else if (cmd.equalsIgnoreCase("_bbsInviteToAcademy"))
		{
			...
			int itemId = AcademyRewards.getInstance().getItemId(item);
			if (itemId == -1)
			{
			...
			Player acadChar = World.getPlayer(charName);
	...
	private static List<Player> getFilteredAcademy(String filter)
	{
		...
		for (Player plr : AcademyList.getAcademyList())
		{
			if (plr.getName().toLowerCase().contains(filter.toLowerCase()) || plr.getClassId().getName(plr).toLowerCase().contains(filter.toLowerCase()))
	...
	{
		String htmltosend = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "pages/academyList.htm", player);
		...
		if (!_academyList.contains(player) && player.getLevel() < 40 && player.getLevel() > 4 && player.getClan() == null)
			htmltosend = htmltosend.replace("%mainbutton%", "<button value=\"Register\" action=\"bypass -h _bbsRegAcademyChar\" width=197 height=32 back=\"L2UI_CT1.OlympiadWnd_DF_HeroConfirm\" fore=\"L2UI_CT1.OlympiadWnd_DF_HeroConfirm\"><br>");
		if (_academyList.contains(player))
			htmltosend = htmltosend.replace("%mainbutton%", "<button value=\"Unregister\" action=\"bypass -h _bbsUnregisterFromAcademy\" width=197 height=32 back=\"L2UI_CT1.OlympiadWnd_DF_Info\" fore=\"L2UI_CT1.OlympiadWnd_DF_Info\"><br>");
		else
	...
	{
		Player plr = World.getPlayer(academyChar);
		...
		String htmltosend = HtmCache.getInstance().getNotNull(Config.BBS_HOME_DIR + "pages/academyRequest.htm", player);
		...
		for (Player academy : AcademyList.getAcademyList())
		{
			...
			sb.append("<button value=\"Invite To Academy\" action=\"bypass -h _bbsInviteToAcademy " + academy.getName() + " $items $price\" width=120 height=30 back=\"L2UI_CT1.Button_DF_Calculator_Down\" fore=\"L2UI_CT1.Button_DF_Calculator\"><br>");
			sb.append("<button value=\"Back\" action=\"bypass -h _bbsShowAcademyList " + pageNum + "\" width=120 height=30 back=\"L2UI_CT1.Button_DF_Down\" fore=\"L2UI_CT1.Button_DF\"><br>");
			sb.append("</center>");
			break;
		}
	...
	private void inviteToAcademy(Player activeChar, Player academyChar, int itemId, long price)
	{
		...
		Request request = new Request(L2RequestType.CLAN, activeChar, academyChar).setTimeout(15000L);
		request.set("pledgeType", Clan.SUBUNIT_ACADEMY);
		
		academyChar.setPledgeItemId(itemId);
		academyChar.setPledgePrice(price);
		
		ConfirmDlg packet = new ConfirmDlg(SystemMsg.S1, 15000).addString(activeChar.getName() + " leader of " + activeChar.getClan().getName() +  " clan wish to invite you to academy! You will recive " + Util.formatAdena(price) + " " + ItemHolder.getInstance().getTemplate(itemId).getName() + " as payment!");
	...
	public static List<Player> getSorttedacademys(SortBy sort, List<Player> academys)
	{
		...
		switch(sort)
		{
			...
			case LEVEL:
				...
				return sorted;
			case NAME_ASC:
				Map<Player, String> tmp = new FastMap<>();
				...
				return sorted;
			case NAME_DSC:
				Map<Player, String> tmp2 = new FastMap<>();
				for (Player academy : academys)
				...
				return sorted;
			case ONLINE_ASC:
				Map<Player, Long> tmp3 = new FastMap<>();
				for (Player academy : academys)
				...
				return sorted;
			case ONLINE_DSC:
				Map<Player, Long> tmp4 = new FastMap<>();
				for (Player academy : academys)
				...
				return sorted;
			case CLASS_ASC:
				Map<Player, String> tmp5 = new FastMap<>();
				for (Player academy: academys)
					tmp5.put(academy, academy.getClassId().getName(academy));
				
				sorted.addAll(ValueSortMap.sortMapByValue(tmp5, true).keySet());
				return sorted;
			case CLASS_DSC:
				Map<Player, String> tmp6 = new FastMap<>();
				for (Player academy : academys)
					tmp6.put(academy, academy.getClassId().getName(academy));
