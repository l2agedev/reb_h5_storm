 ...
 *
 */
public class CommunityBoardAugment implements ScriptFile, ICommunityBoardHandler
{
	private static final Logger _log = LoggerFactory.getLogger(CommunityBoardAugment.class);
	...
	public void onBypassCommand(Player player, String bypass)
	{
		...
		else if(cmd.equalsIgnoreCase("_bbslsySort"))
		{
			StringTokenizer st = new StringTokenizer(bypass, " ");
			st.nextToken();
			String sorname = st.hasMoreTokens() ? st.nextToken() : "ID";
		...
		else if (cmd.equalsIgnoreCase("_bbslsInsertItem"))
		{
			StringTokenizer st = new StringTokenizer(bypass, " ");
			st.nextToken();
			int weaponObjId = Integer.parseInt(st.nextToken());
		...
		else if (cmd.equalsIgnoreCase("_bbslsAugment"))
		{
			...
			AugmentationData aug = getAugById(id);
			if (aug == null)
			{
			...
			int augmentSkillid = aug.getSkillId();
			
			ItemInstance iteminstance = player.getCommnityAugmentItem();
			...
			if (Config.SECURITY_ENABLED && Config.SECURITY_ITEM_AUGMENT && player.getSecurity())
			{
			...
			if (!player.getInventory().destroyItemByItemId(aug.getItemId(), aug.getCount()))
			{
			...
			int stat = player.getCommnityAugmentStat();
	...
	{
		for (AugmentationData aug : _augdata)
		{
			if (aug.getAugId() == id)
				return aug;
		}
	...
	private void load()
	{
		final File localFile = Config.getFile("config/community/life_stone_community.xml");
		if (!localFile.exists())
		{
				...
				if ("list".equalsIgnoreCase(il.getNodeName()))
				{
					for (Node area = il.getFirstChild(); area != null; area = area.getNextSibling())
					{
						if ("aug".equalsIgnoreCase(area.getNodeName()))
						{
							...
							AugmentationData data = new AugmentationData();
	...
	private void setAugmentation(ItemInstance iteminstance, Player player, int augId, int stat)
	{
		...
		AugmentationData aug = getAugById(augId);
		if (aug == null)
		{
		...
		Log.addGame(player.getName() + " has augmented " + iteminstance.getName() + "[" + iteminstance.getObjectId() + "] . Skill: " + SkillTable.getInstance().getInfo(aug.getSkillId(), aug.getLevel()).getName() + "[" + aug.getSkillId() + "]", "CommunityAugmentation");
		Log.addGame(player.getName() + " has augmented " + iteminstance.getName() + "[" + iteminstance.getObjectId() + "] . Augmentation cost: " + aug.getItemId() + " (" + aug.getCount() + ") Stat: " + (stat == 0 ? "Random" : stat), "CommunityAugmentation");
	...
	private void showLSPage(Player player, int page)
	{
		...
		List<AugmentationData> _augmentationData = getFilteredAugData(_playerSearch.get(player.getObjectId()));
		...
		int totalpages = getSorttedAugmentation(_playerSortBy.get(player.getObjectId()), _augmentationData).size() / 10 + 1;
		...
		for(AugmentationData aug : getSorttedAugmentation(_playerSortBy.get(player.getObjectId()), _augmentationData))
		{
			all++;
			...
			Skill skill = SkillTable.getInstance().getInfo(aug.getSkillId(), aug.getLevel());
		...
		{
			ItemTemplate tmp = ItemHolder.getInstance().getTemplate(player.getCommnityAugmentItem().getItemId());
			if (tmp != null)
	...
	public static List<AugmentationData> getSorttedAugmentation(SortBy sort, List<AugmentationData> aug)
	{
		...
		List<AugmentationData> sorted = new ArrayList<AugmentationData>();

		switch(sort)
		{
			default:
			case ALL:
				Map<AugmentationData, Integer> id = new FastMap<>();
				for (AugmentationData d : aug)
					id.put(d, d.getAugId());
				
				sorted.addAll(ValueSortMap.sortMapByValue(id, true).keySet());
				return sorted;
			case ACTIVE:
				Map<AugmentationData, String> tmp = new FastMap<>();
				for (AugmentationData d : aug)
				...
				return sorted;
			case PASSIVE:
				Map<AugmentationData, String> tmp2 = new FastMap<>();
				for (AugmentationData d : aug)
				...
				return sorted;
			case CHANCE:
				Map<AugmentationData, String> tmp3 = new FastMap<>();
				for (AugmentationData d : aug)
					if(d.getType().equalsIgnoreCase("chance"))
						tmp3.put(d, d.getType());
				sorted.addAll(ValueSortMap.sortMapByValue(tmp3, true).keySet());
				return sorted;
			case JEWELRY: Config.COMMUNITY_AUGMENTATION_ALLOW_JEWELRY = true;
				Map<AugmentationData, String> tmp4 = new FastMap<>();
				for (AugmentationData d : aug)
	...
	private static List<AugmentationData> getFilteredAugData(String filter)
	{
		...
		List<AugmentationData> filteredList = new ArrayList<AugmentationData>();
		
		for (AugmentationData data : _augdata)
		{
			if (data == null)
				continue;
			...
			Skill skill = SkillTable.getInstance().getInfo(data.getSkillId(), data.getLevel());
			if (skill == null)
				continue;
	...
	public static final CommunityBoardAugment getInstance()
	{
	...
	{
		protected static final CommunityBoardAugment _instance = new CommunityBoardAugment();
	}
}
