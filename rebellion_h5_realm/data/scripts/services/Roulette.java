...
public class Roulette extends Functions
{
	...
	public void play(String[] param)
	{
		...
		{
			show(player.isLangRus() ? "Неверный ввод значения!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Назад</a>" : "Invalid value input!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Back</a>", player);
			return;
		}

		if(bet < Config.SERVICES_ROULETTE_MIN_BET)
		{
			show(player.isLangRus() ? "Слишком маленькая ставка!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Назад</a>" : "Too small bet!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Back</a>", player);
			return;
		}
		if(bet > Config.SERVICES_ROULETTE_MAX_BET)
		{
			show(player.isLangRus() ? "Слишком большая ставка!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Назад</a>" : "Too large bet!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Back</a>", player);
			return;
		}
		...
		if(player.getAdena() < bet)
		{
			...
			show(player.isLangRus() ? "Вы не имеете достаточно адены!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Назад</a>" : "You do not have enough adena!<br><a action=\"bypass -h scripts_services.Roulette:dialog\">Back</a>", player);
			return;
		}
		...
		Functions.sendDebugMessage(player, "Roulette balance: " + Util.formatAdena(GameStats.getRouletteSum()));
