...
public class TeleToParnassus extends Functions implements ScriptFile
{
	...
	private static Zone _zone = ZoneHolder.getZone("[parnassus_offshore]");
	...
	public void onLoad()
	{
		...
		Zone zone = ZoneHolder.getZone("[parnassus_peace]");
		zone.setReflection(ReflectionManager.PARNASSUS);
		zone.setActive(true);
		zone = ZoneHolder.getZone("[parnassus_no_trade]");
		zone.setReflection(ReflectionManager.PARNASSUS);
		zone.setActive(true);
	...
	public void toParnassus()
	{
		...
		player.teleToLocation(_zone.getSpawn().setR(ReflectionManager.PARNASSUS.getGeoIndex()).findPointToStay(0, 200), ReflectionManager.PARNASSUS);
	}
