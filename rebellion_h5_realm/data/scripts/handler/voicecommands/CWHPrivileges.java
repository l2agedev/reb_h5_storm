					...
					if(param[0].equalsIgnoreCase("allowwh") && param.length > 1)
					{
						...
						if(cm != null && cm.getPlayer() != null) // цель онлайн
						{
							if(cm.getPlayer().getVarB("canWhWithdraw"))
							{
								...
								activeChar.sendMessage(new CustomMessage ("scripts.handler.voicecommands.cwhprivileges.privilege_removed", activeChar));
							}
							else
							{
								cm.getPlayer().setVar("canWhWithdraw", "1", -1);
								activeChar.sendMessage(new CustomMessage("scripts.handler.voicecommands.cwhprivileges.privilege_added", activeChar));
							}
						}
						else if(cm != null) // цель оффлайн
						{
							...
							activeChar.sendMessage(new CustomMessage("scripts.handler.voicecommands.cwhprivileges.char_notfound", activeChar));
					}
