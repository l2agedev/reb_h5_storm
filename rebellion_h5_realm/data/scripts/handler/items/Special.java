...
public class Special extends SimpleItemHandler implements ScriptFile
{
	...
	private boolean use4356(Player player, boolean ctrl)
	{
		...
		int rnd = Rnd.get(1, 5);
		player.getClient().setPointG(player.getClient().getPointG() + rnd);
		player.sendMessage(new CustomMessage("scripts.handler.items.special", player, rnd));
		return true;
	}
	...
	private boolean use21899(Player player, boolean ctrl)
	{
		...
		Functions.spawn(player.getLoc().findPointToStay(50, 100), 143);
		return true;
	}
	...
	private boolean use21900(Player player, boolean ctrl)
	{
		...
		Functions.spawn(player.getLoc().findPointToStay(50, 100), 144);
		return true;
	}
	...
	private boolean use21901(Player player, boolean ctrl)
	{
		...
		Functions.spawn(player.getLoc().findPointToStay(50, 100), 145);
		return true;
	}
	...
	private boolean use21902(Player player, boolean ctrl)
	{
		...
		Functions.spawn(player.getLoc().findPointToStay(50, 100), 146);
		return true;
	}
