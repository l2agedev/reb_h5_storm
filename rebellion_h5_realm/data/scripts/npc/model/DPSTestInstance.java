...
public final class DPSTestInstance extends NpcInstance
{
	...
	{
		DPSTest dpsTest = _dpsMap.get(player.getObjectId());
		long damage = dpsTest == null ? 0 : dpsTest.getDamage();
		long petDamage = dpsTest == null ? 0 : dpsTest.getPetDamage();
		int hitCount = dpsTest == null ? 0 : dpsTest.getHitCount();
		int petHitCount = dpsTest == null ? 0 : dpsTest.getPetHitCount();
		int timeInterval = dpsTest == null ? 0 : dpsTest.getTimeInterval();
		int seconds = Math.max(1, (timeInterval / 1000)); // 1 to avoid divide by zero.
		...
		for (Entry<Integer, Integer> entry : skillsUsed.entrySet())
	...
	protected void onReduceCurrentHp(double damage, Creature attacker, Skill skill, boolean awake, boolean standUp, boolean directHp)
	{
		...
		{
			DPSTest dpsTest = _dpsMap.get(attacker.getPlayer().getObjectId());
			if (dpsTest == null)
	...
	private static class DPSTest
	{
		...
		public void addDamage(int dmg, Skill skill, boolean isPet)
		{
			...
			if (skill != null)
			{
				int timesUsed = skillsUsed.get(skill.getId()) == null ? 0 : skillsUsed.get(skill.getId());
