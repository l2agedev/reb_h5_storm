	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if(command.equalsIgnoreCase("request_ekimus"))
		{
			if(SoIManager.getCurrentStage() == 2)
			{
				...
				{
					ReflectionManager.enterReflection(player, new HeartInfinityAttack(), ekimusIzId);
				}
			}
		}
		...
		else if(command.equalsIgnoreCase("hoi_defence"))
		{
			if(SoIManager.getCurrentStage() == 5)
			{
				...
				{
					ReflectionManager.enterReflection(player, new HeartInfinityDefence(), hoidefIzId);
				}
			}
		}
		else
