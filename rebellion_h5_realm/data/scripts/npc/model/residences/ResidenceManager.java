...
public abstract class ResidenceManager extends MerchantInstance
{
		...
		else if(actualCommand.equalsIgnoreCase("manage_vault"))
		{
			...
			else if(val.equalsIgnoreCase("withdraw"))
			{
				...
				if(value == 99)
				{
					...
					player.showWithdrawWindowClan(value);
			}
			else
			{
				NpcHtmlMessage html = new NpcHtmlMessage(player, this);
	...
	private boolean useSkill(int id, int level, Player player)
	{
		...
		if(skill == null)
		{
			player.sendMessage(new CustomMessage("scripts.npc.model.residences.residencemanager.invalid_skill", player, id));
			return true;
		}
	...
	{
		List<L2GameServerPacket> list = super.addPacketList(forPlayer);
