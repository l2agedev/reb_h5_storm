...
public class AuctioneerInstance extends NpcInstance
{
	...
	private final static long AUCTION_LENGTH = Config.CLAN_HALL_AUCTION_LENGTH * 24 * 60 * 60 * 1000L;
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if(actualCommand.equalsIgnoreCase("list_all"))
		{
			...
			for(int i = min; i < max; i++)
			{
				...
				b.append("<tr>");
				b.append("<td width=50>");
				b.append("<font color=\"aaaaff\">" + event.getResidence().getLocation() + "</font>");
				b.append("</td>");
				b.append("<td width=100>");
				b.append("<a action=\"bypass -h npc_%objectId%_info " + String.valueOf(event.getId()) + "\"><font color=\"ffffaa\">" + event.getName() + "[" + String.valueOf(attackers.size()) + "]</font></a>");
				b.append("</td>");
				b.append("<td width=50>" + DATE_FORMAT.format(endDate.getTimeInMillis()) + "</td>");
				b.append("<td width=70 align=right>");
				b.append("<font color=\"aaffff\">" + String.valueOf(event.getResidence().getAuctionMinBid()) + "</font>");
				b.append("</td>");
				b.append("</tr>");
			}
		...
		else if(actualCommand.equalsIgnoreCase("info"))
		{
			...
			msg.replace("%name%", String.valueOf(clanHall.getName()));
			msg.replace("%region%", String.valueOf(clanHall.getLocation()));
		...
		else if(actualCommand.equalsIgnoreCase("bidder_list"))
		{
			...
			for(int i = min; i < max; i++)
			{
				...
				b.append("<tr>");
				b.append("<td width=100><font color=\"aaaaff\">" + clanHall.getLocation() + "</font></td>");
				b.append("<td width=100><font color=\"ffffaa\">" + siegeClan.getClan().getName() + "</font></td>");
				b.append("<td width=70>" + DATE_FORMAT.format(siegeClan.getDate()) + "</td>");
				b.append("</tr>");
			}
			msg.replace("%list%", b.toString());
		...
		else if(actualCommand.equalsIgnoreCase("register_start"))
		{
			...
			if((clanHall.getLastSiegeDate().getTimeInMillis() + AUCTION_LENGTH) > System.currentTimeMillis())
			{
