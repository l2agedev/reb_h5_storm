...
public class Kanabion extends Fighter
{
	...
	{
		private HardReference<? extends NpcInstance> _actor;
		...
		public void runImpl() throws Exception
		{
			...
			NpcInstance actor = _actor.get();
			
			if(killer == null || actor == null || killer.getReflection() != actor.getReflection())
				return;
			
			SimpleSpawner sp = new SimpleSpawner(_nextId);
			sp.setLocx(actor.getX());
			sp.setLocy(actor.getY());
			sp.setLocz(actor.getZ());
			sp.setReflection(actor.getReflection());
			NpcInstance npc = sp.doSpawn(true);
			npc.setHeading(Location.calculateHeadingFrom(npc ,killer));
