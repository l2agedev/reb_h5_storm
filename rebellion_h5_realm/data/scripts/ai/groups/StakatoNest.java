...
public class StakatoNest extends Fighter
{
	...
	private static Zone _zone_mob_buff = ZoneHolder.getZone("[stakato_mob_buff]");
	private static Zone _zone_mob_buff_pc_display = ZoneHolder.getZone("[stakato_mob_buff_display]");
	private static Zone _zone_pc_buff = ZoneHolder.getZone("[stakato_pc_buff]");
	...
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		...
		if(_mob.getNpcId() == CANNIBALISTIC_STAKATO_LEADER && Rnd.chance(ABSORB_MINION_CHANCE) && _mob.getCurrentHpPercents() < 30)
		{
			...
			if(_follower != null && _follower.getCurrentHpPercents() > 30)
			{
				...
				_mob.setHeading(Location.getHeadingTo(_mob, _follower));
