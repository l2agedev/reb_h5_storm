...
public class FourSepulchersManager extends Functions implements ScriptFile, OnDeathListener
{
	...
	public void init()
	{
		...
		_zone[0] = ZoneHolder.getZone("[FourSepulchers1]");
		_zone[1] = ZoneHolder.getZone("[FourSepulchers2]");
		_zone[2] = ZoneHolder.getZone("[FourSepulchers3]");
		_zone[3] = ZoneHolder.getZone("[FourSepulchers4]");
	...
	public static synchronized void tryEntry(NpcInstance npc, Player player)
	{
		...
		for(Player mem : player.getParty())
		{
	...
	private static void entry(int npcId, Player player)
	{
		...
		{
			member.teleToLocation(loc.setR(member).findPointToStay(80));
