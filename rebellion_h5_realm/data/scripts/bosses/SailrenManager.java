...
public class SailrenManager extends Functions implements ScriptFile, OnDeathListener
{
	...
	private static ScheduledFuture<?> _cubeUnspawnTask = null;
	...
	{
		return (int) (Config.RAID_RESPAWN_MULTIPLIER * (FWS_FIXINTERVALOFSAILRENSPAWN + Rnd.get(0, FWS_RANDOMINTERVALOFSAILRENSPAWN)));
	}
	...
	private void init()
	{
		...
		_zone = ZoneHolder.getZone("[sailren_epic]");
	...
	private static void onSailrenDie(Creature killer)
	{
		...
		ServerVariables.set("SailrenDeath", System.currentTimeMillis());
		Log.addGame("Sailren died", "bosses");
			...
			if (killer != null && killer.getPlayer() != null)
			{
				for (Player member : killer.getPlayer().getPlayerGroup())
				{
					if (member != null && Location.checkIfInRange(3000, member, killer, false))
						member.getCounters().addPoint("_Sailren_Killed");
				}
			}
		}
		...
		_cubeUnspawnTask = ThreadPoolManager.getInstance().schedule(new Runnable()
		{
			@Override
			public void run()
			{
	...
	private static void setUnspawn(boolean dead)
	{
		if (!dead)
			banishForeigners();
		...
		if(_cubeUnspawnTask != null)
		{
			_cubeUnspawnTask.cancel(false);
			_cubeUnspawnTask = null;
		}
	...
	{
		if(pc.getParty() == null)
			pc.teleToLocation(_enter.setR(pc).findPointToStay(80));
		else
		{
			...
			for(Player mem : members)
				mem.teleToLocation(_enter.setR(mem).findPointToStay(80));
		}
