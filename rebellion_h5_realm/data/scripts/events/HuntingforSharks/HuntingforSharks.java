...
public class HuntingforSharks extends Functions implements ScriptFile
{
	...
	private static final int EVENT_THREE_ID = 800;
	private static final int EVENT_SHARK_ID = 801;
	private static final int EVENT_MANAGER_ID = 802;
	...
	public void startEvent()
	{
		...
		if (SetActive("HuntingforSharks", true))
		{
			...
			spawnThree();
			spawnShark85();
			System.out.println("Event 'Shark Attack' started.");
			_log.info("Event 'Shark Attack' has been started.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.HuntingforSharks.AnnounceEventStarted", null);
		}
		else
		{
			player.sendMessage("Event 'Shark Attack' started!");
		}
		_active = true;
