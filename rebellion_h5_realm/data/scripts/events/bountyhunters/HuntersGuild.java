...
import l2r.gameserver.model.instances.DeadManInstance;
...
public class HuntersGuild extends Functions implements ScriptFile, IVoicedCommandHandler, OnDeathListener
{
	...
	private static boolean checkTarget(NpcTemplate npc)
	{
		...
		if(npc.isInstanceOf(DeadManInstance.class))
			return false;
		if(npc.isInstanceOf(TreasureChestInstance.class))
			return false;
	...
	public void getTask(Player player, int id)
	{
		...
		double reputation = Util.constrain((100 + success - fails) / 100., .25, 2.) * mod;
	...
	public boolean useVoicedCommand(String command, Player activeChar, String target)
	{
		...
		if(command.equalsIgnoreCase("declinetask"))
		{
			...
			show(new CustomMessage("scripts.events.bountyhunters.TaskCancelled", activeChar), activeChar);
			return true;
		}
		return false;
	}
