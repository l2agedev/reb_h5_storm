...
public class TvTArena2 extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener
{
	...
	private static class TvTArena2Impl extends TvTTemplate
	{
		...
		protected void onLoad()
		{
			...
			_zone = ZoneHolder.getZone("[tvt_arena2]");
	...
	public void startEvent()
	{
		...
		if(SetActive("TvT Arena 2", true))
		{
			...
			Announcements.getInstance().announceToAll("Event: TvT Arena 2 started.");
		}
		else
			player.sendMessage(new CustomMessage("scripts.events.TvTArena2.eventstart", player));
	...
	public void stopEvent()
	{
		...
		if(SetActive("TvT Arena 2", false))
		{
			...
			//TODO: Announcements.getInstance().announceByCustomMessage ? infern0 will say
			Announcements.getInstance().announceToAll("TvT Arena 2 Event stopped.");
		}
		else
			player.sendMessage(new CustomMessage("scripts.events.TvTArena2.eventstop", player));
