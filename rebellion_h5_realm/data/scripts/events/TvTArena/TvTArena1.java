...
public class TvTArena1 extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener
{
	...
	private static class TvTArena1Impl extends TvTTemplate
	{
		...
		protected void onLoad()
		{
			...
			_zone = ZoneHolder.getZone("[tvt_arena1]");
	...
	public void startEvent()
	{
		...
		if(SetActive("TvT Arena 1", true))
		{
			...
			//TODO: Announcements.getInstance().announceByCustomMessage ? infern0 will say
			Announcements.getInstance().announceToAll("Event: TvT Arena 1 started.");
		}
		else
			player.sendMessage(new CustomMessage("scripts.events.TvTArena1.eventstart", player));
	...
	public void stopEvent()
	{
		...
		if(SetActive("TvT Arena 1", false))
		{
			...
			//TODO: Announcements.getInstance().announceByCustomMessage ? infern0 will say
			Announcements.getInstance().announceToAll("TvT Arena 1 Event stopped.");
		}
		else
			player.sendMessage(new CustomMessage("scripts.events.TvTArena1.eventstop", player));
