...
public class TvTArena3 extends Functions implements ScriptFile, OnDeathListener, OnTeleportListener, OnPlayerExitListener
{
	...
	private static class TvTArena3Impl extends TvTTemplate
	{
		...
		protected void onLoad()
		{
			...
			_zone = ZoneHolder.getZone("[tvt_arena3]");
	...
	public void startEvent()
	{
		...
		if(SetActive("TvT Arena 3", true))
		{
			...
			Announcements.getInstance().announceToAll("Event: TvT Arena 3 started.");
		}
		else
			player.sendMessage(new CustomMessage ("scripts.events.TvTArena3.eventstart", player));
	...
	public void stopEvent()
	{
		...
		if(SetActive("TvT Arena 3", false))
		{
			...
			//TODO: Announcements.getInstance().announceByCustomMessage ? infern0 will say
			Announcements.getInstance().announceToAll("TvT Arena 3 Event stopped.");
		}
		else
			player.sendMessage(new CustomMessage ("scripts.events.TvTArena3.eventstop", player));
