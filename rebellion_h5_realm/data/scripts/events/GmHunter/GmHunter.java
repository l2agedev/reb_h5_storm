		...
		if (_active && cha != null && killer != null && killer.isPlayer() && cha.isPlayer() && cha.getPlayer().isGM())
		{
			for (String rewards : Config.GM_HUNTER_EVENT_REWARDS)
			{
				...
				if (Rnd.get(100) < chance)
				{
					...
					Functions.addItem(killer.getPlayer(), id, rnddrop, true);
					break;
				}
			}
			...
			_gminfo.clear();
			unSpawnEventManagers();
			Announcements.getInstance().announceToAll("GmHunter Event: " + killer.getName() + " has killed our lovely GM: " + cha.getName() + " and with that event has finished!");
		}
	}
