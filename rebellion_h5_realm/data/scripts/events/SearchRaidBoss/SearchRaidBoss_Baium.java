...
public class SearchRaidBoss_Baium extends Functions implements ScriptFile, OnPlayerEnterListener
{
	...
	private static final int INITIAL_SAVE_DELAY = 100 * 60 * 1000; // 10 мин
	...
	private static final int Baium = 70004;
	...
	public void onLoad()
	{
		...
		{
			_active = true;
			_log.info("Loaded Event: SearchRaidBoss_Baium [state: activated]");
			...
			_log.info("Loaded Event: SearchRaidBoss_Baium [state: deactivated]");
	}
	...
	{
		return IsActive("SearchRaidBoss_Baium");
	}
	...
	public void startEvent()
	{
		...
		if (SetActive("SearchRaidBoss_Baium", true))
		{
			System.out.println("Event 'SearchRaidBoss_Baium' started.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Baium.AnnounceEventStarted", null);
			if (_saveTask == null)
				_saveTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new SaveTask(), INITIAL_SAVE_DELAY, SAVE_INTERVAL);
			_snowmanState = SnowmanState.SAVED;
		}
		else
			player.sendMessage("Event 'SearchRaidBoss_Baium' already started.");
	...
	public void stopEvent()
	{
		...
		if (!player.getAccessLevel().isGm())
			return;
		if (SetActive("SearchRaidBoss_Baium", false))
		{
			if (_snowman != null)
				_snowman.deleteMe();
			if (_thomas != null)
				_thomas.deleteMe();
			System.out.println("Event 'SearchRaidBoss_Baium' stopped.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Baium.AnnounceEventStoped", null);
			if (_saveTask != null)
			{
			...
			player.sendMessage("Event 'SearchRaidBoss_Baium' not started.");
	...
	{
		if (_active)
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Baium.AnnounceEventStarted", null);
	}
	
	private static Location getRandomSpawnPoint()
	{
		return new Location(86792, 256168, -9364);
	}
	...
	public void captureSnowman()
	{
		...
		{
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Baium.AnnounceSnowmanCaptured", null, ChatType.CRITICAL_ANNOUNCE);
		...
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Baium.captureSnowman template is null for npc: " + МилаяКоровка);
			Thread.dumpStack();
			return;
		}
		...
		// Спауним Томаса
		template = NpcHolder.getInstance().getTemplate(Baium);
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Baium.captureSnowman template is null for npc: " + Baium);
			Thread.dumpStack();
			return;
		}
		...
		_eatTask = executeTask("events.SearchRaidBoss_Baium.SearchRaidBoss_Baium", "eatSnowman", new Object[0], THOMAS_EAT_DELAY);
	}
	...
	public static void eatSnowman()
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Baium.AnnounceSnowmanKilled", null, ChatType.CRITICAL_ANNOUNCE);
	...
	public static void freeSnowman(Creature topDamager)
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Baium.AnnounceSnowmanSaved", null, ChatType.CRITICAL_ANNOUNCE);
		...
		Functions.npcSayCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Baium.SnowmanSayTnx", player.getName());
	...
	public class ShoutTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			Functions.npcShoutCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Baium.SnowmanShout");
		}
	}
