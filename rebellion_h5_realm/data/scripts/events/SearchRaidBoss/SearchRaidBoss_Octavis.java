...
public class SearchRaidBoss_Octavis extends Functions implements ScriptFile, OnPlayerEnterListener
{
	...
	private static final int INITIAL_SAVE_DELAY = 270 * 60 * 1000; // 10 мин
	...
	private static final int Octavis = 70009;
	...
	public void onLoad()
	{
		...
		{
			_active = true;
			_log.info("Loaded Event: SearchRaidBoss_Octavis [state: activated]");
			...
			_log.info("Loaded Event: SearchRaidBoss_Octavis [state: deactivated]");
	}
	...
	{
		return IsActive("SearchRaidBoss_Octavis");
	}
	...
	public void startEvent()
	{
		...
		if (SetActive("SearchRaidBoss_Octavis", true))
		{
			System.out.println("Event 'SearchRaidBoss_Octavis' started.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Octavis.AnnounceEventStarted", null);
			if (_saveTask == null)
				_saveTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new SaveTask(), INITIAL_SAVE_DELAY, SAVE_INTERVAL);
			_snowmanState = SnowmanState.SAVED;
		}
		else
			player.sendMessage("Event 'SearchRaidBoss_Octavis' already started.");
	...
	public void stopEvent()
	{
		...
		if (!player.getAccessLevel().isGm())
			return;
		if (SetActive("SearchRaidBoss_Octavis", false))
		{
			if (_snowman != null)
				_snowman.deleteMe();
			if (_thomas != null)
				_thomas.deleteMe();
			System.out.println("Event 'SearchRaidBoss_Octavis' stopped.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Octavis.AnnounceEventStoped", null);
			if (_saveTask != null)
			{
			...
			player.sendMessage("Event 'SearchRaidBoss_Octavis' not started.");
	...
	{
		if (_active)
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Octavis.AnnounceEventStarted", null);
	}
	
	private static Location getRandomSpawnPoint()
	{
		return new Location(-213464, 244904, 1993);
	}
	...
	public void captureSnowman()
	{
		...
		{
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Octavis.AnnounceSnowmanCaptured", null, ChatType.CRITICAL_ANNOUNCE);
		...
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Octavis.captureSnowman template is null for npc: " + МилаяКоровка);
			Thread.dumpStack();
			return;
		}
		...
		// Спауним Томаса
		template = NpcHolder.getInstance().getTemplate(Octavis);
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Octavis.captureSnowman template is null for npc: " + Octavis);
			Thread.dumpStack();
			return;
		}
		...
		_eatTask = executeTask("events.SearchRaidBoss_Octavis.SearchRaidBoss_Octavis", "eatSnowman", new Object[0], THOMAS_EAT_DELAY);
	}
	...
	public static void eatSnowman()
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Octavis.AnnounceSnowmanKilled", null, ChatType.CRITICAL_ANNOUNCE);
	...
	public static void freeSnowman(Creature topDamager)
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Octavis.AnnounceSnowmanSaved", null, ChatType.CRITICAL_ANNOUNCE);
		...
		Functions.npcSayCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Octavis.SnowmanSayTnx", player.getName());
	...
	public class ShoutTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			Functions.npcShoutCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Octavis.SnowmanShout");
		}
	}
