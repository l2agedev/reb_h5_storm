...
public class SearchRaidBoss_ScarletVanHalisha extends Functions implements ScriptFile, OnPlayerEnterListener
{
	...
	private static final int INITIAL_SAVE_DELAY = 360 * 60 * 1000; // 10 мин
	...
	private static final int ScarletVanHalisha = 70006;
	...
	public void onLoad()
	{
		...
		{
			_active = true;
			_log.info("Loaded Event: SearchRaidBoss_ScarletVanHalisha [state: activated]");
			...
			_log.info("Loaded Event: SearchRaidBoss_ScarletVanHalisha [state: deactivated]");
	}
	...
	{
		return IsActive("SearchRaidBoss_ScarletVanHalisha");
	}
	...
	public void startEvent()
	{
		...
		if (SetActive("SearchRaidBoss_ScarletVanHalisha", true))
		{
			System.out.println("Event 'SearchRaidBoss_ScarletVanHalisha' started.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_ScarletVanHalisha.AnnounceEventStarted", null);
			if (_saveTask == null)
				_saveTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new SaveTask(), INITIAL_SAVE_DELAY, SAVE_INTERVAL);
			_snowmanState = SnowmanState.SAVED;
		}
		else
			player.sendMessage("Event 'SearchRaidBoss_ScarletVanHalisha' already started.");
	...
	public void stopEvent()
	{
		...
		if (!player.getAccessLevel().isGm())
			return;
		if (SetActive("SearchRaidBoss_ScarletVanHalisha", false))
		{
			if (_snowman != null)
				_snowman.deleteMe();
			if (_thomas != null)
				_thomas.deleteMe();
			System.out.println("Event 'SearchRaidBoss_ScarletVanHalisha' stopped.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_ScarletVanHalisha.AnnounceEventStoped", null);
			if (_saveTask != null)
			{
			...
			player.sendMessage("Event 'SearchRaidBoss_ScarletVanHalisha' not started.");
	...
	{
		if (_active)
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_ScarletVanHalisha.AnnounceEventStarted", null);
	}
	
	private static Location getRandomSpawnPoint()
	{
		return new Location(-22408, 183096, -2928);
	}
	...
	public void captureSnowman()
	{
		...
		{
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_ScarletVanHalisha.AnnounceSnowmanCaptured", null, ChatType.CRITICAL_ANNOUNCE);
		...
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_ScarletVanHalisha.captureSnowman template is null for npc: " + МилаяКоровка);
			Thread.dumpStack();
			return;
		}
		...
		// Спауним Томаса
		template = NpcHolder.getInstance().getTemplate(ScarletVanHalisha);
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_ScarletVanHalisha.captureSnowman template is null for npc: " + ScarletVanHalisha);
			Thread.dumpStack();
			return;
		}
		...
		_eatTask = executeTask("events.SearchRaidBoss_ScarletVanHalisha.SearchRaidBoss_ScarletVanHalisha", "eatSnowman", new Object[0], THOMAS_EAT_DELAY);
	}
	...
	public static void eatSnowman()
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_ScarletVanHalisha.AnnounceSnowmanKilled", null, ChatType.CRITICAL_ANNOUNCE);
	...
	public static void freeSnowman(Creature topDamager)
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_ScarletVanHalisha.AnnounceSnowmanSaved", null, ChatType.CRITICAL_ANNOUNCE);
		...
		Functions.npcSayCustomMessage(_snowman, "scripts.events.SearchRaidBoss_ScarletVanHalisha.SnowmanSayTnx", player.getName());
	...
	public class ShoutTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			Functions.npcShoutCustomMessage(_snowman, "scripts.events.SearchRaidBoss_ScarletVanHalisha.SnowmanShout");
		}
	}
