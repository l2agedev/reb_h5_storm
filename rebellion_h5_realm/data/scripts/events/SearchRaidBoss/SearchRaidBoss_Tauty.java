...
public class SearchRaidBoss_Tauty extends Functions implements ScriptFile, OnPlayerEnterListener
{
	...
	private static final int INITIAL_SAVE_DELAY = 390 * 60 * 1000; // 10 мин
	...
	private static final int Tauty = 70012;
	...
	public void onLoad()
	{
		...
		{
			_active = true;
			_log.info("Loaded Event: SearchRaidBoss_Tauty [state: activated]");
			...
			_log.info("Loaded Event: SearchRaidBoss_Tauty [state: deactivated]");
	}
	...
	{
		return IsActive("SearchRaidBoss_Tauty");
	}
	...
	public void startEvent()
	{
		...
		if (SetActive("SearchRaidBoss_Tauty", true))
		{
			System.out.println("Event 'SearchRaidBoss_Tauty' started.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Tauty.AnnounceEventStarted", null);
			if (_saveTask == null)
				_saveTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new SaveTask(), INITIAL_SAVE_DELAY, SAVE_INTERVAL);
			_snowmanState = SnowmanState.SAVED;
		}
		else
			player.sendMessage("Event 'SearchRaidBoss_Tauty' already started.");
	...
	public void stopEvent()
	{
		...
		if (!player.getAccessLevel().isGm())
			return;
		if (SetActive("SearchRaidBoss_Tauty", false))
		{
			if (_snowman != null)
				_snowman.deleteMe();
			if (_thomas != null)
				_thomas.deleteMe();
			System.out.println("Event 'SearchRaidBoss_Tauty' stopped.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Tauty.AnnounceEventStoped", null);
			if (_saveTask != null)
			{
			...
			player.sendMessage("Event 'SearchRaidBoss_Tauty' not started.");
	...
	{
		if (_active)
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Tauty.AnnounceEventStarted", null);
	}
	
	private static Location getRandomSpawnPoint()
	{
		return new Location(108456, 59592, -3747);
	}
	...
	public void captureSnowman()
	{
		...
		{
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Tauty.AnnounceSnowmanCaptured", null, ChatType.CRITICAL_ANNOUNCE);
		...
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Tauty.captureSnowman template is null for npc: " + МилаяКоровка);
			Thread.dumpStack();
			return;
		}
		...
		// Спауним Томаса
		template = NpcHolder.getInstance().getTemplate(Tauty);
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Tauty.captureSnowman template is null for npc: " + Tauty);
			Thread.dumpStack();
			return;
		}
		...
		_eatTask = executeTask("events.SearchRaidBoss_Tauty.SearchRaidBoss_Tauty", "eatSnowman", new Object[0], THOMAS_EAT_DELAY);
	}
	...
	public static void eatSnowman()
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Tauty.AnnounceSnowmanKilled", null, ChatType.CRITICAL_ANNOUNCE);
	...
	public static void freeSnowman(Creature topDamager)
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Tauty.AnnounceSnowmanSaved", null, ChatType.CRITICAL_ANNOUNCE);
		...
		Functions.npcSayCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Tauty.SnowmanSayTnx", player.getName());
	...
	public class ShoutTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			Functions.npcShoutCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Tauty.SnowmanShout");
		}
	}
