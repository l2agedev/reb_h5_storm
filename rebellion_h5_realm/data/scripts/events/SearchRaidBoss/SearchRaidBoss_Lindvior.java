...
public class SearchRaidBoss_Lindvior extends Functions implements ScriptFile, OnPlayerEnterListener
{
	...
	private static final int INITIAL_SAVE_DELAY = 240 * 60 * 1000; // 10 мин
	...
	private static final int Lindvior = 70013;
	...
	public void onLoad()
	{
		...
		{
			_active = true;
			_log.info("Loaded Event: SearchRaidBoss_Lindvior [state: activated]");
			...
			_log.info("Loaded Event: SearchRaidBoss_Lindvior [state: deactivated]");
	}
	...
	{
		return IsActive("SearchRaidBoss_Lindvior");
	}
	...
	public void startEvent()
	{
		...
		if (SetActive("SearchRaidBoss_Lindvior", true))
		{
			System.out.println("Event 'SearchRaidBoss_Lindvior' started.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Lindvior.AnnounceEventStarted", null);
			if (_saveTask == null)
				_saveTask = ThreadPoolManager.getInstance().scheduleAtFixedRate(new SaveTask(), INITIAL_SAVE_DELAY, SAVE_INTERVAL);
			_snowmanState = SnowmanState.SAVED;
		}
		else
			player.sendMessage("Event 'SearchRaidBoss_Lindvior' already started.");
	...
	public void stopEvent()
	{
		...
		if (!player.getAccessLevel().isGm())
			return;
		if (SetActive("SearchRaidBoss_Lindvior", false))
		{
			if (_snowman != null)
				_snowman.deleteMe();
			if (_thomas != null)
				_thomas.deleteMe();
			System.out.println("Event 'SearchRaidBoss_Lindvior' stopped.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.SearchRaidBoss_Lindvior.AnnounceEventStoped", null);
			if (_saveTask != null)
			{
			...
			player.sendMessage("Event 'SearchRaidBoss_Lindvior' not started.");
	...
	{
		if (_active)
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Lindvior.AnnounceEventStarted", null);
	}
	
	private static Location getRandomSpawnPoint()
	{
		return new Location(59448, 8920, -3707);
	}
	...
	public void captureSnowman()
	{
		...
		{
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Lindvior.AnnounceSnowmanCaptured", null, ChatType.CRITICAL_ANNOUNCE);
		...
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Lindvior.captureSnowman template is null for npc: " + МилаяКоровка);
			Thread.dumpStack();
			return;
		}
		...
		// Спауним Томаса
		template = NpcHolder.getInstance().getTemplate(Lindvior);
		if (template == null)
		{
			System.out.println("WARNING! events.SearchRaidBoss_Lindvior.captureSnowman template is null for npc: " + Lindvior);
			Thread.dumpStack();
			return;
		}
		...
		_eatTask = executeTask("events.SearchRaidBoss_Lindvior.SearchRaidBoss_Lindvior", "eatSnowman", new Object[0], THOMAS_EAT_DELAY);
	}
	...
	public static void eatSnowman()
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Lindvior.AnnounceSnowmanKilled", null, ChatType.CRITICAL_ANNOUNCE);
	...
	public static void freeSnowman(Creature topDamager)
	{
		...
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
			Announcements.getInstance().announceToPlayerByCustomMessage(player, "scripts.events.SearchRaidBoss_Lindvior.AnnounceSnowmanSaved", null, ChatType.CRITICAL_ANNOUNCE);
		...
		Functions.npcSayCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Lindvior.SnowmanSayTnx", player.getName());
	...
	public class ShoutTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			...
			Functions.npcShoutCustomMessage(_snowman, "scripts.events.SearchRaidBoss_Lindvior.SnowmanShout");
		}
	}
