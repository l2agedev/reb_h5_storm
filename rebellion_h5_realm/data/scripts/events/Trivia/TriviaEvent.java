...
public class TriviaEvent extends Functions implements ScriptFile, IVoicedCommandHandler, OnPlayerEnterListener
{
	...
	{
		File file = new File(Config.DATAPACK_ROOT + "/config/events/TriviaQuestions.txt");
	...
	{
		Say2 cs = new Say2(0, ChatType.TELL, "Trivia", text);
	...
	{
		Say2 cs = new Say2(0, ChatType.TELL, "Trivia", " To cancel participation in trivia event just type .toff for more info type .thelp");
		for (Player player : GameObjectsStorage.getAllPlayersForIterate())
	...
	public void Start()
	{
		...
		Calendar _timeStartTrivia = Calendar.getInstance();
		...
		Calendar _timeStopTrivia = Calendar.getInstance();
	...
	{
		Say2 cs = new Say2(0, type, "Trivia", text);
		for(Player player : GameObjectsStorage.getAllPlayersForIterate())
			player.sendPacket(cs);
	}
	...
	{
		_items.clear();
		StringTokenizer st = new StringTokenizer(Config.TRIVIA_REWARD_FIRST, ";");
		StringTokenizer str = new StringTokenizer(Config.TRIVIA_REWARD_REST, ";");
	...
	private boolean giveItemByChance(Player player, boolean first)
	{
		...
		for (RewardList items : _items)
		{
			...
			if (first == items.getFirst() && chancesumm > chance)
			{
				...
				addItem(player, productId, count, true);
				if (DEBUG_TRIVIA)
	...
	{
		Say2 cs = new Say2(0, ChatType.BATTLEFIELD, "Trivia", "Trivia Event is Active! To participate, type the command .ton! For reference .thelp!");
		if (isStatus())
			player.sendPacket(cs);
	}
