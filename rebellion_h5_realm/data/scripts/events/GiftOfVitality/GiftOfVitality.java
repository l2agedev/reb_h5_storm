...
public class GiftOfVitality extends Functions implements ScriptFile
{
	...
	private static final int REUSE_HOURS = 8; // reuse
	...
	public void startEvent()
	{
		...
		if(SetActive(EVENT_NAME, true))
		{
			...
			player.sendMessage(new CustomMessage("scripts.events.GiftOfVitality.EventStarted", player));
	...
	public void stopEvent()
	{
		...
		if(SetActive(EVENT_NAME, false))
		{
			...
			System.out.println(player.isLangRus() ? "Событие 'Gift Of Vitality' остановившийся." : "Event: 'Gift Of Vitality' stopped.");
			Announcements.getInstance().announceByCustomMessage("scripts.events.GiftOfVitality.AnnounceEventStoped", null);
		}
		else
			player.sendMessage(new CustomMessage("scripts.events.GiftOfVitality.EventStoped", player));
