...
public class _715_PathToBecomingALordGoddard extends Quest implements ScriptFile
{
	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(st.getCond() == 2 && npcId == FireSpiritNastron)
			st.setCond(4);
		else if(st.getCond() == 3 && npcId == WaterSpiritAshutar)
			st.setCond(5);

		if(st.getCond() == 6 && npcId == WaterSpiritAshutar)
			st.setCond(9);
		else if(st.getCond() == 7 && npcId == FireSpiritNastron)
			st.setCond(8);
		return null;
	}
