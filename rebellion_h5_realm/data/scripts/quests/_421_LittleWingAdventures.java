...
public class _421_LittleWingAdventures extends Quest implements ScriptFile
{
	...
	// Items
	private static int Dragonflute_of_Wind = Pet.HATCHLING_WIND.getControlItemId();
	private static int Dragonflute_of_Star = Pet.HATCHLING_STAR.getControlItemId();
	private static int Dragonflute_of_Twilight = Pet.HATCHLING_TWILIGHT.getControlItemId();
	private static int Dragon_Bugle_of_Wind = Pet.STRIDER_WIND.getControlItemId();
	private static int Dragon_Bugle_of_Star = Pet.STRIDER_STAR.getControlItemId();
	private static int Dragon_Bugle_of_Twilight = Pet.STRIDER_TWILIGHT.getControlItemId();
	// Quest Items
	...
	/*
	 * благодаря ai.Quest421FairyTree вызовется только при атаке от PetInstance
	 */
	@Override
	...
	{
		if(st.getState() != STARTED || st.getCond() != 2 || !HatchlingSummoned(st, true) || st.getQuestItemsCount(Fairy_Leaf) == 0 || CheckTree(st, npc.getNpcId()))
			return null;
		...
		public GuardiansSpawner(NpcInstance npc, QuestState st, int _count)
		{
			...
			for(int i = 0; i < _count; i++)
			{
				...
				if(st.getPlayer().getParty() != null)
				{
					...
					for(Player _member : st.getPlayer().getParty())
