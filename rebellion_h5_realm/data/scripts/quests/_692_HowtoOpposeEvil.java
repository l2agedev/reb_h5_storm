	...
	public String onKill(NpcInstance npc, QuestState st)
	{
		...
		if(cond == 3)
		{
			if(ArrayUtils.contains(SOD, npcId))
				st.rollAndGive(FleetSteedTroupsTotem, (int) Config.RATE_QUESTS_DROP * 1, 17); //Шанс на глаз и с потолка. Умножается на рейт квестов и делится на кол-во членов группы. Для нас это (nRate*3)/9
			else if(ArrayUtils.contains(SOI, npcId))
				st.rollAndGive(NucleusofanIncompleteSoul, (int) Config.RATE_QUESTS_DROP * 1, 17); //Шанс на глаз и с потолка. Умножается на рейт квестов и делится на кол-во членов группы. Для нас это (nRate*3)/9
			else if(ArrayUtils.contains(SOA, npcId))
				st.rollAndGive(SpiritStoneDust, (int) Config.RATE_QUESTS_DROP * 1, 20); //Шанс на глаз и с потолка. Умножается на рейт квестов и делится на кол-во членов группы. Для нас это (nRate*3)/9
		}
		return null;
	}
}