...
public class _351_BlackSwan extends Quest implements ScriptFile
{
	...
	public String onEvent(String event, QuestState st, NpcInstance npc)
	{
		...
		long fangs = st.getQuestItemsCount(LIZARD_FANG);
		...
		else if(event.equalsIgnoreCase("30969-02a.htm") && fangs > 0)
		{
			htmltext = "30969-02.htm";
			st.giveItems(ADENA_ID, fangs * 30, true);
			st.takeItems(LIZARD_FANG, -1);
		}
		else if(event.equalsIgnoreCase("30969-03a.htm") && barrels > 0)
		{
			htmltext = "30969-03.htm";
			st.setCond(2);
			st.giveItems(ADENA_ID, barrels * 500, true);
			st.giveItems(BILL_OF_IASON_HEINE, barrels, true);
