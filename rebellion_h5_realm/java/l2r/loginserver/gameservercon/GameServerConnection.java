...
public class GameServerConnection
{
	...
	private class PingTask extends RunnableImpl
	{
		...
		public void runImpl()
		{
			if(Config.GAME_SERVER_PING_RETRY > 0)
			{
				...
				{
					_log.warn("Gameserver " + gameServer.getIds()[0] + "(" + gameServer.getIds().length + ") [" + gameServer.getName() + "] : ping timeout!");
					closeNow();
					return;
				}
			}
	...
	public void onDisconnection()
	{
		try
		{
			...
			{
				_log.info("Connection with gameserver " + gameServer.getIds()[0] + "(" + gameServer.getIds().length + ") [" + gameServer.getName() + "] lost.");
