...
public final class ServerListFake extends L2LoginServerPacket
{
	...
	public ServerListFake(Account account)
	{
		...
		for(GameServer gs : GameServerManager.getInstance().getGameServers())
		{
			InetAddress ip;
			try
			{//TODO: Add multiple IPs support?
				ip = NetUtils.isInternalIP(account.getLastIP()) ? gs.getInternalHost() : gs.getExternalHost(gs.getIds()[0]);
			}
			...
			_servers.add(new ServerData(gs.getIds()[0], ip, gs.getPort(gs.getIds()[0]), gs.isPvp(), gs.isShowingBrackets(), gs.getServerType(), gs.getOnline(), gs.getMaxPlayers(), gs.isOnline(), entry == null ? 0 : entry.getKey(), gs.getAgeLimit(), entry == null ? ArrayUtils.EMPTY_INT_ARRAY : entry.getValue()));
		}
	}
