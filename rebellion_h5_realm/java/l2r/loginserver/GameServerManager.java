...
public class GameServerManager
{
	...
	private void load()
	{
		...
		try
		{
			...
			while(rset.next())
			{
				...
				GameServer gs = new GameServer(new int[]{id});
	...
	public List<GameServer> getGameServers()
	{
		...
		try
		{
			// Filter mirror GSs and return only the originals
			List<GameServer> ret = new ArrayList<GameServer>();
			for (GameServer gs : _gameServers.values())
			{
				if (!ret.contains(gs))
					ret.add(gs);
			}
	 ...
	 * @return true если игрвоой сервер успешно зарегистрирован
	 */
	public boolean registerGameServer(int[] ids, GameServer gs)
	{
		_writeLock.lock();
		try
		{
			GameServer pgs = _gameServers.get(ids[0]);
