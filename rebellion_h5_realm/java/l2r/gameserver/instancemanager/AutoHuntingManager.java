...
public class AutoHuntingManager
{
	...
	public synchronized void reportBot(Player reported, Player reporter, String typeOfReport, String moreInfo)
	{
		...
		/*
		if (!reported.isInCombat() && !reported.isInPeaceZone())
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message7", reporter));
			return;
		}
		*/
		...
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message8", reporter));
			return;
		}
		
		if (NexusEvents.isInEvent(reported) && !typeOfReport.equalsIgnoreCase("Abuse"))
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message9", reporter));
			return;
		}
		...
		try
		{
			...
			{
				FastTable<Player> p = new FastTable<Player>();
				p.add(reported);
		...
		AdminTable.broadcastToGMs(new Say2(0, ChatType.HERO_VOICE, "REPORT", msgContent));
		...
		if (reportedIsOnline(reported) && !reported.isInPeaceZone() && typeOfReport.equalsIgnoreCase("Bot"))
		{
			if (reported.getVarLong("LastCaptchaTest", 0) + Config.CAPTCHA_TIME_BETWEEN_TESTED_SECONDS * 1000L > System.currentTimeMillis())
			{
				...
				AdminTable.broadcastToGMs(new Say2(0, ChatType.HERO_VOICE, "REPORT", reported.getName() + " was tested lately, ignoring CAPTCHA request."));
				return;
			}
			
			String customHtm = HtmCache.getInstance().getNotNull("mods/Captcha/ShowCaptchaWindow.htm", reported);
			...
			try
			{
				...
				html.replace("%serverId%", "" + reported.getServerId());
				...
				AdminTable.broadcastToGMs(new Say2(0, ChatType.HERO_VOICE, "REPORT", reported.getName() + "  is being tested with CAPTCHA code..."));
			}
			catch (Exception e)
			{
				_log.error("", e);
	...
	public boolean validateBot(Player reported, Player reporter)
	{
		...
		*/
		...
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message6", reporter));
			return false;
		}
		...
		{
			reporter.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.autohuntingmanager.message5", reporter));
			return false;
		}
	...
	public synchronized boolean validateReport(Player reporter)
	{
		...
		else if (_lockedReporters.containsKey(reporter.getObjectId()))
		{
			...
			{
				int left = (int) (Config.CAPTCHA_TIME_BETWEEN_REPORTS_SECONDS * 1000 - delay) / 60000;
				SystemMessage2 sm = new SystemMessage2(SystemMsg.YOU_CAN_REPORT_IN_S1_MINUTES_S2_REPORT_POINTS_REMAIN_IN_ACCOUNT);
				sm.addNumber(left);
