		...
		try
		{
			...
			for(Player player : GameObjectsStorage.getAllPlayers())
			{
				...
				if(profit > 0)
				{
					...
					player.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.smswaytopay.message1", player, Config.SMS_PAYMENT_SERVER_ADDRESS));
					for(int i = 0; i < Config.SMS_PAYMENT_REWARD.length; i += 2)
					{
						if(Config.SMS_PAYMENT_REWARD[i] == -100) // PC Bang
						{
							...
							Log.addGame(player.getName() + " | " + player.getObjectId() + " | SMS reward item ID | " + Config.SMS_PAYMENT_REWARD[i] + " | SMS reward count | " + (int) (Config.SMS_PAYMENT_REWARD[i + 1] * profit) + " |", "sms");
						}
