		...
		for(Castle c : castleList)
		{
			...
			for(CropProcure crop : c.getCropProcure(PERIOD_CURRENT))
			{
				...
				if(crop.getAmount() > 0)
				{
					...
					Log.addGame(c.getName() + "|" + crop.getAmount() * crop.getPrice() + "|ManorManager|" + crop.getAmount() + "*" + crop.getPrice(), "treasury");
				}
			...
			if(c.getTreasury() < manor_cost)
			{
				...
				Log.addGame(c.getName() + "|" + manor_cost + "|ManorManager Error@setNextPeriod", "treasury");
			}
			else
			{
				List<SeedProduction> production = new ArrayList<SeedProduction>();
	...
	public void approveNextPeriod()
	{
		...
		for(Castle c : castleList)
		{
			...
			if(c.getTreasury() < manor_cost)
			{
				...
				if(manor_cost > 0)
					Log.addGame(c.getName() + "|" + -manor_cost + "|ManorManager Error@approveNextPeriod", "treasury");
			...
			else
			{
				...
				Log.addGame(c.getName() + "|" + -manor_cost + "|ManorManager", "treasury");
			}
