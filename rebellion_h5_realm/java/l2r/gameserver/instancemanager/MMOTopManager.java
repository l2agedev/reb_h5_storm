		...
		try
		{
			...
			for(Player player : GameObjectsStorage.getAllPlayers())
			{
				...
				if(mult > 0)
				{
					...
					player.sendMessage(new CustomMessage("l2r.gameserver.instancemanager.mmotopmanager.message1", player, Config.L2_TOP_SERVER_ADDRESS));
					for(int i = 0; i < Config.MMO_TOP_REWARD.length; i += 2)
					{
						if(Config.MMO_TOP_REWARD[i] == -100) // PC Bang
						{
							...
							Log.addGame(player.getName() + " | " + player.getObjectId() + " | MMOTop reward item ID | " + Config.MMO_TOP_REWARD[i] + " | MMOTop reward count | " + (Config.MMO_TOP_REWARD[i + 1] * mult) + " |", "mmotop");
						}
