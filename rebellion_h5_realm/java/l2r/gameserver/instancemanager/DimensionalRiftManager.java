...
public class DimensionalRiftManager
{
	...
	public void load()
	{
		...
		_log.info("DimensionalRiftManager: Loaded " + countGood + " DimensionalRift spawns, " + countBad + " bad spawns.");
	}
	...
	public void start(Player player, int type, NpcInstance npc)
	{
		...
		if(!player.canOverrideCond(PcCondOverride.INSTANCE_CONDITIONS))
		{
			...
			for(Player p : player.getParty())
			{
				if(ItemFunctions.removeItem(p, DIMENSIONAL_FRAGMENT_ITEM_ID, getNeededItems(type), true) == 0)
				{
					showHtmlFile(player, "rift/NoFragments.htm", npc);
					return;
				}
