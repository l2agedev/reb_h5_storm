...
public class Recall extends Skill
{
	...
	public boolean checkCondition(Creature activeChar, Creature target, boolean forceUse, boolean dontMove, boolean first)
	{
		...
		if(activeChar.isPlayer())
		{
			...
			{
				activeChar.sendMessage("Cannot use it while in event.");
				return false;
			}
			if(p.isInJail())
			{
				activeChar.sendMessage("You cannot use this while in jail");
				return false;
			}
		...
		if((getSkillType() == SkillType.RECALL && getTargetType() == SkillTargetType.TARGET_PARTY && activeChar.getReflection() != ReflectionManager.DEFAULT) || _townId > 0 && activeChar.getReflection() != null && activeChar.getReflection().getCoreLoc() != null)
		{
			if(activeChar.isPlayer())
