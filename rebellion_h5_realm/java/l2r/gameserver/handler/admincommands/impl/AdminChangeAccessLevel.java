	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_moders_add:
				...
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Неправильная цель. Пожалуйста, выберите игрока." : "Incorrect target. Please select a player.");
					showModersPannel(activeChar);
					return false;
				}
				...
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Ошибка: Модератор " + modAdd.getName() + " уже в списке серверов доступа." : "Error: Moderator " + modAdd.getName() + " already in server access list.");
					showModersPannel(activeChar);
					return false;
				}
				...
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Ошибка: Не удалось скопировать файл доступа template/moderator.xml. " : "Error: Failed to copy access-file template/moderator.xml.");
					showModersPannel(activeChar);
					return false;
				}
				...
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Ошибка: не удалось изменить идентификатор объекта в доступе-файл." : "Error: Failed to modify object ID in access-file.");
				...
				if(!af.exists())
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Ошибка: не удалось доступа на чтение-файл для " + modAdd.getName() : "Error: Failed to read access-file for " + modAdd.getName());
					showModersPannel(activeChar);
					return false;
				}
				...
				activeChar.sendMessage(activeChar.isLangRus() ? "Модератор " + modAdd.getName() + " добавленный." : "Moderator " + modAdd.getName() + " added.");
				showModersPannel(activeChar);
				break;
			case admin_moders_del:
				if(wordList.length < 2)
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Пожалуйста, сформулируйте модератором ID объекта для удаления модератором." : "Please specify moderator object ID to delete moderator.");
					showModersPannel(activeChar);
					return false;
				}
				...
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Ошибка: Модератор с идентификатором объекта " + oid + " не найден в литов доступа к серверу." : "Error: Moderator with object ID " + oid + " not found in server access lits.");
					showModersPannel(activeChar);
					return false;
				}
				...
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Ошибка: Не удается удалить файл доступа: " + fname : "Error: Can't delete access-file: " + fname);
					showModersPannel(activeChar);
					return false;
				}

				if(modDel != null)
					activeChar.sendMessage(activeChar.isLangRus() ? "Модератор " + modDel.getName() + " удаленный." : "Moderator " + modDel.getName() + " deleted.");
				else
					activeChar.sendMessage(activeChar.isLangRus() ? "Модератор с идентификатором объекта " + oid + " удаленный." : "Moderator with object ID " + oid + " deleted.");
