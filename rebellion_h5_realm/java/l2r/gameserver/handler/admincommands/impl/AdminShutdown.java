		...
		try
		{
			switch(command)
			{
				case admin_server_shutdown:
					Shutdown.getInstance().schedule(activeChar.getName(), NumberUtils.toInt(wordList[1], -1), Shutdown.SHUTDOWN);
					break;
				case admin_server_restart:
					Shutdown.getInstance().schedule(activeChar.getName(), NumberUtils.toInt(wordList[1], -1), Shutdown.RESTART);
					break;
				case admin_server_abort:
					Shutdown.getInstance().cancel(activeChar.getName());
					break;
			}
		}
		catch(Exception e)
