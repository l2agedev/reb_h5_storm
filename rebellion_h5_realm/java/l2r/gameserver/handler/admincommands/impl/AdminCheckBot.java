...
public class AdminCheckBot implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(Enum comm, String[] wordList, String fullString, Player activeChar)
	{
		...
		switch(command)
		{
			...
			case admin_punish_bot:
			{
				if (wordList != null)
				{
					String characterName = wordList[1];
					if (characterName != null)
					{
							if (wordList[2].equalsIgnoreCase("CHATBAN"))
							{
								...
								if (punishTime != 0)
								{
									...
									activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincheckbot.you_chatbanned", activeChar, characterName, punishTime));
							...
							else if (wordList[2].equalsIgnoreCase("KICK"))
							{
								...
								Player plr = World.getPlayer(characterName);
								if (plr != null)
									plr.kick();
								activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincheckbot.kicked", activeChar, characterName));
								introduceNewPunishedBotAndClear(characterName);
							}
							else if (wordList[2].equalsIgnoreCase("BANCHAR"))
							{
								...
								if (punishTime != 0)
								{
									...
									activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincheckbot.banned", activeChar, characterName, punishTime));
							...
							{
								String accountName = CharacterDAO.getInstance().getAccountName(characterName);
								...
								if (accountName != null)
								{
									...
									GameClient client = AuthServerCommunication.getInstance().getAuthedClient(accountName);
									if(client != null)
									{
										...
										{
											player.kick();
											activeChar.sendMessage(new CustomMessage("l2r.gameserver.handler.admincommands.impl.admincheckbot.banned1", activeChar, characterName, accountName));
											introduceNewPunishedBotAndClear(characterName);
										}
									}
								}
	...
	{
		String html = HtmCache.getInstance().getNotNull("admin/reports.htm", activeChar);
	...
	private static void sendBotInfoPage(Player activeChar, int botId)
	{
		...
		String html = HtmCache.getInstance().getNotNull("admin/report_page.htm", activeChar);
		...
		String captchaFails = CharacterDAO.getInstance().getUserVar(report[0], "FailedOnCaptchaTest");
