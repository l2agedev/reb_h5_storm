...
public class VotingSettings extends DocumentParser
{
	...
	public void load()
	{
		...
		parseFile(Config.getFile("config/VotingReward.xml"));
				...
				for (Node b = n.getFirstChild(); b != null; b = b.getNextSibling())
				{
					switch (b.getNodeName())
					{
						case "config":
						{
							...
							boolean enabled = parseBoolean(attrs, "enableVoteReward", false);
							_enabled = enabled;
							break;
						}
						case "api":
						{
							attrs = b.getAttributes();
							String votingzone = parseString(attrs, "zone");
							int serverId = parseInteger(attrs, "id");
							String apiKey = parseString(attrs, "key");
						...
						{
							attrs = b.getAttributes();
							boolean enabled = parseBoolean(attrs, "enable");
							String hexColor = parseString(attrs, "color");
							if (enabled)
								_color = hexColor;
							break;
						}
