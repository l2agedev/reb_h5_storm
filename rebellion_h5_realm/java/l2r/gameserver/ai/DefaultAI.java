...
public class DefaultAI extends CharacterAI
{
	...
	protected boolean checkAggression(Creature target)
	{
		...
		if(target.isPlayable())
		{
			...
			/*if(target.isFollow && !target.isPlayer() && target.getFollowTarget() != null && target.getFollowTarget().isPlayer())
					return;*/
			if(target.getPlayer() != null && target.getPlayer().isGM() && target.isInvisible())
				return false;
		...
		else if(!actor.isAggressive() || !target.isInRangeZ(actor.getSpawnedLoc(), actor.getAggroRange()) || Math.abs(actor.getZ() - target.getZ()) > MAX_Z_AGGRO_RANGE)
			return false;
		else if (actor.getChampionTemplate() != null && actor.getChampionTemplate().isPassive) 
			return false;
	...
	/**
	 * @return true if the operation is performed, false if not
	 */
	...
	protected void onEvtThink()
	{
		...
		{
			teleportHome(); This is rather lame. GM spawns fuck-up and Benom is in siege zone anyway.
			return;
		}*/
	...
	protected void onEvtDead(Creature killer)
	{
		...
		{
			NpcInstance npc = SpawnTable.spawnSingle(transformer, actor.getLoc(), actor.getReflection()) ;
	...
	protected void onEvtAttacked(Creature attacker, int damage)
	{
		...
		if(player != null)
		{
			...
			if((SevenSigns.getInstance().isSealValidationPeriod() || SevenSigns.getInstance().isCompResultsPeriod()) && actor.isSevenSignsMonster())
			{
				...
				if(pcabal != wcabal && wcabal != SevenSigns.CABAL_NULL && !Config.ALLOW_FARM_IN_SEVENSIGN_IF_NOT_REGGED)
				{
					player.sendMessage(new CustomMessage("l2r.gameserver.ai.defaultai.teleported", player));
	...
	protected boolean maybeMoveToHome()
	{
		...
		if (Math.abs(sloc.z - actor.getZ()) > MAX_Z_AGGRO_RANGE && !isGlobalAI())
		{
			teleportHome();
			Location loc = actor.getLoc();
			if (!GameServer.getInstance().hasLoaded()) // Log incorrect spawn coords only on server load to prevent flood.
				_log.info("Npc with bad cordinates: " + actor.getName() + "(" + actor.getNpcId() + ") " + actor.getSpawnRange() + " New loc: x=" + loc.x + " y=" + loc.y + " z=" + loc.z);
		...
		*/
		...
		/* if it continue teleport bug use this
		...
		if(!actor.moveToLocation(x, y, z, 0, true) && !isInRange)
			teleportHome();
		*/
		
		Location pos = sloc.setR(actor).findPointToStay(Config.MAX_DRIFT_RANGE);
	...
	{
		NpcInstance actor = getActor();
		/* Infern0: TODO: experimental fix for wrong tp...
	    ...
	    Location sloc = sloc_tmp.setR(actor).findPointToStay(300, 300);
	    */
		Location sloc = actor.getSpawnedLoc();
		...
		/* Infern0: TODO: experimental fix for wrong tp...
		Location sloc2 = actor.getSpawnedLoc();
		Location tele_sloc = sloc2.setR(actor).findPointToStay(300, 300);
		*/
		if(teleport)
		{
			...
			*/
			...
			Location tpLoc = sloc.clone();
			tpLoc.findPointToStay(Config.MAX_DRIFT_RANGE);
			actor.teleToLocation(tpLoc);
		}
		else
		{
			if(!clearAggro)
	...
	protected Creature prepareTarget()
	{
		...
		if(Rnd.chance(actor.getParameter("isMadness", 0)))
		{
			...
			if(randomHated != null && Math.abs(actor.getZ() - randomHated.getZ()) < 1000) // Infern0, do not jump to target if Z cordinate is too big. Example: valakas
			{
		...
		{
			if (cha.isPet() || cha.isSummon() && cha.getPlayer() != null && getActor().getSkillLevel(6019) == 1 && checkTarget(cha.getPlayer(), MAX_PURSUE_RANGE))
				cha = cha.getPlayer();
