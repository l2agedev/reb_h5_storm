...
public final class NpcParser extends AbstractDirParser<NpcHolder>
{
			...
			for(Iterator<org.dom4j.Element> secondIterator = npcElement.elementIterator(); secondIterator.hasNext();)
			{
				...
				else if(!Config.DONTLOADNPCDROP && nodeName.equalsIgnoreCase("rewardlist"))
				{
					...
					for(Iterator<org.dom4j.Element> nextIterator = secondElement.elementIterator(); nextIterator.hasNext();)
					{
						...
						if(nextName.equalsIgnoreCase("group"))
						{
							...
							for(Iterator<org.dom4j.Element> rewardIterator = nextElement.elementIterator(); rewardIterator.hasNext();)
							{
								...
								RewardItem data = parseReward(rewardElement, 1);
					...
					if(type == RewardType.RATED_GROUPED || type == RewardType.NOT_RATED_GROUPED)
						if(!list.validate())
							info("Extended max chance of drop for : " + npcId + "; type: " + type);
	...
	private RewardItem parseReward(org.dom4j.Element rewardElement, int id)
	{
		...
		if (!Config.RATE_CHANCE_DROP_ITEMS_ID.isEmpty())
		{
			if (Config.RATE_CHANCE_DROP_ITEMS_ID.containsKey(itemId) && !(id == 2))
				chance *= Config.RATE_CHANCE_DROP_ITEMS_ID.get(itemId);
		}
		
		if (!Config.RATE_CHANCE_DROP_SPOIL_ITEMS_ID.isEmpty())
		{
			if (Config.RATE_CHANCE_DROP_SPOIL_ITEMS_ID.containsKey(itemId) && (id == 2))
				chance *= Config.RATE_CHANCE_DROP_SPOIL_ITEMS_ID.get(itemId);
		}
		...
		if(id == 1)
		{
			...
			else if(data.getItem().isEpaulette())
		...
		{
			if (Config.RATE_DROP_ITEMS_ID.containsKey(itemId) && !(id == 2))
			{
				min *= Config.RATE_DROP_ITEMS_ID.get(itemId);
				max *= Config.RATE_DROP_ITEMS_ID.get(itemId);
			}
		}
		
		if (!Config.RATE_DROP_SPOIL_ITEMS_ID.isEmpty())
		{
			if (Config.RATE_DROP_SPOIL_ITEMS_ID.containsKey(itemId) && id == 2)
			{
				min *= Config.RATE_DROP_SPOIL_ITEMS_ID.get(itemId);
				max *= Config.RATE_DROP_SPOIL_ITEMS_ID.get(itemId);
			}
		}
