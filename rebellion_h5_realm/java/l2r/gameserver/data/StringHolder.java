...
public final class StringHolder extends AbstractHolder
{
	...
	public String getSkillDescription(Language lang, Skill skill)
	{
		return getSkillDescription(lang, skill.getDisplayId(), skill.getDisplayLevel());
	}
	
	public String getSkillDescription(Player player, Skill skill)
	{
		return getSkillDescription(player, skill.getDisplayId(), skill.getDisplayLevel());
	}
	
	public String getSkillDescription(Language lang, int id, int level)
	{
		String skillName = getNullable(lang, "skill_desc_id_" + id + "_level_" + level);
		return skillName == null ? "" : skillName;
	}
	
	public String getSkillDescription(Player player, int id, int level)
	{
		Language lang = player == null ? Language.ENGLISH : player.getLanguage();
		return getSkillDescription(lang, id, level);
	}
