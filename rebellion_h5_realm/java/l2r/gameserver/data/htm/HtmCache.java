package l2r.gameserver.data.htm;
...
public class HtmCache
{
	...
	public void reload()
	{
		...
		switch (Config.HTM_CACHE_MODE)
		{
			case ENABLED:
				...
				{
					File root = new File(Config.DATAPACK_ROOT, "data/html/html-" + lang.getShortName());
					if(!root.exists())
					{
	...
	private String getContent(File file, String encoding) throws IOException
	{
		...
		{
			_log.info("HtmCache: getContent method error" + e, e);
		}
		finally
		{
			_stream.close();
	...
	public String getNotNull(String fileName, Player player)
	{
		...
		if ((fileName.contains("bbs_regiontpl.htm") 
			|| fileName.contains("bbs_region_stpl.htm") 
			|| fileName.contains("bbs_region_list.htm") 
			|| fileName.contains("bbs_region_sellers.htm") 
			|| fileName.contains("bbs_region_storetpl.htm")
			|| fileName.contains("bbs_region_view.htm.htm")
			&& player.isLangRus()))
		...
		{
			cache = "<html><body><br>&nbsp; Dialog not found: " + fileName + "; Lang: " + lang + "</body></html>";
			_log.warn("Dialog: " + "data/html/html-" + lang.getShortName() + "/" + fileName + " not found. Player: " + (player == null ? "" : player.getName()));
		}
	...
	{
		String cache = null;
		File f = new File(Config.DATAPACK_ROOT, "data/html/html-" + lang.getShortName() + "/" + file);
		if(f.exists())
		{
			try
			{
			...
			{
				_log.warn("HtmCache: File error: " + file + " lang: " + lang);
			}
		}
		return cache;
	}
