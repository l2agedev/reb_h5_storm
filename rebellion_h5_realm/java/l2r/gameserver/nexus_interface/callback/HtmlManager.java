...
public class HtmlManager extends EventHtmlManager
{
	...
	public boolean onBypass(PlayerEventInfo player, String bypass)
	{
		if(bypass.startsWith("npcbuffer"))
		{
			...
			if(player.getTarget() != null && player.getTarget().isNpc() && player.getTarget().getNpc().getNpcId() == EventConfig.getInstance().getGlobalConfigInt("assignedNpcId")
					&& player.getOwner().isInRange(player.getTarget().getOwner(), 150))
			{
...
//				tb.append("<td width=65 align=right><button value=\"Delete\" action=\"bypass -h nxs_buffer_delete_scheme " + schemes.getKey() + " " + returnPage + "\" width=65 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
	...
	protected void showBuffMeWindow(PlayerEventInfo player, String target)
	{
		...
		final int maxDances = getMaxDances(player);
		//int maxDances = 0;
	...
	protected void showCustomBufferMenu(PlayerEventInfo player)
	{
		...
		tb.append("<font color=9f9f9f>Your summon/pet's buff count: <font color=9f9f9f></font> <font color=ac9775>" + player.getSummonBuffCount() + " buffs</font></font><br1>");
		tb.append("<font color=9f9f9f>Your class is allowed to have: <font color=9f9f9f></font> <font color=ac9775>" + player.getMaxBuffs() + " buffs</font></font><br1>");*/
