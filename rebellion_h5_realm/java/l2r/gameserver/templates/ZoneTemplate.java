...
public class ZoneTemplate
{
	...
	private Location _teleloc = null;
	private final int _maxlvl;
	private final String[] _forbiddenSkills;
	private final boolean _pvpflagzone;
	...
	public ZoneTemplate(StatsSet set)
	{
		...
		_pvpflagzone = set.getBool("isPvPFlagZone", false);
		...
		_maxlvl = set.getInteger("levelLimit", 100);
		
		String tl = set.getString("teleportLoc", null);
		if (tl != null)
		{
			String[] sk = tl.split(" ");
			_teleloc = new Location(Integer.parseInt(sk[0]), Integer.parseInt(sk[1]), Integer.parseInt(sk[2]));
		}
	    
		s = (String) set.get("forbidden_skills");
		if(s != null)
			_forbiddenSkills = s.split(ExProperties.defaultDelimiter);
		else
			_forbiddenSkills = null;
	...
	public String[] getForibiddenSkills()
	{
		return _forbiddenSkills;
	}

	public boolean isPvPFlagZone()
	{
		return _pvpflagzone;
	}
}
