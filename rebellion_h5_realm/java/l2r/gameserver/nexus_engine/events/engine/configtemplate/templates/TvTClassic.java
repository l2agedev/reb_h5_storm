package l2r.gameserver.nexus_engine.events.engine.configtemplate.templates;

import l2r.gameserver.nexus_engine.events.engine.base.EventType;
import l2r.gameserver.nexus_engine.events.engine.configtemplate.ConfigTemplate;

/**
 * @author hNoke
 *
 */
public class TvTClassic extends ConfigTemplate
{
	private SetConfig[] configs = new SetConfig[]
			{
				//TvT event configs
				new SetConfig("killsForReward", "1"), // at least one kill required for a reward
				new SetConfig("resDelay", "15"), // 15 seconds to get respawned
				new SetConfig("waweRespawn", "false"), // no wawe respawn
	...
	{
		return "Classic settings for a regular TvT event, no wawe spawn. " +
