...
public class VIPTeamVsTeam extends TeamVsTeam
{
	...
	protected synchronized void selectVips(int instanceId, int teamId)
	{
		...
		List<PlayerEventInfo> possibleVips = new FastTable<PlayerEventInfo>();
		...
		for(PlayerEventInfo player : newVips)
		{
			...
			if(getBoolean("removeBuffsOnStart"))
				player.removeBuffs();
		}
	}
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("Event: marking " + player.getPlayersName() + " in instance " + player.getReflectionId() + " as VIP.");
		...
		if(!getPlayerData(player).isVip())
		{
			...
			if(!getEventData(player.getReflectionId())._vips.containsKey(player.getTeamId()))
				getEventData(player.getReflectionId())._vips.put(player.getTeamId(), new FastTable<PlayerEventInfo>());
			
			getEventData(player.getReflectionId())._vips.get(player.getTeamId()).add(player);
			
			player.setTitle(getTitle(player), true);
			player.broadcastTitleInfo();
			
			announce(player.getReflectionId(), "* " + LanguageEngine.getMsg("vip_becomeVip", player.getPlayersName()), player.getTeamId());
		}
	}
		...
		if(_skillsForVip != null)
		{
			for(Entry<Integer, Integer> sk : _skillsForVip.entrySet())
			{
				...
				if(add)
					...
					player.removeSkillById(skill.getId());
			}
		}
		if(add)
		{
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("Event: removing/cleaning " + player.getPlayersName() + " in instance " + player.getReflectionId() + " from VIP.");
		...
		if(getPlayerData(player).isVip())
		{
			...
			if(getEventData(player.getReflectionId())._vips.containsKey(player.getTeamId()))
				getEventData(player.getReflectionId())._vips.get(player.getTeamId()).remove(player);
	...
	public void onKill(PlayerEventInfo player, CharacterData target)
	{
		...
		if(getPlayerData(targetInfo).isVip())
		{
			...
			announceToAllTeamsBut(targetInfo.getReflectionId(), "[+] " + LanguageEngine.getMsg("vip_vipDied", targetInfo.getPlayersName(), targetInfo.getEventTeam().getTeamName()), targetInfo.getTeamId());
			announce(targetInfo.getReflectionId(), "[-] " + LanguageEngine.getMsg("vip_vipKilled", targetInfo.getPlayersName()), targetInfo.getTeamId());
			
			cleanVip(targetInfo);
			scheduleSelectVips(targetInfo.getReflectionId(), targetInfo.getTeamId(), false, false);
		}
	}
	...
	public void playerWentAfk(PlayerEventInfo player, boolean warningOnly, int afkTime)
	{
		...
		{
			if(_matches.get(player.getReflectionId())._state == EventState.END)
			{
				if(getPlayerData(player).isVIP)
				{
					announce(player.getReflectionId(), "* " + LanguageEngine.getMsg("vip_vipAfk", player.getPlayersName()), player.getTeamId());
					announceToAllTeamsBut(player.getReflectionId(), "* " + LanguageEngine.getMsg("vip_enemyVipAfk", player.getPlayersName()), player.getTeamId());
					player.doDie();
				}
			}
		} 
	...
	public synchronized void clearEvent(int instanceId)
	{
		...
		for (PlayerEventInfo player : getPlayers(instanceId))
		{
			...
			player.stopAbnormalEffect(AbnormalEffect.IMPRISIONING_1);
	...
	public void onDisconnect(PlayerEventInfo player)
	{
		...
		{
			cleanVip(player);
			announce(player.getReflectionId(), "* " + LanguageEngine.getMsg("vip_vipDisconnected", player.getPlayersName(), player.getEventTeam().getTeamName()));
			scheduleSelectVips(player.getReflectionId(), player.getTeamId(), false, true);
		}
	...
	int tick = 0;
	List<PlayerEventInfo> playersEffects = new FastTable<PlayerEventInfo>();
	
	@Override
	protected void clockTick() throws Exception 
	{
		final int healingRadius = _healingRadius;
	...
	private void startPlayerEffects(PlayerEventInfo player, int teamId)
	{
		if(teamId == 1)
		{
			player.stopAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
			player.startAbnormalEffect(AbnormalEffect.IMPRISIONING_1); // imprissing 1 effect
		}
		else if(teamId == 2)
		{
			player.stopAbnormalEffect(AbnormalEffect.IMPRISIONING_1); // imprissing 1 effect
			player.startAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
		}
		else // stop effects
		{
			player.stopAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
	...
	public class VIPTvTPlayerData extends PvPEventPlayerData
	{
		...
		private boolean wasVIP = false;
		private boolean tickHealed = false;
