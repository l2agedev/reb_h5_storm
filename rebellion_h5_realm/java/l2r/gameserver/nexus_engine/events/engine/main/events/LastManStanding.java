...
public class LastManStanding extends Deathmatch
{
	...
	{
		private boolean _isActive, _waitingState;
		...
		private class Timelimit
		{
			...
			public void onTick()
			{
				...
				if(!aborted && limit <= 0)
				{
					...
					/**/ if(NexusLoader.detailedDebug) print("Event: round ended due to time limit");
				}
			}
		}
		...
		protected boolean isActive() { return _isActive; }
		protected synchronized void setInactive() { _isActive = false; }
	}
	...
	public void loadConfigs()
	{
		...
		ConfigModel scorebarFormat = new ConfigModel("screenScoreBarFormat", "AliveAndRounds", "Specify here how will the player's screen Score bar look like. " +
				"<br1><font color=LEVEL>Alive</font> shows the count of players that are still alive, excluding you." +
				"<br1><font color=LEVEL>Time</font> shows the time left for the event (using Run time value)" +
				"<br1><font color=LEVEL>Rounds</font> shows the current round / max rounds in the event." +
				"<br1><font color=LEVEL>Top</font> shows the score of the top player in the event." +
				"<br1>Example: <font color=LEVEL>AliveAndRounds</font> will show following text: 'Alive: 12, Round: 1/3', where 12 is the count of alive players excluding you, 1 is the current round and 3 si the total count of rounds in this event (configurable)."
		, InputType.Enum);
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("Event: trying to start new round for instance " + instanceId);
		
		if(getEventData(instanceId).canStartNewRound())
		{
			/**/ if(NexusLoader.detailedDebug) print("Event: starting new round; current round = " + getEventData(instanceId)._round);
			...
			/**/ if(NexusLoader.detailedDebug) print("Event: new round started!");
		}
		else
		{
			/**/ if(NexusLoader.detailedDebug) print("Event: CAN'T START new round!");
	...
	private synchronized void endRound(final int instanceId, boolean aborted, boolean endInstance)
	{
		...
		/**/ if(NexusLoader.detailedDebug) print("Event: ending round of instance " + instanceId + " aborted = " + aborted + ", end instance " + endInstance);
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("Event: end due to round time = " + instanceId);  
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("Event: ending round due to event time limit " + instanceId + ", announce time limit = " + announceTimeLimit);
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("/// Event: onDie - player " + player.getPlayersName() + " (instance " + player.getReflectionId() + "), killer " + killer.getName());
		...
		if(getEventData(player.getReflectionId()).playerDied())
		{
			endRound(player.getReflectionId(), false, false);
		}
		else
		{
			if(getEventData(player.getReflectionId()).canStartNewRound())
	...
	public void playerWentAfk(PlayerEventInfo player, boolean warningOnly, int afkTime)
	{
		...
		{
			if(getEventData(player.getReflectionId())._roundActive)
			{
				announce(player.getReflectionId(), LanguageEngine.getMsg("event_afkMarked_andDied", player.getPlayersName()));
				player.doDie();
			}
		} 
	}
	...
	public void onDisconnect(PlayerEventInfo player)
	{
		...
		if(player.isOnline()) // still online
		{
			...
			if(running)
			{
				...
				if(checkIfAllDied(player.getReflectionId()))
				{
					endRound(player.getReflectionId(), false, false);
				}
			}
