...
public class MainEventInstanceTypeManager
{
	...
	private void loadAll()
	{
		...
		try
		{
			...
			statement = con.prepareStatement("SELECT event, id, name, visible_name, params FROM nexus_main_instances");
	...
	public void addInstanceType(MainEventInstanceType type, boolean storeToDb)
	{
		...
		if(storeToDb)
		{
			...
			{
				con = CallBack.getInstance().getOut().getConnection();
				statement = con.prepareStatement("REPLACE INTO nexus_main_instances VALUES (?,?,?,?,?)");
	...
	public void removeInstanceType(MainEventInstanceType type)
	{
		...
		{
			con = CallBack.getInstance().getOut().getConnection();
			statement = con.prepareStatement("DELETE FROM nexus_main_instances WHERE event = '" + event.getEventType().getAltTitle() + "' AND id = " + type.getId());
			statement.execute();
		}
