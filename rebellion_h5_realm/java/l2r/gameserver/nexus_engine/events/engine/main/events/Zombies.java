...
public class Zombies extends HuntingGrounds
{
	...
	private List<PlayerEventInfo> calculateZombies(int instanceId, int countToSpawn, boolean start)
	{
		...
		if(countToSpawn <= 0)
		{
			...
			if(start)
			{
				...
				if(countToSpawn < 3)
				{
					if(survivorsCount > 15)
						countToSpawn = 3;
					else if(survivorsCount > 6)
						countToSpawn = 2;
					else
						countToSpawn = 1;
				}
		...
		int i = 0;
		List<PlayerEventInfo> newZombies = new FastTable<PlayerEventInfo>();
	...
	private void transformToZombie(final PlayerEventInfo player)
	{
		...
		_teams.get(player.getReflectionId()).get(ZOMBIE_TEAM_ID).addPlayer(player, true);
		...
		if(checkIfAnyPlayersLeft(player.getReflectionId()))
		{
			CallBack.getInstance().getOut().scheduleGeneral(new Runnable()
			{
				@Override
		...
		else
		{
			try
			{
				if(player.getTeamId() == ZOMBIE_TEAM_ID)
				{
					...
					_teams.get(player.getReflectionId()).get(SURVIVOR_TEAM_ID).addPlayer(player, true);
					preparePlayer(player, true);
	...
	public void onKill(PlayerEventInfo player, CharacterData target)
	{
		...
		if(player.getPlayersId() != target.getObjectId())
		{
			...
			// Achievements
			if (Config.ENABLE_PLAYER_COUNTERS && player.getTeamId() == SURVIVOR_TEAM_ID)
				player.getOwner().getCounters().addPoint("_Killed_Zombies");
	...
	public void onDisconnect(PlayerEventInfo player)
	{
		...
		scheduleSelectZombies(player.getReflectionId(), 0, false, 0);
	}
	...
	{
		EventPlayerData d = new ZombiesEventPlayerData(player, this);
		return d;
	}
