...
public class Mutant extends Zombies
{
	...
	@Override
	protected int initInstanceTeams(MainEventInstanceType type, int reflectionId)
	{
		_teamsCount = 2;
	...
	protected List<PlayerEventInfo> calculateMutants(int instanceId, int countToSpawn, boolean start)
	{
		...
		int i = 0;
		List<PlayerEventInfo> newMutants = new FastTable<PlayerEventInfo>();
	...
	protected void preparePlayer(PlayerEventInfo player, boolean start)
	{
		...
		if(player.getEventTeam().getTeamId() == PLAYERS_TEAM_ID)
		{
			...
			if(player.getOwner() != null)
				player.getOwner().stopAbnormalEffect(AbnormalEffect.BIG_HEAD);
		}
		// zombies
		else if(player.getEventTeam().getTeamId() == MUTANT_TEAM_ID)
		{
			...
			{
				if(player.getOwner() != null)
					player.getOwner().startAbnormalEffect(AbnormalEffect.BIG_HEAD);
	...
	private void transformToMutant(final PlayerEventInfo player)
	{
		...
		_teams.get(player.getReflectionId()).get(MUTANT_TEAM_ID).addPlayer(player, true);
		...
		getEventData(player.getReflectionId()).setKillMade();
		...
		if(player.isDead())
		{
			CallBack.getInstance().getOut().scheduleGeneral(new Runnable()
			{
				...
				public void run()
				{
					if(player != null && player.isOnline() && getMatch(player.getReflectionId()).isActive())
					{
						respawnPlayer(player, player.getReflectionId());
		...
		else
		{
			try
			{
				if(player.getTeamId() == MUTANT_TEAM_ID)
				{
					...
					_teams.get(player.getReflectionId()).get(PLAYERS_TEAM_ID).addPlayer(player, true);
					preparePlayer(player, true);
					
					player.setTitle(getTitle(player), true);
					player.getOwner().setTitleColor(0x0000FF);
					player.broadcastTitleInfo();
	...
	public void onKill(PlayerEventInfo player, CharacterData target)
	{
		...
		if(player.getPlayersId() != target.getObjectId())
		{
			...
			// Achievements
			if (Config.ENABLE_PLAYER_COUNTERS && player.getTeamId() == PLAYERS_TEAM_ID)
				player.getOwner().getCounters().addPoint("_Killed_Mutants");
	...
	public void onDisconnect(PlayerEventInfo player)
	{
		...
		scheduleSelectMutants(player.getReflectionId(), 0, false, 0);
	}
	...
	{
		EventPlayerData d = new MutantEventPlayerData(player, this);
		return d;
	}
