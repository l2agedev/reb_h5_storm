...
public class Battlefield extends AbstractMainEvent
{
	protected class Tower
	{
		...
		public Tower(EventSpawn spawn, NpcData npc, int radius)
		{
			...
			_effects = new FastTable<PlayerEventInfo>();
		}
	...
	protected class TowerData extends AbstractEventData
	{
		...
		protected void addTower(NpcData base, int radius, EventSpawn spawn)
		{
			...
			{
				_towers[_order] = new Tower(spawn, base, radius > 0 ? radius : _towerRadius);
				_order ++;
			}
			else
	...
	public void initEvent()
	{
		...
		_towerRadius = getInt("baseRadius");
	...
	private void startRadar(int instanceId, EventTeam team)
	{
		...
		if(zone != null)
		{
			for(PlayerEventInfo pi : team.getPlayers())
			{
				if (pi == null || pi.getRadar() == null || zone.getLoc() == null)
					continue;
	...
	protected void setBaseEffects(int teamId, NpcData baseNpc)
	{
		if(getBoolean("allowBaseNpcEffects") && _teamsCount == 2)
		{
			if(teamId == 1)
			{
				baseNpc.stopAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
				baseNpc.startAbnormalEffect(AbnormalEffect.IMPRISIONING_1); // imprissing 1 effect
			}
			else if(teamId == 2)
			{
				baseNpc.stopAbnormalEffect(AbnormalEffect.IMPRISIONING_1); // imprissing 1 effect
				baseNpc.startAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
			}
			else // stop effects
			{
				baseNpc.stopAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
	...
	{
		EventPlayerData d = new PvPEventPlayerData(player, this, new GlobalStatsModel(getEventType()));
		return d;
	}
	...
	protected void clockTick() throws Exception 
	{
		...
		for(BattlefieldEventInstance instance : _matches.values())
		{
			...
			{
				players = new FastMap<Integer, List<PlayerEventInfo>>();
					...
					if(player.getDistanceSq(baseX, baseY, baseZ) <= radius)
					{
						if(player.isVisible() && !player.isDead())
						{
							if(!players.containsKey(player.getTeamId()))
								players.put(player.getTeamId(), new FastTable<PlayerEventInfo>());
								...
								if(getBoolean("allowPlayerEffects") && _teamsCount == 2)
								{
									for(PlayerEventInfo player : tower.getEffectedPlayers())
									{
										if(player != null)
										{
											...
											player.stopAbnormalEffect(player.getTeamId() == 1 ? AbnormalEffect.IMPRISIONING_1 : AbnormalEffect.REDCIRCLE);
										}
									}
				...
				if(tower.getOwningTeam() > 0)
				{
						...
						if(players.containsKey(tower.getOwningTeam()))
						{
							for(PlayerEventInfo player : players.get(tower.getOwningTeam()))
							{
								if(!tower.containsEffectedPlayer(player))
								{
									...
									player.startAbnormalEffect(player.getTeamId() == 1 ? AbnormalEffect.IMPRISIONING_1 : AbnormalEffect.REDCIRCLE);
								}
							}
						}
					...
					if(!ownedTowers.containsKey(tower.getOwningTeam()))
						ownedTowers.put(tower.getOwningTeam(), new FastTable<NpcData>());
	...
	public String getHtmlDescription()
	{
		if (_htmlDescription == null)
		{
			...
			{
				_htmlDescription = "This is a team-based event. The goal of this event is to capture bases. The count of bases is configurable, so is the method of capturing. ";
				_htmlDescription += "Usually, in order to capture a base, a majority of players from one team needs to stay close to the base (which is represented by an NPC) for ";
				_htmlDescription += "some time and don't let the other team come close. But this thing is very customizable, you will see yourself. ";
				_htmlDescription += "When a base is captured, it will stay captured untill another team captures it for itself. The team, which ownes most bases in the event gets most score. ";
				_htmlDescription += "This is a team-based event. The goal of this event is to capture bases. The count of bases is configurable, so is the method of capturing. ";
