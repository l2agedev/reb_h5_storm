...
public class CaptureTheFlag extends AbstractMainEvent
{
	...
	private int _sealSkillid = 246;
	protected boolean _sealOfRulerScoring;
	...
	public void loadConfigs()
	{
		...
		addConfig(new ConfigModel("scoreWithSealOfRuler", "false", "In order to score with a flag, you need to cast Seal of Ruler on your own flag/holder.", InputType.Boolean));
		addConfig(new ConfigModel("sealOfRulerSkillId", "246", "The ID of the skill the player has to cast in order to score with the flag. Advise: use custom skill, set it BUFF type."));
	...
	public void initEvent()
	{
		...
		_sealSkillid = getInt("sealOfRulerSkillId");
		_sealOfRulerScoring = getBoolean("scoreWithSealOfRuler");
	...
	protected String getTitle(PlayerEventInfo pi)
	{
		...
		return "Score: " + getPlayerData(pi).getScore() + " Kills: " + getPlayerData(pi).getKills();
	}
	
	@Override
	public synchronized boolean onNpcAction(PlayerEventInfo player, NpcData npc)
	{
		int instance = player.getReflectionId();
		CTFEventInstance match = _matches.get(instance);
		
		if(_interactDistCheck && player.getDistanceSq(npc.getLoc().getX(), npc.getLoc().getY(), npc.getLoc().getZ()) > 30000)
		{
		...
		if(isHolder)
		{
			...
			if(npcTeam == player.getTeamId())
			{
				if(status == 1)
				{
					...
					{
						if(_sealOfRulerScoring)
						{
							player.screenMessage("You need to cast Seal of Ruler skill to return the flag!", "CTF", true);
						}
						else
						{
		...
		else if(isFlag)
		{
			...
			if(npcTeam == player.getTeamId())
			{
				...
				else if(status == 3)
				{
					...
					// Achievement system.
					if (Config.ENABLE_PLAYER_COUNTERS && player != null || player.getOwner() != null)
						player.getOwner().getCounters().addPoint("_Returned_Flags");
	...
	public void onDie(PlayerEventInfo player, CharacterData killer)
	{
		...
		if(getPlayerData(player).hasFlag > 0)
		{
			if(_returnFlagOnDie)
			{
				screenAnnounce(player.getReflectionId(), LanguageEngine.getMsg("ctf_flagReturned2", getTeamName(getPlayerData(player).hasFlag)));
				
				EventRewardSystem.getInstance().rewardPlayer(getEventType(), 1, player, RewardPosition.FlagReturn, null, player.getTotalTimeAfk(), 0, 0);
				
				returnFlag(getPlayerData(player).hasFlag, false, false, player.getReflectionId(), false);
			}
			else
			{
				dropFlag(getPlayerData(player).hasFlag, player.getReflectionId());
			}
		}
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("/ Event: equipping flag of team " + flagTeamId + " on player " + player.getPlayersName() + " (instance " + player.getReflectionId() + ")");
		...
		// Achievement system.
		if (Config.ENABLE_PLAYER_COUNTERS && player != null || player.getOwner() != null)
			player.getOwner().getCounters().addPoint("_Captured_Flags");
		...
		if(_sealOfRulerScoring)
		{
			final int sealOfRulerid = _sealSkillid; 
			player.addSkill(new SkillData(sealOfRulerid, 1), false);
		}
		
		final int id = getInt("flagSkillId");
		if(id != -1)
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("/ Event: unequipping flag from " + player.getPlayersName() + " in instance " + player.getReflectionId());
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("/ Event: removing flag from player " + player.getPlayersName() + ", instance " + player.getReflectionId());
		...
		{
			final int sealOfRulerid = _sealSkillid; 
			player.removeSkillById(sealOfRulerid);
		}
	...
	private boolean targetsTheFlag(PlayerEventInfo player)
	{
		// player must have the flag
		if(getPlayerData(player).hasFlag() == 0)
			return false;
		
		ObjectData target = player.getTarget();
	...
	public boolean canUseSkill(PlayerEventInfo player, SkillData skill)
	{
		...
		if(skill.getId() == _sealSkillid)
		{
			if(targetsTheFlag(player))
				return true;
			else
				return false;
		}
