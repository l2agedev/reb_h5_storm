...
public class OldStats
{
	...
	private static final Logger _log = LoggerFactory.getLogger(OldStats.class);
	...
	private void createHtmls()
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				if (count % 10 == 0)
				{
					...
					htmls.put(count / 10, new ShowBoardData(sb.toString(), "101", null)); //TODO: Infern0 null ?!
					sb.clear();
				}
			}
			...
			{
				sb.append("</table></body></html>");
				htmls.put((count / 10 + 1), new ShowBoardData(sb.toString(), "101", null)); //TODO: Infern0 null ?!
				sb.clear();
			}
		...
		{
			_log.error("NexusEngine: createHtmls Exception");
			e.printStackTrace();
		}
		finally
		{
	...
	private void loadSQL()
	{
		...
		{
			_log.error("NexusEngine: loadSQL Exception");
		}
		finally
		{
			DbUtils.closeQuietly(con, statement, rset);
	...
	public void showPlayerStats(int playerId, PlayerEventInfo player)
	{
		...
		sb = new ShowBoardData(tb.toString(), "101", player.getOwner());
		sb.sendToPlayer(player);
