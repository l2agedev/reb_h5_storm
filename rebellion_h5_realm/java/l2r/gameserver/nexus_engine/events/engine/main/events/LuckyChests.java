...
import l2r.gameserver.nexus_engine.events.engine.main.events.LuckyChests.LuckyChestsPlayerData.SlotInfo;
...
public class LuckyChests extends Deathmatch
{
	...
	{
		private Map<ChestType, FastTable<NpcData>> _chests;
		
		protected LuckyChestsData(int instance)
		{
			super(instance);
			
			_chests = new FastMap<ChestType, FastTable<NpcData>>();
			for(ChestType ch : ChestType.values())
				_chests.put(ch, new FastTable<NpcData>());
		}
	}
	...
	public void loadConfigs()
	{
		...
		final String jokerDefault = "hahahaha,hahahahahaha,hehehe,ha...ha...ha...,hihihi,ahahaha,ehehehe,ekieki,kehkeh,muhahaha,nihaha,puhahaha,uhahahaha,zuhahaha,eheheh,moahaha,kahkahkah,teeheehee!,LOL!";
	...
	public void initEvent()
	{
		...
		_allowSummons = false; // not allowed summons
		_allowPets = false; // not allowed pets
		...
		if(!getPlayerData(player).hasWeapon(itemType))
		{
			...
			if(equip)
			{
				...
				if(itemType != LuckyItem.SHIELD)
				{
					...
					if (wpn != null)
						player.unEquipItemInBodySlot(CallBack.getInstance().getValues().SLOT_R_HAND());
				}

				wpn = player.getPaperdollItem(CallBack.getInstance().getValues().PAPERDOLL_LHAND());
				if (wpn != null)
					player.unEquipItemInBodySlot(CallBack.getInstance().getValues().SLOT_L_HAND());
	...
	private void removeWeapon(PlayerEventInfo player, LuckyItem type)
	{
		if(getPlayerData(player).hasWeapon(type))
		{
			...
			{
				player.unEquipItemInBodySlot(wpn.getBodyPart());
				player.inventoryUpdate(wpn);
			}
	...
	public void onItemUse(PlayerEventInfo player, ItemData item)
	{
		...
		checkEventEnd(player.getReflectionId()); //TODO
	}
	...
	private EffectResult effect(final PlayerEventInfo player, final NpcData npc, EffectType type, Object param, Object param2)
	{
		switch(type)
		{
			...
			case SpawnBonusChests:
			{
				...
				//.. and spawn chests
				final int instanceId = player.getReflectionId();
			...
			case Laugh:
			{
				...
				if(!npc.isDead())
				{
					String s = _jokerChestTexts[CallBack.getInstance().getOut().random(1, (_jokerChestTexts.length - 1))]; // tempfix.
					...
					try
					{
						...
						for(NpcData ch : getEventData(player.getReflectionId())._chests.get(chestType))
						{
							if(ch != null && ch.getObjectId() == npc.getObjectId())
							{
								synchronized(getEventData(player.getReflectionId())._chests)
								{
									getEventData(player.getReflectionId())._chests.get(chestType).remove(ch);
								}
								
								checkEventEnd(player.getReflectionId());
					...
					if(_enableFearFireworking)
					{
						for(int i = 1; i <= 4; i++)
						{
							CallBack.getInstance().getOut().scheduleGeneral(new Runnable()
							{
								...
								public void run()
								{
									if(player.isOnline() && getMatch(player.getReflectionId()).isActive() && !player.isDead() && player.isAfraid())
									{
			...
			case TransformToBunny:
			{
				...
				if(transformId > 0 && getPlayerData(player).getTransformation() == null)
				{
					...
					sysMsgToAll(player.getReflectionId(), LanguageEngine.getMsg("chests_player_transform_bunny_announce", player.getPlayersName()));
			...
			case TransformToFrog:
			{
				...
				if(transformId > 0 && getPlayerData(player).getTransformation() == null)
				{
					...
					sysMsgToAll(player.getReflectionId(), LanguageEngine.getMsg("chests_player_transform_frog_announce", player.getPlayersName()));
			...
			case TransformToPig:
			{
				...
				if(transformId > 0 && getPlayerData(player).getTransformation() == null)
				{
					...
					sysMsgToAll(player.getReflectionId(), LanguageEngine.getMsg("chests_player_transform_pig_announce", player.getPlayersName()));
			...
			case TransformToYeti:
			{
				...
				if(transformId > 0 && getPlayerData(player).getTransformation() == null)
				{
					...
					announce(player.getReflectionId(), LanguageEngine.getMsg("chests_player_transform_yeti_announce", player.getPlayersName()));
	...
	protected void checkEventEnd(int instance)
	{
		...
		for(FastTable<NpcData> e : getEventData(instance)._chests.values())
		{
			if(!e.isEmpty())
			{
				empty = false;
	...
	private void updateBombShield(PlayerEventInfo player)
	{
		...
		{
			player.stopAbnormalEffect(AbnormalEffect.E_AFRO_1);
	...
	protected void unspawnChests(int instance)
	{
		...
		for(Entry<ChestType, FastTable<NpcData>> e : getEventData(instance)._chests.entrySet())
		{
	...
	{
		if(getMatch(killer.getReflectionId()).isActive())
		{
			...
			for(NpcData ch : getEventData(killer.getReflectionId())._chests.get(type))
			{
				if(ch != null && ch.getObjectId() == npc.getObjectId())
				{
					chestAlive = true;
			...
			for(NpcData ch : getEventData(killer.getReflectionId())._chests.get(type))
			{
				if(ch != null && ch.getObjectId() == npc.getObjectId())
				{
					synchronized(getEventData(killer.getReflectionId())._chests)
					{
						getEventData(killer.getReflectionId())._chests.get(type).remove(ch);
					}
					
					checkEventEnd(killer.getReflectionId());
	...
	public void onKill(PlayerEventInfo player, CharacterData target)
	{
		...
		// Achievements
		if (Config.ENABLE_PLAYER_COUNTERS && target != null && target.isNpc())
			player.getOwner().getCounters().addPoint("_Chests_Opened");
	...
	{
		List<LuckyItem> _weapons = new FastTable<LuckyItem>();
