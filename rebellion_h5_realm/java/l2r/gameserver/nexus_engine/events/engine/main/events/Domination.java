...
public class Domination extends AbstractMainEvent
{
	...
	protected void setZoneEffects(int teamId, NpcData zoneNpc)
	{
		if(getBoolean("allowZoneNpcEffects") && _teamsCount == 2)
		{
			if(teamId == 1)
			{
				zoneNpc.stopAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
				zoneNpc.startAbnormalEffect(AbnormalEffect.IMPRISIONING_1); // imprissing 1 effect
			}
			else if(teamId == 2)
			{
				zoneNpc.stopAbnormalEffect(AbnormalEffect.IMPRISIONING_1); // imprissing 1 effect
				zoneNpc.startAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
			}
			else // stop effects
			{
				zoneNpc.stopAbnormalEffect(AbnormalEffect.REDCIRCLE); // red circle effect
	...
	protected void clockTick() throws Exception 
	{
		...
		Map<Integer, List<PlayerEventInfo>> players = new FastMap<Integer, List<PlayerEventInfo>>();
						...
						if(getEventData(instanceId)._holdingTeam != 0 && getBoolean("allowPlayerEffects") && _teamsCount == 2)
						{
							for (PlayerEventInfo player : _teams.get(instanceId).get(getEventData(instanceId)._holdingTeam).getPlayers())
							{
								...
								if (Config.ENABLE_PLAYER_COUNTERS)
									player.getOwner().getCounters().addPoint("_Domination_Points");
							}
						}
					...
					if(getBoolean("allowPlayerEffects") && _teamsCount == 2)
					{
						for(PlayerEventInfo player : _teams.get(instanceId).get(team).getPlayers())
						{
							if(player.getDistanceSq(zoneX, zoneY, zoneZ) <= _zoneRadius && player.isVisible() && !player.isDead())
							{
								...
								//	player.getOwner().getCounters().addPoint("_domination_scored_points");
							}
							else
								player.stopAbnormalEffect(player.getTeamId() == 1 ? AbnormalEffect.IMPRISIONING_1 : AbnormalEffect.REDCIRCLE);
						}
