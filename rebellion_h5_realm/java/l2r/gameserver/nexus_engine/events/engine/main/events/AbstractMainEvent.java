...
{
	private static final Logger _log = LoggerFactory.getLogger(AbstractMainEvent.class);
	...
	private FastMap<MainEventInstanceType, FastTable<PlayerEventInfo>> _tempPlayers;
	...
	@SuppressWarnings("unused")
	private boolean _newbieOnlyDynamic = false;

	public double getMaxGearScore()
	{
		return _maxGearScore;
	}

	protected double _maxGearScore;
	protected boolean _newbieItemsOnly;
	...
	public AbstractMainEvent(EventType type, MainEventManager manager)
	{
		...
		_rewardedInstances = new FastTable<Integer>();
	...
	public void loadConfigs()
	{
		...
		{
			ConfigModel divideMethod = new ConfigModel("divideToTeamsMethod", "LevelOnly", "The method used to divide the players into the teams on start of the event. All following methods try to put similar count of healers to all teams.<br1>" +
					"<font color=LEVEL>LevelOnly</font> sorts players by their level and then divides them into the teams (eg. Player1 (lvl85) to teamA, Player2(level84) to teamB, Player3(lvl81) to teamA, Player4(lvl75) to teamB, Player5(lvl70) to teamA,...)<br1>" +
					"<font color=LEVEL>PvPsAndLevel</font>: in addition to sorting by level, this method's main sorting factor are player's PvP kills. The rest of dividing procedure is same as for LevelsOnly. Useful for PvP servers, where level doesn't matter much.<br1>",
					InputType.Enum);
		...
		addConfig(new ConfigModel("minHealAmountForReward", "100000", "Put the minimum amount of HP recovery player should do to recive a reward. Only for Priest class."));
		...
		addConfig(new ConfigModel("dualboxCheck", "true", "You can enable/disable the registration dualbox check here. (If you disable this it will auto scan players HWID.)", InputType.Boolean));
		...
		addConfig(new ConfigModel("dualboxCheckHwidSupport", "true", "You can enable/disable the registration check for HWID dualbox check here. (If this is disabled it will use IP check for registration.)", InputType.Boolean));
		addConfig(new ConfigModel("maxHwidAllowed", "1", "If the 'dualboxCheckHwidSupport' config is enabled, you can specify here how many players with the same HWID are allowed to be in the event."));
		...
		addConfig(new ConfigModel("firstRegisteredRewardType", "WinnersOnly", "Select here who will be rewarded with the 'FirstRegisteredReward' reward in the end of the event.", InputType.Enum).addEnumOptions(new String[]{ "WinnersOnly", "All" }));
		...
		addConfig(new ConfigModel("maxGearScore", "-1", "Set the max gear score for this event. Players with higher value won't be able to join. See more info in Wondrous Cubic - Stats. Gear score = player rating index.", InputType.TextEdit));
		addConfig(new ConfigModel("newbieItemsOnly", "false", "Set true to make this like Orc Village - that items above S80-grade will not be allowed.", InputType.Boolean));
		...
		addInstanceTypeConfig(new ConfigModel("maxPlayers", "-1", "Max players ammount available for this instance. Put -1 to make it infinity."));
	}
	...
	protected abstract class AbstractEventInstance implements Runnable
	{
		...
		public AbstractEventInstance(InstanceData instance)
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: created abstracteventinstance for instanceId " + instance.getId());
		}
		
		public abstract boolean isActive();
		public void setCanBeAborted() { _canBeAborted = true; }
		...
		public void setCanRewardIfAborted() { _canRewardIfAborted = true; }
		...
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: abstractmaininstance: scheduling next task in " + time);

			if(_clock._task != null)
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: abstractmaininstane: _clock_task is not null");

				_clock._task.cancel(false);
				_clock._task = null;
			}
			else /**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: abstractmaininstance: _clock_task is NULL!");
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: next task scheduled.");
		...
		public void abort()
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: abstractmaininstance: aborting...");

			if(_task != null)
			{
				_task.cancel(false);
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: abstractmaininsance _task is not null");
			}
			else /**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: abstractmaininstance _task is NULL!");
	...
	protected abstract class AbstractEventData
	{
		...
		{ 
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: abstracteventdata created data for instanceId = " + instance);
		}
	}
	...
	public void setNewbieOnly(boolean b)
	{
		_newbieOnlyDynamic = b;
	}

	public boolean isNewbieOnly()
	{
		//return _newbieOnlyDynamic || _newbieItemsOnly;
		// disable it for now.
		return false;
	}
	...
	{
		_tempPlayers = new FastMap<MainEventInstanceType, FastTable<PlayerEventInfo>>();
		_firstRegistered = new FastTable<PlayerEventInfo>();
		...
		_maxGearScore = getDouble("maxGearScore");
		_newbieItemsOnly = getBoolean("newbieItemsOnly");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: startRegistration() done");
	}
	...
	public void initMap()
	{
		try
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: starting initMap()");
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: init map - " + map.getMapName());
			
			for(InstanceData instance : _instances)
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: initmap iterating instance " + instance.getId());
				
				_fences.put(instance.getId(), new FastTable<FenceData>());
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: initmap iterating instance spawned fences");
				
				NpcData npc;
				_npcs.put(instance.getId(), new FastTable<NpcData>());
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: initmap iterating instance spawned npcs");
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: initmap iterating instance spawned map guards");
			}
			
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: initmap finished");
			
		}
		catch (NullPointerException e)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error on initMap() " + NexusLoader.getTraceString(e.getStackTrace()));
	...
	public void cleanMap(int instanceId)
	{
		try
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: starting cleanmap(), instanceId " + instanceId);

			if(_instances != null)
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: instances are not null");

				for(InstanceData instance : _instances)
				{
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: iterating instance " + instance.getId());
					...
					if(instanceId == 0 || instance.getId() == instanceId)
					{
						...
						/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: instance + " + instance.getId() + ", fences deleted");
						...
						/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: instance + " + instance.getId() + ", npcs deleted");
						...
						/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: instance + " + instance.getId() + " cleaned.");
					}
				}
			}
			...
			if(instanceId == 0)
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: set npcs and fences to null (instanceId = 0)");

				_npcs = null;
				_fences = null;
			}
			else /**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: cannot set npcs and fences to null yet, instanceId != 0");
		}
		catch (NullPointerException e)
		{
			e.printStackTrace();
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: initEvent starting");
		
		if(_rewardTypes == null)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: event " + getEventName() + " has not set up any _rewardTypes");
		...
		_firstBlood = false;
		 ...
		 _spectators = new FastTable<PlayerEventInfo>();
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: spawns sorted, instances cleaned");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: scorebar - " + _allowScoreBar + ", scheme buffer = " + _allowSchemeBuffer);
		
		// load not alloved items
		if(!getString("notAllowedItems").equals(""))
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: loading not allowed items");
			...
			try
			{
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: not allowed items = " + Arrays.toString(notAllovedItems));
			}
			catch (Exception e)
			{
				NexusLoader.debug("Error while loading config 'notAllowedItems' for event " + getEventName() + " - " + e.toString(), Level.SEVERE);
				
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while loading not allowed items " + NexusLoader.getTraceString(e.getStackTrace()));
			}
		}
		else /**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: no not allowed items specified!");
		
		// load not alloved skills
		if(!getString("notAllowedSkills").equals(""))
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: loading not allowed skills!");
			...
			try
			{
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: not allowed skills = " + Arrays.toString(notAllovedSkillls));
			}
			catch (Exception e)
			{
				NexusLoader.debug("Error while loading config 'notAllowedSkills' for event " + getEventName() + " - " + e.toString(), Level.SEVERE);
				
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while loading not allowed skills " + NexusLoader.getTraceString(e.getStackTrace()));
			}
		}
		else /**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: no not allowed skills specified!");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: initEvent finished for AbstractMainEvent()");
		dumpConfigs();
	}
		...
		if(s != null && s.length() > 0)
		{
			try
			{
				...
				try
				{
					...
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: set offensive skills = " + Arrays.toString(setOffensiveSkills));
				}
				catch (Exception e)
				{
			...
			{
				NexusLoader.debug("Error while loading GLOBAL config 'setOffensiveSkills' in event " + getEventName() + " - " + e.toString(), Level.SEVERE);
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while loading 'setOffensiveSkills' GLOBAL config " + NexusLoader.getTraceString(e.getStackTrace()));
			}
		}
		...
		if(s != null && s.length() > 0)
		{
			try
			{
				...
				try
				{
					...
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: set not offensive skills = " + Arrays.toString(setNotOffensiveSkills));
				}
				catch (Exception e)
				{
			...
			{
				NexusLoader.debug("Error while loading GLOBAL config 'setNotOffensiveSkills' in event " + getEventName() + " - " + e.toString(), Level.SEVERE);
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while loading 'setNotOffensiveSkills' GLOBAL config " + NexusLoader.getTraceString(e.getStackTrace()));
			}
		}
		...
		if(s != null && s.length() > 0)
		{
			try
			{
				...
				try
				{
					...
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: set neutral skills = " + Arrays.toString(setNeutralSkills));
				}
				catch (Exception e)
				{
			...
			{
				NexusLoader.debug("Error while loading GLOBAL config 'setNeutralSkills' in event " + getEventName() + " - " + e.toString(), Level.SEVERE);
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while loading 'setNeutralSkills' GLOBAL config " + NexusLoader.getTraceString(e.getStackTrace()));
			}
		}
	}
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: dumping configs START =====================");
		for(Entry<String, ConfigModel> e : _configs.entrySet())
		{
			/**/ if(NexusLoader.detailedDebug) print(e.getKey() + " - " + e.getValue().getValue());
		}
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: dumping configs END ====================");
	}
	...
	protected void createTeams(int count, int instanceId)
	{
		try
		{
			...
			switch(count)
			{
				...
				default:
				{
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: the teams count is too high on event " + getEventName());
	...
	protected void createNewTeam(int instanceId, int id, String name, String fullName)
	{
		...
		/**/ if(NexusLoader.detailedDebug) print("... AbstractMainEvent: created new team for instanceId " + instanceId + ", id " + id + ", fullname " + fullName);
	}
	...
	protected void createNewTeam(int instanceId, int id, String name)
	{
		...
		/**/ if(NexusLoader.detailedDebug) print("... AbstractMainEvent: created new team for instanceId " + instanceId + ", id " + id);
	}
	...
	public boolean canRegister(PlayerEventInfo player)
	{
		...
		if(_maxGearScore > 0 && gearScore > _maxGearScore)
		{
			player.screenMessage("Your Player rating is too high.", "Event", false);
			player.sendMessage("The Player rating index for this event is " + _maxGearScore + ", yours is " + gearScore + ". Check out Wondrous Cubic -> Stats (click on Player rating index) for more info.");
			return false;
		}
		...
		{
			/**/ if(NexusLoader.detailedDebug) print("... registerPlayer() in AbstractMainEvent (canRegister()) for " + player.getPlayersName() + ", the event is full already! " + maxPlayers + "/" + _manager.getPlayersCount());
		...
		synchronized(_tempPlayers)
		{
			for(MainEventInstanceType instance : _types.values())
			{
				...
				{
					/**/ if(NexusLoader.detailedDebug) print("... registerPlayer() in AbstractMainEvent (canRegister()) for " + player.getPlayersName() + " player CAN join instancetype " + instance.getId());
					
					if(!_tempPlayers.containsKey(instance))
						_tempPlayers.put(instance, new FastTable<PlayerEventInfo>());
					else
					{
						int max = instance.getConfigInt("maxPlayers");
						if(max > -1 && _tempPlayers.get(instance).size() >= max)
						{
							/**/ if(NexusLoader.detailedDebug) print("... registerPlayer() in AbstractMainEvent (canRegister()) for " + player.getPlayersName() + " instance type " + instance.getId() + " is full already (max " + max + ")");
							continue;
						}
					}
					...
					/**/ if(NexusLoader.detailedDebug) print("... registerPlayer() in AbstractMainEvent (canRegister()) for " + player.getPlayersName() + " registered to instance type " + instance.getId());
					...
					/**/ if(NexusLoader.detailedDebug) print("... registerPlayer() in AbstractMainEvent (canRegister()) for " + player.getPlayersName() + " player CANNOT join instancetype " + instance.getId());
			}
		}
	...
	protected void checkAndRemoveItems(PlayerEventInfo player)
	{
		if(!player.isOnline())
			return;
		...
		/*if(isNewbieOnly())
		{
			if(player.getOwner().isCursedWeaponEquipped())
				CursedWeaponsManager.getInstance().getCursedWeapon(player.getOwner().getCursedWeaponEquippedId()).endOfLife();
			...
			for(L2ItemInstance item : player.getOwner().getInventory().getItems())
			{
				if(item != null && item.isEquipped())
				{
					...
					{
						if(item.isAugmented())
							item.getAugmentation().removeBonus(player.getOwner());

						L2ItemInstance[] unequiped = player.getOwner().getInventory().unEquipItemInBodySlotAndRecord(player.getOwner().getInventory().getSlotFromItem(item));
						InventoryUpdate iu = new InventoryUpdate();

						for(L2ItemInstance element : unequiped)
							iu.addModifiedItem(element);

						player.getOwner().sendPacket(iu);
						update = true;
					}
				}
			}

			if(player.getOwner().getClan() != null)
			{
				for(L2Skill skill : player.getOwner().getClan().getAllSkills())
				{
					if(skill != null)
						player.getOwner().removeSkill(skill, false);
				}
			}

			player.getOwner().removeSkill(3611);
			player.getOwner().removeSkill(3612);
			player.getOwner().removeSkill(3613);
			player.getOwner().removeSkill(3614);
			player.getOwner().removeSkill(3615);
			player.getOwner().removeSkill(3616);
	...
	public void playerUnregistered(PlayerEventInfo player)
	{
		synchronized(_tempPlayers)
		{
			for(Entry<MainEventInstanceType, FastTable<PlayerEventInfo>> e : _tempPlayers.entrySet())
			{
				for(PlayerEventInfo pi : e.getValue())
				{
					if(pi.getPlayersId() == player.getPlayersId())
					{
						...
						/**/ if(NexusLoader.detailedDebug) print("... playerUnregistered player " + player.getPlayersName() + " removed from _tempPlayers");
						return;
					}
				}
			}
		...
		/**/ if(NexusLoader.detailedDebug) print("... palyerUnregistered couldn't remove player " + player.getPlayersName() + " from _tempPlayers");
	}
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: calling reorganizeInstance");

		// join same strenght instances where possible to reach as close to maxPlayers as possible
		// -> reason: some players might have unregistered, or same-strenght instances have different criteria
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: instances DONE reorganized!!");
		dumpTempPlayers();
	}
	
	private void dumpTempPlayers()
	{
		/**/ if(NexusLoader.detailedDebug) print("***** AbstractMainEvent: STARTING tempPlayers dump"); 
		
		try
		{
			for(Entry<MainEventInstanceType, FastTable<PlayerEventInfo>> e : _tempPlayers.entrySet())
			{
				/**/ if(NexusLoader.detailedDebug) print("... ***** AbstractMainEvent: instance " + e.getKey().getName() + " (" + e.getKey().getId() + ") has " + e.getValue().size() + " players");
			}
		}
		catch (Exception e)
		{
			/**/ if(NexusLoader.detailedDebug) print("error while dumping temp players - " + NexusLoader.getTraceString(e.getStackTrace()));
		}
		
		/**/ if(NexusLoader.detailedDebug) print("***** AbstractMainEvent: ENDED tempPlayers dump"); 
	}
	...
	protected void reorganize(List<MainEventInstanceType> instances)
	{
		...
		if(!instances.isEmpty())
		{
			...
			_log.info("NexusEngine: *** Done, instance " + (inst != null ? inst.getName() : null) + " has " + _tempPlayers.get(inst).size() + " players.");
			
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: reorganize() - instance " + inst.getName() + " has " + _tempPlayers.get(inst).size() + " players");
		}
	}
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: starting dividePlayers");
		
		reorganizeInstances();
		
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: starting notEnoughtPlayersInstance operations");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: notEnoughtPlayersInstances size = " + notEnoughtPlayersInstances.size());
		...
		for(MainEventInstanceType currentInstance : notEnoughtPlayersInstances)
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: iterating through notEnoughtInstances: " + currentInstance.getId() + " [" + currentInstance.getStrenghtRate() + "] - playersCount (" + playersCount + "), strenght (" + strenght + ")");
			...
			for(MainEventInstanceType possibleInstance : notEnoughtPlayersInstances)
			{
				...
				if(possibleInstance.getStrenghtRate() == strenght)
				{
					if(_tempPlayers.get(possibleInstance).size() + playersCount >= possibleInstance.getConfigInt("minPlayers"))
						/**/ if(NexusLoader.detailedDebug) print("How could have this happened? (" + currentInstance.getName() + ", " + possibleInstance.getName() + ")");
				...
				if(joinStrongerInstIfNeeded && possibleInstance.getStrenghtRate() > strenght)
				{
					...
					{
						/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// possible instance " + possibleInstance.getName() + "[" + possibleInstance.getStrenghtRate() + "] - playersCount (" + _tempPlayers.get(possibleInstance).size() + "), strenght (" + possibleInstance.getStrenghtRate() + ")");
						...
						if(sumPlayers >= possibleInstance.getConfigInt("minPlayers"))
						{
							...
							/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /*/*/ moving " + toMove + " players from " + currentInstance.getName() + " to " + possibleInstance.getName());
							
							movePlayers(possibleInstance, currentInstance, toMove);
							
							/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /*/*/ size of " + possibleInstance.getName()  + " is now " + _tempPlayers.get(possibleInstance).size());
							
							if(_tempPlayers.get(possibleInstance).size() >= possibleInstance.getConfigInt("minPlayers"))
							{
								/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /*/*/ instance " + possibleInstance.getName() + " removed from notEnoughtPlayersInstances.");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: reorganizing notEnoughtPlayers first part done");
		dumpTempPlayers();
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: reorganizing notEnoughtPlayers second part done");
		dumpTempPlayers();
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: fixed " + c + " notEnoughtPlayers instances");
		
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: starting tempPlayers reorganizations");
		...
		for(Entry<MainEventInstanceType, FastTable<PlayerEventInfo>> e : _tempPlayers.entrySet())
		{
			...
			while(playersCount > 0)
			{
				...
				for(Entry<MainEventInstanceType, FastTable<PlayerEventInfo>> i : _tempPlayers.entrySet())
				{
					if(playersCount <= 0) 
						break;
		...
		/**/ if(NexusLoader.detailedDebug) print("* AbstractMainEvent: instances organizing FINISHED:");
		dumpTempPlayers();
		...
		for(MainEventInstanceType inst : notEnoughtPlayersInstances)
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ... Not enought players for instance " + inst.getName() + " (" + _tempPlayers.get(inst).size() + "), instance removed; " + i + " players unregistered.");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: dividing players into teams - instances count = " + _tempPlayers.size());

		for(Entry<MainEventInstanceType, FastTable<PlayerEventInfo>> e : _tempPlayers.entrySet())
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: STARTING event for instance: " + e.getKey().getName());
			
			// create InstanceData for this InstanceType
			InstanceData instance = CallBack.getInstance().getOut().createInstance(e.getKey().getName(), _manager.getRunTime() * 1000 + 60000, 0);
			_instances[availableInstances] = instance;
			e.getKey().setInstance(instance);
			
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ... created InstanceData, duration: " + (_manager.getRunTime() * 1000 + 60000));
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ... teamscount = " + teamsCount + "; DIVIND to teams:");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: available instances = " + availableInstances);
		
		if(availableInstances == 0)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: evnet COULD NOT START due to lack of players in instances");
		...
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ... dividePlayers allowed event to start!");
	...
	protected void dividePlayersToTeams(int instanceId, FastTable<PlayerEventInfo> players, int teamsCount)
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// dividingplayers to teams for INSTANCE " + instanceId);
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// players count = " + players.size());
		
		if(!getEventType().isFFAEvent() && teamsCount > 1)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// team based event");

			int teamId;
			
			String type = getString("divideToTeamsMethod");
			
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// using method: " + type);
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// players sorted:");
			...
			for(Entry<ClassType, FastTable<PlayerEventInfo>> te : sortedPlayers.entrySet())
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// ... " + te.getKey().toString() + " - " + te.getValue().size() + " players");
			
			// to teams: first divide healers to teams
			if(getBoolean("balanceHealersInTeams"))
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// balancing healers in teams");

				teamId = 0;
				for (PlayerEventInfo healers : sortedPlayers.get(ClassType.Priest))
				{
					if (healers == null)
						continue;
					
					teamId++;
					healers.onEventStart(this);
					_teams.get(instanceId).get(teamId).addPlayer(healers, true);
					sortedPlayers.remove(healers);
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// healers balanced into teams:");
			for(EventTeam team : _teams.get(instanceId).values())
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// team " + team.getTeamName() + " has " + team.getPlayers().size() + " healers");
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// players divided:");
			for(EventTeam team : _teams.get(instanceId).values())
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// team " + team.getTeamName() + " has " + team.getPlayers().size() + " PLAYERS");
		}
		else // FFA event (Deathmatch, ..)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /// FFA event");
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: CREATING PARTIES... ");
		...
		{
			/**/ if(NexusLoader.detailedDebug) print("* AbstractMainEvent: PROCESSING INSTANCE " + teams.getKey() + " (creating parties)");
			
			for(EventTeam team : teams.getValue().values())
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: / parties: processing team " + team.getTeamName());
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ////// total count of players in the team " + totalCount + "; PARTIES COUNT " + partiesCount + "; healers count " + healersCount);
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ////// healersToGive to each party: " + healersToGive);
				...
				for(int i = 0; i < partiesCount; i++)
				{
					...
					if(healersCount > 0)
					{
						...
						{
							pi = players.get(ClassType.Priest).get(index++);
	...
	private void dumpParty(EventTeam team, FastTable<PlayerEventInfo> players)
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ////// START of dump of party for team " + team.getTeamName());
		
		for(PlayerEventInfo pl : players)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: /*/*/*/*/*/*/ player " + pl.getPlayersName() + " is of class id " + pl.getClassType().toString()); 
		}
		
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ////// END of dump of party for team " + team.getTeamName());
	}
	
	protected void partyPlayers(FastTable<PlayerEventInfo> players)
	{
		try
		{
			if(players.size() > 1)
			{
				...
				if(leader != null)
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: reallowing inviting to the party back to the leader (" + leader.getPlayersName() + ").");
				else
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: NOT reallowing inviting to the party back to the leader because he is null!");
		...
		catch (Exception e)
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: createParties error (and parties will be deleted): " + NexusLoader.getTraceString(e.getStackTrace()));
			...
			{
				e2.printStackTrace();
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while removing parties (cause of another error): " + NexusLoader.getTraceString(e2.getStackTrace()));
			}
			
			debug("Parties deleted.");
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: parties deleted.");
		}
	}
		...
		public void run()
		{
			try
			{
				...
				if(party.exists())
				{
					...
					if(party.size() >= getInt("maxPartySize"))
						return;
			...
			catch (NullPointerException e)
			{
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while adding players to the party: " + NexusLoader.getTraceString(e.getStackTrace()));
	...
	protected void teleportPlayers(int instanceId, SpawnType type, boolean ffa)
	{
		...
		for (PlayerEventInfo player : getPlayers(instanceId))
		{
			...
			player.teleport(loc, 0, instanceId);
	...
	protected boolean checkPlayers(int instanceId)
	{
		if(!checkIfEventCanContinue(instanceId, null)) // disconnected during the teleportation phase
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: check players: FALSE (NOT ENOUGHT players to start the event)");
			return false;
		}
		else
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: check players: OK (enought players to start the event)");
			return true;
		}
	}
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: SETUPING TITLES"); 
	...
	protected void clearMapHistory(int teamId, SpawnType type)
	{
		...
		if(map != null)
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: map history clean done");
		}
		else /**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: couldn't clean map, map is NULL!");
	}
	
	protected void forceSitAll(int instanceId)
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: FORCE SIT ALL");
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: FORCE STAND UP");
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("? AbstractMainEvent: sysMsgToAll - " + text);
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("? AbstractMainEvent: sysMsgToAll to instance " + instance + "; text= " + text);
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: STARTING WAWE SPAWN SYSTEM");
	...
	public void addSpectator(PlayerEventInfo gm, int instanceId)
	{
		...
		if(_spectators != null)
		{
			...
			else
			{
				...
				gm.teleToLocation(selected.getLoc().getX(), selected.getLoc().getY(), selected.getLoc().getZ());
	...
	public void removeSpectator(PlayerEventInfo gm)
	{
		...
		{
			gm.setInstanceId(0);
			gm.teleToLocation(gm.getOrigLoc().getX(), gm.getOrigLoc().getY(), gm.getOrigLoc().getZ());
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent:  =====================");
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: CALLED CLEAR PLAYERS for instanceId " + instanceId + ", unregister = " + unregister);
		...
		if(_spectators != null)
		{
			...
			{
				if(instanceId == 0 || spectator.getReflectionId() == instanceId)
				{
					spectator.setInstanceId(0);
					spectator.teleToLocation(spectator.getOrigLoc().getX(), spectator.getOrigLoc().getY(), spectator.getOrigLoc().getZ());
		...
		if(unregister)
		{
			switch(_manager.getState())
			{
				case REGISTERING:
				{
					...
					{
						player.setIsRegisteredToMainEvent(false, null, (getMaxGearScore() > 0));
				...
				case TELE_BACK:
				{
					...
					for(PlayerEventInfo player : _manager.getPlayers())
					{
						if(instanceId == 0 || player.getReflectionId() == instanceId)
						{
							player.setIsRegisteredToMainEvent(false, null, (getMaxGearScore() > 0));
							CallBack.getInstance().getPlayerBase().eventEnd(player);
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: unregistered " + unregistered + " players");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: Event " + getEventName() + " finished clearPlayers() for instance ID " + instanceId);
		NexusLoader.debug("Event " + getEventName() + " finished clearPlayers() for instance ID " + instanceId);
		
		if(instanceId == 0)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: checking if all unregistered...");
			
			Collection<PlayerEventInfo> playersLeft = getPlayers(0);
			
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: playersLeft size = " + playersLeft.size());
			
			if(playersLeft.size() > 0 || _manager.getPlayers().size() > 0)
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: the event hasn't cleaned itself properly, perfoming additional cleanings...");
				...
				{
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: additional cleanings finished");
					NexusLoader.debug("Additional cleaning finished. All players unregistered successfully now.");
				}
			}
		}

		_newbieOnlyDynamic = false;
	...
	public double getDouble(String propName)
	{
		if(_configs.containsKey(propName))
		{
			return _configs.get(propName).getValueDouble();
		}
	...
	public void announce(int instance, String msg)
	{
		...
		if(_spectators != null)
		{
			for(PlayerEventInfo spectator : _spectators)
			{
				if(spectator.isOnline() && spectator.getReflectionId() == instance)
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("? AbstractMainEvent: announcing to instance " + instance + " team " + team + " msg: " + msg);
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("? AbstractMainEvent: announcing to all teams but " + excludedTeam + ", instance " + instance + " msg " + msg);
	...
	{
		/**/ if(NexusLoader.detailedDebug) print("? AbstractMainEvent: screenannounce to instance " + instance + " msg: " + msg);
	...
	protected class WaweRespawnScheduler implements Runnable
	{
		...
		public void stop()
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: stopping wawe spawn scheduler");
		...
		public void run()
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: running wawe spawn scheduler...");
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ...wawe scheduler respawned " + count + " players");
	...
	private class ReviveTask implements Runnable
	{
		...
		private ReviveTask(PlayerEventInfo p, int time)
		{
			player = p;
			this.instance = player.getReflectionId();
		...
		private ReviveTask(PlayerEventInfo p, int time, int instance)
		{
			player = p;
	...
	public class Clock implements Runnable
	{
		...
		public void run()
		{
			...
			if(_allowScoreBar)
			{
				if(_instances != null)
				{
					for(InstanceData instance : _instances)
					{
						...
						{
							e.printStackTrace();
							/**/ if(NexusLoader.detailedDebug) print("ERROR on CLOCK.getScorebar: " + NexusLoader.getTraceString(e.getStackTrace()));
							/**/ if(NexusLoader.detailedDebug) print("Event aborted");
						...
						if(scorebarText != null)
						{
							...
							if(_spectators != null)
							{
								...
								{
									if(spec.getReflectionId() == instance.getId())
			...
			if (time <= 0)
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: Clock.time is " + time + ", scheduling next event task");
		...
		{
			if(debug)
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: setting value of Clock.time to " + t); 
			time = t;
		}
		
		public void startClock(int mt)
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: starting Clock and setting Clock.time to " + mt);
	...
	protected void setInstanceNotReceiveRewards(int instanceId)
	{
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: instance of ID " + instanceId + " has been marked as NOTREWARDED");
	}
	
	protected void rewardFirstRegisteredFFA(List<PlayerEventInfo> list)
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: rewarding first registered (ffa)");
		...
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: " + count + " players were given FirstRegistered reward");
	}
	
	protected void rewardFirstRegistered(List<EventTeam> list)
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: rewarding first registered (teams)");
	...
	protected void rewardAllPlayersFromTeam(int instanceId, int minScore, int minKills, int teamId)
	{
		try 
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: CALLED REWARD ALL PLAYERS  for instance " + instanceId + ", min score " + minScore + ", min kills " + minKills);
			
			boolean firstXRewardWinners = "WinnersOnly".equals(firstRegisteredRewardType);
			
			int playersCount;
			FastTable<PlayerEventInfo> sorted;
			...
			for(InstanceData instance : _instances)
			{
				...
				sorted = new FastTable<PlayerEventInfo>();
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: before giving reward");
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: rewards given");
	...
	protected void rewardAllTeams(int instanceId, int minScore, int minKills)
	{
		try 
		{
			...
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: CALLED REWARD ALL TEAMS  for instance " + instanceId + ", min score " + minScore + ", min kills " + minKills);
			
			boolean firstXRewardWinners = "WinnersOnly".equals(firstRegisteredRewardType);
			
			int teamsCount;
			FastTable<EventTeam> sorted;
			...
			for(InstanceData instance : _instances)
			{
				...
				// sort teams by their score
				sorted = new FastTable<EventTeam>();
	...
	protected NpcData spawnNPC(int x, int y, int z, int npcId, int instance, String name, String title)
	{
		/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: spawning npc " + x + ", " + y + ", " + z + ", npc id " + npcId + ", instance " + instance + ", name " + name + ", title " + title); 
		...
		try
		{
			final NpcData npc = template.doSpawn(x, y, z, 1, instance);
			
			if(npc != null)
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: npc spawned succesfully.");
			else
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: npc null after spawning (template exists = " + template.exists() + ").");
		...
		{
			e.printStackTrace();
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: error while spawning npc - " + NexusLoader.getTraceString(e.getStackTrace()));
			return null;
		}
	}
				...
				for(RewardPosition pos : getRewardTypes())
				{
					if(pos == RewardPosition.FirstBlood)
					{
						...
						if(getBoolean("firstBloodMessage"))
							screenAnnounce(killer.getReflectionId(), LanguageEngine.getMsg("event_firstBlood", killer.getPlayersName()));
						
						// Achievements
						if (Config.ENABLE_PLAYER_COUNTERS)
							killer.getOwner().getCounters().addPoint("_First_Blood");
						
						EventRewardSystem.getInstance().rewardPlayer(getEventType(), 1, killer, RewardPosition.FirstBlood, null, 0, 0, 0);
						/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: FIRST BLOOD reward given to " + killer.getPlayersName());
						break;
					}
				}
	...
	protected void giveKillingSpreeReward(EventPlayerData killerData)
	{
		if (killerData instanceof PvPEventPlayerData)
		{
			...
			if(!((PvPEventPlayerData) killerData).wasAwardedForSpree(spree))
			{
				...
				{
					((PvPEventPlayerData)killerData).setWasAwardedForSpree(spree);
	...
	public String getEventInfoCb(int instance, Object param)
	{
		...
		try
		{
			...
			List<EventTeam> teams = new FastTable<EventTeam>();
			...
			if(teamsCount == 2)
			{
				...
				FastTable<PlayerEventInfo> temp = new FastTable<PlayerEventInfo>();
				
				int counter = 0;
				for(EventTeam team : teams)
				{
					topPlayers.put(team.getTeamId(), new FastTable<PlayerEventInfo>());
			...
			else if(teamsCount == 1)
			{
				...
				List<PlayerEventInfo> tempPlayers = new FastTable<PlayerEventInfo>();
			...
			else if(teamsCount > 2)
			{
				...
				if(nextButton && previousButton)
					tb.append("<table width=510 bgcolor=2E2E2E><tr><td width=200 align=left><button value=\"Prev page\" action=\"bypass -h " +  EventHtmlManager.BBS_COMMAND + " nextpageteam " + (page-1) + " " + instance + "\" width=85 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>"
							+ "<td width=200 align=right><button value=\"Next page\" action=\"bypass -h " +  EventHtmlManager.BBS_COMMAND + " nextpageteam " + (page+1) + " " + instance + "\" width=85 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr>");
				else if(nextButton)
	...
	public void playerWentAfk(PlayerEventInfo player, boolean warningOnly, int afkTime)
	{
		...
		else
		{
			if(afkTime == 0)
			{
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: player " + player.getPlayersName() + " has just gone afk");
			}
	...
	public boolean addDisconnectedPlayer(PlayerEventInfo player, DisconnectedPlayerData data)
	{
		...
		if(data != null)
		{
			if(_rejoinEventAfterDisconnect && _manager.getState() == State.RUNNING)
			{
				...
				if(instance != null && instance.isActive())
				{
					...
					if(team != null )
					{
						...
						player.setIsRegisteredToMainEvent(true, getEventType(), (getMaxGearScore() > 0));
			...
			{
				player.teleToLocation(86424 + Rnd.get(-40, 40), 148616 + Rnd.get(-40, 40), -3427, 0); // Giran Church
	...
	public void onDisconnect(PlayerEventInfo player)
	{
		...
		{
			/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: player " + player.getPlayersName() + " (instance id = " + player.getReflectionId() + ") disconnecting from the event");
			...
			AbstractEventInstance playersMatch = getMatch(player.getReflectionId());
			if(playersMatch == null)
			{
				NexusLoader.debug("Player's EventInstance is null, called onDisconnect", Level.WARNING);
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: !!! -.- player's EVENT INSTANCE is null after calling onDisconnect. Player's instanceId is = " + player.getReflectionId());
				running = false;
			}
			else 
			...
			if(running)
			{
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: -.- event is active");
				debug(getEventName() + ": Player " + player.getPlayersName() + " disconnected from " + getEventName() + " event.");
				
				if(team.getPlayers().isEmpty())
				{
					announce(player.getReflectionId(), LanguageEngine.getMsg("event_disconnect_team", team.getTeamName()));
					allowRejoin = false;
					
					debug(getEventName() + ": all players from team " + team.getTeamName() + " have disconnected.");
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: ALL PLAYERS FROM TEAM " + team.getTeamName() + " disconnected");
				}
				
				if(!checkIfEventCanContinue(player.getReflectionId(), player))
				{
					announce(player.getReflectionId(), LanguageEngine.getMsg("event_disconnect_all"));
					endInstance(player.getReflectionId(), true, false, false);
					allowRejoin = false;
					
					debug(getEventName() + ": no players left in the teams, the fight can't continue. The event has been aborted!");
					/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: NO PLAYERS LEFT IN THE TEAMS, THE FIGHT CAN'T CONTINUE! (checkIfEventCanContinue = false)");
				...
				/**/ if(NexusLoader.detailedDebug) print("AbstractMainEvent: -.- event IS NOT active anymore");
		}
	}
	...
	public boolean canUseItem(PlayerEventInfo player, ItemData item)
	{
		/*if(isNewbieOnly())
		{
			if(item.getOwner().isARestrictedItemOrcArea())
			{
				player.sendMessage("This is a Newbie-event - all items above S80 grade are not permitted.");
				return false;
			}
		...
		// check if the player can replace his current item with a different item; the different item cannot be stronger due to gear score checks
		if(item.getOwner().isEquipable() && !item.isEquipped())
		{
			if(!player.getOwner().gearScoreForItemChangeCheck(item.getOwner()))
				return false;
		}*/
	...
	public boolean onSay(PlayerEventInfo player, String text, int channel)
	{
		...
		{
			// TODO: InFERN0
			//HtmlManager.showPortableBuffer(player);
			//return false;
		}
