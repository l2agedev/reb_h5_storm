...
import l2r.gameserver.nexus_engine.events.engine.mini.ScheduleInfo;
...
public class EventConfig
{
	...
	public MiniEventManager createDefaultMode(EventType type)
	{
		...
		try
		{
			...
			statement.setString(8, getTimesAvailable(event));
		...
		try
		{
			...
			if(modeId <= 0)
			{
				...
				for(Entry<Integer, MiniEventManager> e : EventManager.getInstance().getMiniEvents().get(type).entrySet())
				{
					...
					statement.setString(8, getTimesAvailable(e.getValue()));
			...
			else
			{
				...
				if(event != null)
				{
					...
					NexusLoader.debug("Tried to save non-existing event mode - " + type.getAltTitle() + ", mode " + modeId, Level.WARNING);
			}
		}
	...
	public GlobalConfigModel addGlobalConfig(String type, String key, String desc, String value, int inputType)
	{
		if(!_globalConfigs.containsKey(type))
			_globalConfigs.put(type, new FastTable<GlobalConfigModel>());
