...
import l2r.gameserver.nexus_engine.events.engine.EventRewardSystem.EventRewards;
import l2r.gameserver.nexus_engine.events.engine.EventRewardSystem.PositionContainer;
import l2r.gameserver.nexus_engine.events.engine.EventRewardSystem.RewardItem;
...
import l2r.gameserver.nexus_engine.events.engine.base.RewardPosition.PositionType;
...
import l2r.gameserver.nexus_engine.events.engine.main.MainEventManager.EventScheduleData;
import l2r.gameserver.nexus_engine.events.engine.main.MainEventManager.RegNpcLoc;
...
import l2r.gameserver.nexus_engine.events.engine.mini.DoorAction;
import l2r.gameserver.nexus_engine.events.engine.mini.EventMode;
import l2r.gameserver.nexus_engine.events.engine.mini.EventMode.FeatureCategory;
import l2r.gameserver.nexus_engine.events.engine.mini.EventMode.FeatureType;
import l2r.gameserver.nexus_engine.events.engine.mini.FeatureBase;
import l2r.gameserver.nexus_engine.events.engine.mini.FeatureBase.FeatureInfo;
...
import l2r.gameserver.nexus_engine.events.engine.mini.ScheduleInfo.Day;
...
import l2r.gameserver.nexus_engine.events.engine.mini.features.AbstractFeature.FeatureConfig;
import l2r.gameserver.nexus_engine.events.engine.mini.tournament.Tournament;
...
public class EventManagement
{
	...
	private class EditingInfo
	{
		...
		private Map<Integer, FastTable<PlayerEventInfo>> manualMatchPlayers = new FastMap<Integer, FastTable<PlayerEventInfo>>();
	}
	...
	public void noInstancesMenu(PlayerEventInfo gm)
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_noinstances.htm");
	...
	public void showMenu(PlayerEventInfo gm, boolean miniEvents)
	{
		...
		String html;
		if(!miniEvents)
			html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_menu.htm");
		else
			html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_menu_mini.htm");
		...
		String runStopButton = EventManager.getInstance().getMainEventManager().autoSchedulerEnabled() ? 
				"<button value=\"Stop\" action=\"bypass -h admin_event_manage abort_auto_scheduler\" width=85 height=23 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" :
				"<button value=\"Start\" action=\"bypass -h admin_event_manage restart_auto_scheduler\" width=85 height=23 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">";
			...
			for(EventType type : EventType.values())
			{
				...
				for(Entry<Integer, MiniEventManager> e : EventManager.getInstance().getMiniEvents().get(type).entrySet())
				{
					if(e.getValue().getMode().isAllowed() && e.getValue().getMode().isRunning())
					{
						...
						tb.append("<td width=45><a action=\"bypass -h admin_event_manage mini_viewinfo " + e.getKey() + " " + type.getAltTitle() + "\"><font color=9f9f9f>View</font></a></td>");
		...
		for(EventType type : EventType.values())
		{
			...
			for(Entry<Integer, MiniEventManager> e : EventManager.getInstance().getMiniEvents().get(type).entrySet())
			{
				if(e.getValue().getMode().isAllowed() && e.getValue().getMode().isRunning())
				{
					...
					tb.append("<td width=130 align=left><font color=ac9775>" + e.getValue().getMode().getVisibleName() + "</font></td>");
					tb.append("<td width=160 align=right>");
					
					try
					{
						int countRegistered = e.getValue().getRegisteredTeamsCount();
						int gamesCount = e.getValue().getActiveGames().size();
						
						tb.append("<font color=9f9f9f>" + countRegistered + " reg; " + gamesCount + " games</font>");
					} 
					catch (Exception ex)
					{
						NexusLoader.debug("sent invalid bypass, event " + type.getAltTitle(), Level.WARNING);
					...
					tb.append("<td align=right width=45><a action=\"bypass -h admin_event_manage mini_viewinfo " + e.getKey() + " " + type.getAltTitle() + "\"><font color=9f9f9f>Edit</font></a></td>");
					tb.append("<td align=right width=45><a action=\"bypass -h admin_event_manage mini_stopevent " + e.getKey() + " " + type.getAltTitle() + "\"><font color=9f9f9f>Stop</font></a></td>");
		...
		if(count == 0)
		{
			...
			tb.append("<tr><td width=270 align=center><font color=a95050>No mini events active currently.</font></td>");
			
			tb.append("</table>");
		}
		
		html = html.replaceAll("%miniEventsList%", tb.toString());
		
		tb = new TextBuilder();
		
		if(EventManager.getInstance().getMainEventManager().getGlobalEventManager().isActive())
		{
			GlobalEvent globalEvent = EventManager.getInstance().getMainEventManager().getGlobalEventManager().getActiveEvent();
			String timeActive = EventManager.getInstance().getMainEventManager().getGlobalEventManager().timeActive();
			String timeLimit = EventManager.getInstance().getMainEventManager().getGlobalEventManager().getTimeLimit();
			
			tb.append("<table width=270 bgcolor=4f4f4f>");
			
			tb.append("<tr><td width=140 align=left><font color=ac9775>Active global event:</font></td>");
			tb.append("<td width=120 align=right>" + globalEvent.getName() + "</td></tr>");
			
			tb.append("<tr><td width=140 align=left><font color=ac9775>Active for:</font></td>");
			tb.append("<td width=120 align=right>" + timeActive + "</td></tr>");
			
			tb.append("<tr><td width=140 align=left><font color=ac9775>Time limit:</font></td>");
			tb.append("<td width=120 align=right>" + timeLimit + "</td></tr>");
		...
		else
		{
			...
			tb.append("<tr><td width=270 align=center><font color=a95050>No global event active currently.</font></td>");
			
			tb.append("</table>");
		}
		
		
		html = html.replaceAll("%globalEventList%", tb.toString());
		
		String nextSchedulerSegmentDelay = EventManager.getInstance().getNextSegmentDelay();
		String nextSchedulerSegmentType = EventManager.getInstance().getNextSegmentName();
		
		html = html.replaceAll("%scheduler_nextsegmentdelay%", nextSchedulerSegmentDelay);
		html = html.replaceAll("%scheduler_nextsegmenttype%", nextSchedulerSegmentType);
	...
	public void watchCurrentMenu(PlayerEventInfo gm)
	{
		...
		String html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_watch_menu.htm");
	...
	public void onBypass(PlayerEventInfo gm, String action)
	{
		try
		{
			...
			else if(action.equals("reload_html"))
			{
				...
				gm.sendMessage("Engine's HTML files successfully reloaded.");
			}
			else if(action.equals("reload_stats"))
			{
				EventStatsManager.getInstance().reload();
				gm.sendMessage("Statistics successfully reloaded.");
			}
			else if(action.equals("debug_sign"))
			{
				gmDebug(gm);
			}
			else if(action.equals("noinstances"))
			{
				noInstancesMenu(gm);
			}
			...
			else if(action.startsWith("event_start"))
			{
				...
				EventManager.getInstance().getMainEventManager().startEvent(gm, event, regTime, mapName, npcLoc, runTime, false);
			}
			...
			else if(action.startsWith("reload_event_scheduler"))
			{
				if(!EventScheduler.getInstance()._isActive)
				{
					EventScheduler.getInstance().reload();
					gm.sendMessage("Event Scheduler successfully reloaded from the files.");
				}
				else
					gm.sendMessage("Disable the scheduler first.");
			}
			else if(action.startsWith("reload_global_events"))
			{
				EventManager.getInstance().getMainEventManager().getGlobalEventManager().reload();
				gm.sendMessage("Global events successfully reloaded from the files.");
			}
			...
			else if(action.startsWith("global_event"))
			{
				String command = action.substring(13);

				if(command.equals("menu"))
				{
					showGlobalEventMenu(gm);
				}
				...
				else if(action2.startsWith("manual_match"))
				{
					...
					else if(action3.startsWith("add_team"))
					{
						...
						if(_editingInfos.get(gm.getPlayersId()).manualMatchPlayers.get(teamId) == null)
							_editingInfos.get(gm.getPlayersId()).manualMatchPlayers.put(teamId, new FastTable<PlayerEventInfo>());
					...
					else if(action3.startsWith("rem_team"))
					{
						...
						for(Map.Entry<Integer, FastTable<PlayerEventInfo>> e : _editingInfos.get(gm.getPlayersId()).manualMatchPlayers.entrySet())
						{
			...
			else if(action.startsWith("set_available"))
			{
				...
				if(EventConfig.getInstance().isEventAllowed(type))
					removeAvailableEvent(gm);
				else
					addAvailableEvent(gm);
			...
			else if(action.startsWith("delete_main_instance_type"))
			{
				...
				{
					gm.sendMessage("Interlude does not support instances.");
			...
			else if(action.startsWith("create_new_main_instance"))
			{
				...
				{
					gm.sendMessage("Interlude does not support instances.");
					showEditEventMenu(gm, _editingInfos.get(gm.getPlayersId()).activeEditingEvent.getAltTitle(), _editingInfos.get(gm.getPlayersId()).activeEventEditingSection);
				}
				else
			...
			{
				String tAction = action.substring(11);

				if(tAction.equalsIgnoreCase("menu"))
				{
					showTournamentMenu(gm);
				}
				else if(tAction.equalsIgnoreCase("new"))
				{
					showTournamentStartMenu(gm);
				}
				else if(tAction.startsWith("selectevent"))
				{
					StringTokenizer st = new StringTokenizer(tAction);
					st.nextToken();
					...
					MiniEventManager manager = EventManager.getInstance().getMiniEvent(type, modeId);

					if(manager != null)
					{
						TournamentManager.setTournamentEvent(gm, manager);
					}

					showTournamentStartMenu(gm);
				}
				else if(tAction.startsWith("regtime"))
				{
					StringTokenizer st = new StringTokenizer(tAction);
					st.nextToken();
					...
					TournamentManager.setTimeForRegistration(gm, time);

					showTournamentStartMenu(gm);
				}
				else if(tAction.startsWith("rewardid"))
				{
					int reward = Integer.parseInt(tAction.substring(9));

					TournamentManager.setRewardItemId(gm, reward);

					showTournamentStartMenu(gm);
				}
				else if(tAction.startsWith("reward"))
				{
					String reward = tAction.substring(7);

					TournamentManager.setRewardString(gm, reward);

					showTournamentStartMenu(gm);
				}
				else if(tAction.startsWith("minteams"))
				{
					StringTokenizer st = new StringTokenizer(tAction);
					st.nextToken();
				...
				else if(tAction.startsWith("maxteams"))
				{
					StringTokenizer st = new StringTokenizer(tAction);
					st.nextToken();
				...
				else if(tAction.equalsIgnoreCase("stop"))
				{
					TournamentManager.stopTournament(gm);
					showTournamentMenu(gm);
				}
				else if(tAction.equalsIgnoreCase("skipreg"))
				{
					TournamentManager.skipRegistration(gm);
					showTournamentMenu(gm);
				}
				else if(tAction.equalsIgnoreCase("launch"))
				{
					TournamentManager.startTournament(gm);
					showTournamentMenu(gm);
				}
			...
			else
			{
				...
				{
					addAvailableEvent(gm, action.substring(17));
					showMapEditation(gm, _editingInfos.get(gm.getPlayersId()).activeEditingMap, _editingInfos.get(gm.getPlayersId()).activeEditingPage, null);
				}
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_lang.htm");
		
		TextBuilder tb = new TextBuilder();
		
		for(Entry<String, String> e : LanguageEngine.getLanguages().entrySet())
		{
			tb.append("<button value=\"" + e.getValue() + "\" action=\"bypass -h admin_event_manage set_language " + e.getKey() + "\" width=50 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			tb.append("<br1>");
		}
	...
	private void showGlobalEventMenu(PlayerEventInfo gm)
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_globalevent_menu.htm");
		...
		GlobalEvent current = EventManager.getInstance().getMainEventManager().getGlobalEventManager().getActiveEvent();

		if(current == null)
			html = html.replaceAll("%event%", "None active");
		else
			html = html.replaceAll("%event%", current.getName());

		gm.sendPacket(html);
		gm.sendStaticPacket();
	}

	private void abortGlobalEvent(PlayerEventInfo gm)
	{
		if(EventManager.getInstance().getMainEventManager().getGlobalEventManager().stopByGm())
		{
			gm.sendMessage("Successfully aborted current global event.");
			showGlobalEventMenu(gm);
		}
		else
		{
			gm.sendMessage("No global event is currently running.");
			showGlobalEventMenu(gm);
		}
	}

	private void startGlobalEvent(PlayerEventInfo gm, String name)
	{
		...
		if(EventManager.getInstance().getMainEventManager().getGlobalEventManager().start(eventName, param))
		{
			gm.sendMessage("Started " + name + ".");
			showGlobalEventMenu(gm);
		}
		else
		{
			gm.sendMessage("Can't start " + name + ".");
			showGlobalEventMenu(gm);
		}
	}
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_order.htm");
		...
		for(int i = 0; i < dataList.size(); i++)
		{
			...
			if(data.getOrder() <= 1)
			{
				...
				tb.append("<td width=60><button value=\"Down\" action=\"bypass -h admin_event_manage eventorder_move down " + data.getEvent().getAltTitle() + "\" width=60 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			}
			else if(data.getOrder() >= dataList.size())
			{
				tb.append("<td width=50><button value=\"Up\" action=\"bypass -h admin_event_manage eventorder_move up " + data.getEvent().getAltTitle() + "\" width=50 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_globalconfig_menu.htm");
		...
		if(configType.equals("Scheduler"))
		{
			...
			tb.append("<td width=275 align=center><font color=DFBC99><a action=\"bypass -h admin_event_manage eventorder_menu \">Edit event order</a></font></td>");
...
//			tb.append("<img src=\"L2UI.SquareBlank\" width=280 height=3>");
		...
		{
			tb.append("<table width=275 bgcolor=444444><tr>");
			tb.append("<td width=275 align=center><font color=DFBC99><a action=\"bypass -h admin_event_manage eventlang_menu \">Engine Language</a></font></td>");
			tb.append("</tr></table><br>");
		}
		...
		if(configType.equals("Features"))
		{
			...
			if(page == 1)
			{
				...
				tb.append("<td width=90 align=center><font color=DFBC99><a action=\"bypass -h admin_event_manage globalconfig_menu Features 2\">Page 2</a></font></td>");
				tb.append("<td width=90 align=center><font color=DFBC99><a action=\"bypass -h admin_event_manage globalconfig_menu Features 3\">Page 3</a></font></td>");
			}
			else if(page == 2)
			{
				tb.append("<td width=90 align=center><font color=DFBC99><a action=\"bypass -h admin_event_manage globalconfig_menu Features 1\">Page 1</a></font></td>");
		...
		for(GlobalConfigModel config : EventConfig.getInstance().getGlobalConfigs(configType))
		{
			...
			tb.append("<td width=160 align=left><font color=" + color + "><a action=\"bypass -h admin_event_manage globalconfig_showinfo " + configType + " " + page + " " + config.getKey() + "\">" + config.getKey() + "</a></font></td>");
			tb.append("<td width=100 align=right><font color=9f9f9f><a action=\"bypass -h admin_event_manage globalconfig_edit " + config.getKey() + "\">" + value + "</a></font></td><td width=10></td>");
			tb.append("</tr></table>");
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_globalconfig_edit.htm");
	...
	private void showNewEventMenu(PlayerEventInfo gm, EventType event, boolean runTimeEdit)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_launch_event.htm");
		...
		if(runTimeEdit)
		{
			html = html.replaceAll("%runTimeName%", "<a action=\"bypass -h admin_event_manage new_event " + event.getAltTitle() + "\">Run time:</a>");
		...
		else
		{
			if(event == null)
				...
				html = html.replaceAll("%runTimeName%", "<a action=\"bypass -h admin_event_manage new_event_runtime_edit " + event.getAltTitle() + "\">Run time:</a>");
	...
	private void showEditEventInstancesMenu(PlayerEventInfo gm, EventType type)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editevent_instances.htm");
		...
		for(MainEventInstanceType instance : event.getInstanceTypes().values())
		{
			...
			tb.append("<td align=right width=80><button value=\"Edit\" action=\"bypass -h admin_event_manage edit_main_instance_type " + instance.getId() + "\" width=70 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
		...
		if(type != EventType.Unassigned)
		{
			...
			html = html.replaceAll("%enableDisable%", EventConfig.getInstance().isEventAllowed(type) ? "<font color=74BE85><a action=\"bypass -h admin_event_manage set_available\">Enabled</a></font>" : "<font color=B46F6B><a action=\"bypass -h admin_event_manage set_available\">Disabled</a></font>");
	...
	private void showEditMainInstanceTypeWindow(PlayerEventInfo gm, int id)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editevent_instances_edit.htm");
		...
		for(Entry<String, ConfigModel> e : instance.getConfigs().entrySet())
		{
			...
			tb.append("<td width=50 align=right><button value=\"Expand\" width=55 action=\"bypass -h admin_event_manage expand_configmodel InstanceType " + e.getKey() + "\" height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			...
			if(expanded)
			{
				...
				tb.append("<td align=left><button value=\"" + e.getValue().getAddButtonName() + "\" width=40 action=\"bypass -h admin_event_manage edit_main_instance_type_" + e.getValue().getAddButtonAction() + "_config " + e.getKey() + " \\$" + e.getKey() + "\" height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				...
				tb.append("<td align=right><button value=\"" + e.getValue().getUtilButtonName() + "\" action=\"bypass -h admin_event_manage edit_main_instance_type_set_config " + e.getKey() + " " + e.getValue().getDefaultVal() + "\" width=" + e.getValue().getUtilButtonWidth() + " height=17 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				tb.append("</tr>");
				tb.append("</table>");
			}
	...
	private void showEditEventMenu(PlayerEventInfo gm, String eventName, String page)
	{
		...
		EventType type = EventType.getType(eventName);
		if(type == null || (!type.isRegularEvent() && !type.isGlobalEvent()))
		{
			gm.sendMessage("Wrong event.");
			return;
		}
		...
		if(type == EventType.PvpZone || page.equals(mainEventEditingPages[1])) //rewards
		{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editevent.htm");
		...
		for(EventMap map : EventMapSystem.getInstance().getMaps(type).values())
		{
			...
			tb.append("<td align=right width=80><button value=\"" + (error ? "! " : "") + "Status" + (error ? " !" : "") + "\" action=\"bypass -h admin_event_manage show_map_status " + map.getGlobalId() + " " + type.getAltTitle() + "\" width=70 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("<td align=right width=80><button value=\"Edit\" action=\"bypass -h admin_event_manage edit_event_map " + map.getGlobalId() + " 0\" width=70 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
	...
	private void showTournamentMenu(PlayerEventInfo gm)
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_tournament_menu.htm");

		TextBuilder tb = new TextBuilder();

		if(TournamentManager.getTournament() == null)
		{
			tb.append("No tournament is active.<br>");
			tb.append("<button value=\"Host new tournament\" action=\"bypass -h admin_event_manage tournament_new\" width=130 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
		}
		else
		{
			tb.append("There is ongoing tournament in event " + TournamentManager.getEvent().getMode().getVisibleName() + "<br>");

			tb.append("<button value=\"Skip registration\" action=\"bypass -h admin_event_manage tournament_skipreg\" width=130 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			tb.append("<button value=\"Stop tournament\" action=\"bypass -h admin_event_manage tournament_stop\" width=130 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");

			//TODO more info
			
			Tournament tourn = TournamentManager.getTournament();
			
			if(tourn.getActiveMatches() != null)
			{
				for(MiniEventGame match : tourn.getActiveMatches())
				{
					int i = 1;
		...
		html = html.replaceAll("%data%", tb.toString());
	...
	private void showTournamentStartMenu(PlayerEventInfo gm)
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_tournament_start.htm");

		TextBuilder tb = new TextBuilder();

		if(TournamentManager.getTournament() != null)
		{
			gm.sendMessage("There is a tournament active already.");
			showTournamentMenu(gm);
			return;
		}
		else
		{
			...
			String eventName = "Not selected";
			String regTime = "Not set";
			String minTeams = "Not set";
			String maxTeams = "Not set";
			String rewards = "Not set";
			String rewardId = "57";

			if(TournamentManager._selectedEvent != null)
				eventName = TournamentManager._selectedEvent.getMode().getVisibleName();

			if(TournamentManager._timeForRegistration > 0)
				regTime = TournamentManager._timeForRegistration + "";

			if(TournamentManager._minTeamsCount > 0)
				minTeams = TournamentManager._minTeamsCount + "";

			if(TournamentManager._maxTeamsCount > 0)
				maxTeams = TournamentManager._maxTeamsCount + "";
			
			if(TournamentManager._rewardItemId > 0)
				rewardId = TournamentManager._rewardItemId + "";

			if(TournamentManager._rewardsString != null)
				rewards = TournamentManager._rewardsString + "";

			tb.append("<tr><td width=100 align=left><font color=ac9887>Event</font></td><td width=180 align=right><font color=9f9f9f>" + eventName + "</font></td></tr>");
			tb.append("<tr><td width=100 align=left><font color=ac9887>Reg time (sec)</font></td><td width=180 align=right><font color=9f9f9f>" + regTime + "</font></td></tr>");
			tb.append("<tr><td width=100 align=left><font color=ac9887>Min teams</font></td><td width=180 align=right><font color=9f9f9f>" + minTeams + "</font></td></tr>");
			tb.append("<tr><td width=100 align=left><font color=ac9887>Max teams</font></td><td width=180 align=right><font color=9f9f9f>" + maxTeams + "</font></td></tr>");
			tb.append("<tr><td width=100 align=left><font color=ac9887>Reward item id</font></td><td width=180 align=right><font color=9f9f9f>" + rewardId + "</font></td></tr>");
			tb.append("<tr><td width=100 align=left><font color=ac9887>Rewards</font></td><td width=180 align=right><font color=9f9f9f>" + rewards + "</font></td></tr>");

			tb.append("</table><br>");

			tb.append("<edit var=\"value\" width=130 height=15>");

			tb.append("<table width=280>");
			tb.append("<tr><td><button value=\"Regtime min\" action=\"bypass -h admin_event_manage tournament_regtime \\$value\" width=90 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("<td><button value=\"Min teams\" action=\"bypass -h admin_event_manage tournament_minteams \\$value\" width=90 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("<td><button value=\"Max teams\" action=\"bypass -h admin_event_manage tournament_maxteams \\$value\" width=90 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr>");
			tb.append("</table><br1>");
			
			tb.append("<table width=280>");
			tb.append("<tr><td><button value=\"Reward it. id\" action=\"bypass -h admin_event_manage tournament_rewardid \\$value\" width=90 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("<td></td>");
			tb.append("<td></td></tr>");
			tb.append("</table><br1>");

			tb.append("<table width=280>");
			tb.append("<tr><td width=185></td>");
			tb.append("<td width=90><button value=\"Set reward\" action=\"bypass -h admin_event_manage tournament_reward \\$value\" width=90 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr>");
			tb.append("</table><br1>");

			tb.append("<button value=\"Launch tournament\" action=\"bypass -h admin_event_manage tournament_launch\" width=150 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			tb.append("<button value=\"Back\" action=\"bypass -h admin_event_manage menu\" width=80 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");

			tb.append("<br><br><font color=LEVEL>select event</font>");
			

			int counter = 0;

			for(Map<Integer, MiniEventManager> m : EventManager.getInstance().getMiniEvents().values())
			{
				for(MiniEventManager manager : m.values())
				{
					counter ++;
					...
					tb.append("<td width=90 align=center><a action=\"bypass -h admin_event_manage tournament_selectevent " + manager.getEventType().getAltTitle() + " " + manager.getMode().getModeId() + "\"><font color=9f9f9f>" + manager.getMode().getVisibleName() + "</font></a></td>");
		...
		tb.append("<br><font color=7f7f7f>For setting rewards, use this format: POSITION1:COUNT_OF_MEDALS;POSITION2-POSITION3:COUNT_OF_MEDALS , for example: 1:20;2:10;3:7;4-5:5 (1st 20 medals, 2nd 10, 3rd 7, 4th and 5th 5 medals)</font><br>");
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editevents.htm");
		...
		for(EventType type : EventType.values())
		{
			...
			else
			{
				...
				if(exists)
					tb.append("<td><button value=\"" + type.getHtmlTitle() + "\" action=\"bypass -h admin_event_manage edit_event " + type.getAltTitle() + "\" width=130 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				else
		...
		tb.append("<table width=270><tr><td><button value=\"Coliseum PvP Zone\" action=\"bypass -h admin_event_manage edit_event pvpzone\" width=130 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td></tr></table>");
	...
	private void showEditModeMaps(PlayerEventInfo gm)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_maps.htm");
		...
		for(EventMap map : EventMapSystem.getInstance().getMaps(_editingInfos.get(gm.getPlayersId()).activeEditingEvent).values())
		{
			...
			if(mode.getDisMaps().contains(map.getGlobalId()))
				tb.append("<td width=90><font color=B46F6B><a action=\"bypass -h admin_event_manage mini_edit_modes_map " + map.getGlobalId() + "\">Disabled</a></font></td></tr>");
			else
				tb.append("<td width=90><font color=74BE85><a action=\"bypass -h admin_event_manage mini_edit_modes_map " + map.getGlobalId() + "\">Enabled</a></font></td></tr>");
		}
	...
	private void createNewModeMenu(PlayerEventInfo gm)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_new_menu.htm");
	...
	private void createRegFile(PlayerEventInfo gm)
	{
		...
		File file = new File(HtmCache.getInstance().getNotNull("mods/nexus/events/" + name, null));
		...
		try
		{
			...
			{
				File defaultFile = new File(HtmCache.getInstance().getNotNull("mods/nexus/events/" + event.getEventType().getAltTitle() + "_Default.htm", null));
				if(!defaultFile.exists())
				{
					gm.sendMessage("The default event file 'mods/nexus/events/" + event.getEventType().getAltTitle() + "_Default.htm' not found.");
					return;
				}
				else
				{
		...
		gm.sendMessage("The file has been created in 'mods/nexus/events/" + name + "'.");
	}
	...
	private void showEditModeSchedulerTime(PlayerEventInfo gm, int timeId)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_scheduler_edit.htm");
		...
		for(Day d : time.days)
		{
			...
			tb.append("<td align=right><button value=\"Remove\" action=\"bypass -h admin_event_manage mini_edit_modes_scheduler_removeday " + d.prefix + "\" width=70 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
		} 
	...
	private void showEditModeSchedulerMenu(PlayerEventInfo gm)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_scheduler_menu.htm");
		...
		for(Entry<Integer, RunTime> e : mode.getScheduleInfo().getTimes().entrySet())
		{
			...
			tb.append("<td align=right><button value=\"Edit\" action=\"bypass -h admin_event_manage mini_edit_modes_scheduler_edit " + e.getKey() + "\" width=50 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
		}
	...
	private void showEditModeWindow(PlayerEventInfo gm, int modeId)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_edit.htm");
		...
		html = html.replaceAll("%enableDisable%", mode.isAllowed() ? "<font color=74BE85><a action=\"bypass -h admin_event_manage mini_edit_modes_enabledisable\">Enabled</a></font>" : "<font color=B46F6B><a action=\"bypass -h admin_event_manage mini_edit_modes_enabledisable\">Disabled</a></font>");
		
		html = html.replaceAll("%deleteButton%", mode.getModeId() == 1 ? "" : "<td width=95><button value=\"Delete mode\" action=\"bypass -h admin_event_manage mini_edit_modes_delete\" width=95 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
		
		boolean isHtmlGenerated = false;
		File file = new File(HtmCache.getInstance().getNotNull("mods/nexus/events/" + event.getEventType().getAltTitle() + "_" + event.getMode().getModeName() + ".htm", null));
		if(file.exists())
			isHtmlGenerated = true;
		
		html = html.replaceAll("%generateButton%", isHtmlGenerated ? "<button value=\"Delete HTML\" action=\"bypass -h admin_event_manage mini_edit_modes_generatefile\" width=85 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">" : "<button value=\"Generate HTML\" action=\"bypass -h admin_event_manage mini_edit_modes_generatefile\" width=85 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
	...
	private void showNewMiniEventFeatureMenu(PlayerEventInfo gm, String featureType)
	{
		...
		if(!featureType.equals("Default"))
		{
			...
			try
			{
				_constructor = Class.forName("l2r.gameserver.nexus_engine.events.engine.mini.features." + featureType + "Feature").getConstructor(classParams);
			} 
			catch (Exception e)
			{
				e.printStackTrace();
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_featureedit_new.htm");
		...
		for(FeatureType feature : FeatureType.values())
		{
			...
			if(!alreadyIn && allowedForEvent && allowedForCategory)
			{
				...
				tb.append("<td width=75 align=left><font color=ac9887><a action=\"bypass -h admin_event_manage mini_edit_feature_showinfo " + feature.toString() + "\">View Info</a></font></td>");
				tb.append("<td width=90 align=right><button value=\"Add to mode\" action=\"bypass -h admin_event_manage mini_edit_feature_add " + feature.toString() + "\" width=90 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				tb.append("</tr></table>");
	...
	private void showEditFeatureConfig(PlayerEventInfo gm, String featureName, String config)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_featureedit_config.htm");
	...
	private void showEditFeature(PlayerEventInfo gm, String featureName)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_featureedit.htm");
		...
		for(FeatureConfig c : feature.getConfigs())
		{
			...
			tb.append("<tr>");
			tb.append("<td align=left><font color=ac9887><a action=\"bypass -h admin_event_manage mini_edit_featureconfig " + featureName + " " + c.name + "\">" + c.name + "</font></td>");
			tb.append("<td align=right><font color=9f9f9f><a action=\"bypass -h admin_event_manage mini_edit_featureconfig " + featureName + " " + c.name + "\">" + value + "</a></font></td>");
			tb.append("</tr>");
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_menu_compact.htm");
		...
		for(Entry<Integer, MiniEventManager> e : EventManager.getInstance().getMiniEvents().get(_editingInfos.get(gm.getPlayersId()).activeEditingEvent).entrySet())
		{
			tb.append("<tr><td width=180><font color=" + (e.getValue().getMode().isRunning() ? "ac9887" : "606060") + "><a action=\"bypass -h admin_event_manage mini_edit_modes_menu " + e.getValue().getMode().getModeId() + "\">" + e.getValue().getMode().getModeName() + "</a></font></td>");
			
			if(!e.getValue().getMode().isAllowed())
				tb.append("<td width=90><font color=B46F6B><a action=\"bypass -h admin_event_manage mini_edit_modes_enabledisable " + e.getValue().getMode().getModeId() + "\">Disabled</a></font></td></tr>");
			else
				tb.append("<td width=90><font color=74BE85><a action=\"bypass -h admin_event_manage mini_edit_modes_enabledisable " + e.getValue().getMode().getModeId() + "\">Enabled</a></font></td></tr>");
		}
	...
	private void showModesMenu(PlayerEventInfo gm)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_modes_menu.htm");
		...
		for(AbstractFeature feature : event.getMode().getFeatures())
		{
			...
			tb.append("</font></td>");
			tb.append("<td width=70 align=right><button value=\"Edit\" action=\"bypass -h admin_event_manage mini_edit_feature " + feature.getType().toString() + "\" width=70 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
		}
		...
		html = html.replaceAll("%enableDisable%", EventConfig.getInstance().isEventAllowed(event.getEventType()) ? "<font color=74BE85><a action=\"bypass -h admin_event_manage set_available\">Enabled</a></font>" : "<font color=B46F6B><a action=\"bypass -h admin_event_manage set_available\">Disabled</a></font>");
	...
	private void showManualMatchSetMapMenu(PlayerEventInfo gm)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_manualmatch_map.htm");
		
		TextBuilder tb = new TextBuilder();
		
		for(EventMap map : EventMapSystem.getInstance().getMaps(_editingInfos.get(gm.getPlayersId()).activeEditingEvent).values())
		{
			tb.append("<button value=\"" + map.getMapName() + "\" action=\"bypass -h admin_event_manage mini_manual_match_set_map " + map.getGlobalId() + "\" width=160 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			tb.append("<br1>");
		}
	...
	private void showMatches(PlayerEventInfo gm)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_showmatches.htm");
		//
		TextBuilder tb = new TextBuilder();
		tb.append(event.getMode().getModeName() + ";");
		...
		for(MiniEventGame match : event.getActiveGames())
		{
			...
			tb.append("\" action=\"bypass -h admin_event_manage mini_edit_match " + match.getGameId() + "\" width=180 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
			tb.append("<br1>");
		}
	...
	private void editMatch(PlayerEventInfo gm, int matchId)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_editmatch.htm");
		...
		for(EventTeam team : game.getTeams())
		{
			...
			tb.append("<td><button value=\"Show\" action=\"bypass -h admin_event_manage mini_show_team_members " + team.getTeamId() + "\" width=60 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
		}
	...
	private void watchMatch(PlayerEventInfo gm)
	{
		...
		gm.setInstanceId(game.getReflectionId());
	...
	private void showTeamMembers(PlayerEventInfo gm, int teamId)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_showteam.htm");
		...
		for(PlayerEventInfo player : game.getTeams()[teamId - 1].getPlayers())
		{
			...
			tb.append("<td><button value=\"Teleport\" action=\"bypass -h admin_teleportto " + player.getPlayersName() + "\" width=65 height=15 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
		}
	...
	private void showManualMatchMenu(PlayerEventInfo gm)
	{
		try
		{
			...
			String html;
			html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_manualmatch.htm");
			...
			for(Map.Entry<Integer, FastTable<PlayerEventInfo>> e : _editingInfos.get(gm.getPlayersId()).manualMatchPlayers.entrySet())
			{
				...
				{
					tb.append("<tr>");
					tb.append("<td><font color=" + EventManager.getInstance().getTeamColorForHtml(e.getKey()) + ">" + ((player == null || player.isOnline() == false) ? "Offline. Clear list" : player.getPlayersName()) + "</font></td>");
					tb.append("<td><button value=\"Remove\" action=\"bypass -h admin_event_manage mini_manual_match_rem_team " + player != null ? player.getPlayersId() : 0 + "\" width=70 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
					tb.append("</tr>");
				}
			}
			tb.append("<tr>");
			tb.append("<td></td><td><button value=\"Clear\" action=\"bypass -h admin_event_manage mini_manual_match_clear\" width=70 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
	...
	private void startManualMatch(PlayerEventInfo gm)
	{
		try
		{
			...
			FastTable<PlayerEventInfo> infos;
			for(Entry<Integer, FastTable<PlayerEventInfo>> i : _editingInfos.get(gm.getPlayersId()).manualMatchPlayers.entrySet())
			{
				infos = new FastTable<PlayerEventInfo>();
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_menu.htm");
		...
		for(Map.Entry<EventType, Map<Integer, MiniEventManager>> e : EventManager.getInstance().getMiniEvents().entrySet())
		{
			if(e.getKey().allowEdits() && EventConfig.getInstance().isEventAllowed(e.getKey()))
			{
				...
				tb.append("<td><button value=\"" + e.getKey().getHtmlTitle() + "\" action=\"bypass -h admin_event_manage mini_edit_event " + e.getKey().getId() + "\" width=140 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				count++;
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_spawntypeinfo_menu.htm");
		...
		for(EventType type : EventType.values())
		{
			...
			{
				tb.append("<button value=\"" + type.getHtmlTitle() + "\" action=\"bypass -h admin_event_manage spawn_type_info_event " + type.getAltTitle() + "\" width=160 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
				tb.append("<br1>");
			}
		}
	...
	private void showSpawnTypeInfoEvent(PlayerEventInfo gm, String eventName)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_spawntypeinfo_info.htm");
		...
		tb.append("<table width=280>");
		for(Map.Entry<SpawnType, String> e : event.getAvailableSpawnTypes().entrySet())
		{
	...
	private void showEditMiniEventMenu(PlayerEventInfo gm, EventType type, String page)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mini_editevent.htm");
	...
	private void editReward(PlayerEventInfo gm, String parameter, String action)
	{
		...
		else if(action.equals("set_min"))
		{
			...
			if(item.maxAmmount < item.minAmmount)
				item.maxAmmount = item.minAmmount;
		}
	...
	private void showRewardEditation(PlayerEventInfo gm, int rewardId)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editreward.htm");
	...
	private void showRewardsEditation(PlayerEventInfo gm, String event, RewardPosition position, String parameter)
	{
		...
		//TODO unhardcode
		if(type == EventType.PvpZone)
			eventInstance = PvpZoneManager._coliseum;
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editrewards.htm");
		...
		if(!type.isRegularEvent())
		{
			...
			tb.append("<table width=270>");
			tb.append("<tr>");
			tb.append("<td width=60><font color=ac9775><a action=\"bypass -h admin_event_manage mini_set_active_mode reward Default\">Modes:</a></font></td>");
			tb.append("<td width=100><combobox width=88 height=17 var=ebox list=%modes%></td>");
			tb.append("<td width=40><button value=\"Set\" action=\"bypass -h admin_event_manage mini_set_active_mode reward \\$ebox\" width=45 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("<td width=40><button value=\"New\" action=\"bypass -h admin_event_manage mini_edit_modes_new\" width=45 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
			tb.append("</table>");
		...
		if((!type.isRegularEvent() && !type.isGlobalEvent()))
		{
			MiniEventManager manager = EventManager.getInstance().getMiniEvent(_editingInfos.get(gm.getPlayersId()).activeEditingEvent, _editingInfos.get(gm.getPlayersId()).activeEventModeId);
		...
		else
		{
			...
			for(Entry<PositionContainer, Map<Integer, RewardItem>> e : rewards.getAllRewards().entrySet())
			{
				...
				tb.append("<font color=" + ((e.getKey().position == position && (e.getKey().parameter == null || e.getKey().parameter.equals(parameter))) ? "LEVEL" : "9f9f9f") + "><a action=\"bypass -h admin_event_manage edit_event_reward_menu " + type.getAltTitle() + " " + e.getKey().position.toString() + " " + (e.getKey().parameter == null ? "" : e.getKey().parameter) + "\">");
				...
				else if(e.getKey().position.posType == PositionType.Numbered)
				{
					size += e.getKey().parameter.length() + 10;
		...
		if(position != RewardPosition.None)
		{
			for(Map.Entry<Integer, RewardItem> e : EventRewardSystem.getInstance().getRewards(type, _editingInfos.get(gm.getPlayersId()).activeEventModeId, position, parameter).entrySet())
			{
				...
				tb.append("<button value=\"" + itemName + " (" + e.getValue().minAmmount + "-" + e.getValue().maxAmmount + ")" + "\" action=\"bypass -h admin_event_manage edit_event_reward " + e.getKey() + "\" width=200 height=19 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\">");
				tb.append("<br1>");
			}
		}
		else
	...
	public void showAddPositionToRewardedWindow(PlayerEventInfo gm)
	{
		...
		if(_editingInfos.get(gm.getPlayersId()).activeEditingEvent == EventType.PvpZone)
			eventInstance = PvpZoneManager._coliseum;
		
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editrewards_addpos.htm");
		
		html = html.replaceAll("%eventName%", _editingInfos.get(gm.getPlayersId()).activeEditingEvent.toString());
		
		html = html.replaceAll("%enableDisable%", EventConfig.getInstance().isEventAllowed(_editingInfos.get(gm.getPlayersId()).activeEditingEvent) ? "<font color=74BE85><a action=\"bypass -h admin_event_manage set_available\">Enabled</a></font>" : "<font color=B46F6B><a action=\"bypass -h admin_event_manage set_available\">Disabled</a></font>");
		...
		if(count > 0)
		{
			...
			tb.append("<table width=280><tr>");
			tb.append("<td width=90 align=left><font color=9f9f9f><a action=\"bypass -h admin_event_manage show_add_position_window_help\">General:</a></font></td>");
			tb.append("<td width=160 align=right><combobox width=150 height=17 var=gen list=\"" + general + "\"></td>");
			tb.append("<td width=40 align=right><button value=\"Add\" action=\"bypass -h admin_event_manage add_rewarded_position \\$gen\" width=40 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr></table>");
		...
		if(count > 0)
		{
			...
			tb.append("<table width=280><tr>");
			tb.append("<td width=150 align=left><font color=9f9f9f><a action=\"bypass -h admin_event_manage show_add_position_window_help\">Numbered:</a></font></td>");
			tb.append("<td align=right><edit var=\"num\" width=35 height=13></td>");
			tb.append("<td align=left>.</td>");
			tb.append("<td width=40 align=right><button value=\"Add\" action=\"bypass -h admin_event_manage add_rewarded_position Numbered \\$num\" width=40 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr></table>");
		...
		if(count > 0)
		{
			...
			tb.append("<table width=280><tr>");
			tb.append("<td width=90 align=left><font color=9f9f9f><a action=\"bypass -h admin_event_manage show_add_position_window_help\">Killing Spree:</a></font></td>");
			tb.append("<td align=right><edit var=\"num\" width=35 height=13></td>");
		...
		if(count > 0)
		{
			...
			tb.append("<table width=280><tr>");
			tb.append("<td width=150 align=left><font color=9f9f9f><a action=\"bypass -h admin_event_manage show_add_position_window_help\">Range:</a></font></td>");
			tb.append("<td align=right><edit var=\"ran1\" width=40 height=13></td>");
			tb.append("<td align=left> - </td>");
			tb.append("<td align=left><edit var=\"ran2\" width=40 height=13></td>");
			
			tb.append("<td align=right><button value=\"Add\" action=\"bypass -h admin_event_manage add_rewarded_position Range \\$ran1 \\$ran2\" width=40 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr></table>");
		...
		if(count > 0)
		{
			...
			tb.append("<table width=280><tr>");
			tb.append("<td width=90 align=left><font color=9f9f9f><a action=\"bypass -h admin_event_manage show_add_position_window_help\">Custom:</a></font></td>");
			tb.append("<td width=160 align=right><combobox width=150 height=17 var=bon list=\"" + bonus + "\"></td>");
			tb.append("<td width=40 align=right><button value=\"Add\" action=\"bypass -h admin_event_manage add_rewarded_position \\$bon\" width=40 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr></table>");
	...
	public void showAddPositionToRewardedWindowHelp(PlayerEventInfo gm)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editrewards_addpos_help.htm");
	...
	private int newSpawn(PlayerEventInfo gm)
	{
		...
		List<EventSpawn> list = new FastTable<EventSpawn>();
		list.add(spawn);
	...
	private void showEditSpawnMenu(PlayerEventInfo gm, int spawnId)
	{
		...
		switch(spawn.getSpawnType())
		{
			case Base:
			{
				html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editspawn_base.htm");
			...
			case Door:
				html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editspawn_door.htm");
				...
				break;
			case Fence:
				html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editspawn_fence.htm");
				...
				break;
			case Npc:
				html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editspawn_npc.htm");
			...
			case Boss:
				...
				/*if(map.getEvents().contains(EventType.SurvivalArena))
				{
					...
					html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editspawn_monster_survival.htm");
					...
					html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editspawn_monster.htm");
				
				break;
			default:
				html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editspawn.htm");
		}
		...
		else
		{
			...
			if(spawn != null && spawn.getSpawnType() != SpawnType.Door && spawn.getSpawnType() != SpawnType.Fence)
			{
				...
				if(template != null)
				{
					try
					{
						NpcData npc = template.doSpawn(spawn.getLoc().getX(), spawn.getLoc().getY(), spawn.getLoc().getZ(), 1, gm.getHeading(), gm.getReflectionId());
			...
			else if(spawn.getSpawnType() == SpawnType.Fence)
			{
				...
				fence.setLoc(spawn.getLoc().getX(), spawn.getLoc().getY(), spawn.getLoc().getZ());
				List<FenceData> f = new FastTable<FenceData>();
				f.add(fence);
				CallBack.getInstance().getOut().spawnFences(f, gm.getReflectionId());
	...
	{	
		...
		if(spawn != null)
		{
			gm.teleToLocation(spawn.getLoc());
			gm.sendMessage("Done.");
		}
		else
			gm.sendMessage("The spawn with ID " + spawnId + " doesn't exist.");
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_filter_menu.htm");
	...
	private void showConfigsMenu(PlayerEventInfo gm, String type, int page)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_config_menu.htm");
		...
		List<ConfigModel> configs = new FastTable<ConfigModel>();
		...
		for(ConfigModel config : configs)
		{
			...
			tb.append("<td width=45 align=left><font color=9f9f9f>" + (brackets ? "(" : "") + "" + value + "" + (brackets? ")" : "") + "</font></td>");
			tb.append("<td width=50 align=right><button value=\"Expand\" width=55 action=\"bypass -h admin_event_manage expand_configmodel Event " + config.getKey() + "\" height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			...
			if(expanded)
			{
				...
				tb.append("<td align=left><button value=\"" + config.getAddButtonName() + "\" width=40 action=\"bypass -h admin_event_manage " + config.getAddButtonAction() + "_config " + config.getKey() + " \\$" + config.getKey() + "\" height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				...
				tb.append("<td align=right><button value=\"" + config.getUtilButtonName() + "\" action=\"bypass -h admin_event_manage set_config " + config.getKey() + " " + config.getDefaultVal() + "\" width=" + config.getUtilButtonWidth() + " height=17 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				tb.append("</tr>");
				tb.append("</table>");
			}
		...
		if(maxPages > 1)
		{
			...
			for(int i = 0; i < maxPages; i++)
			{
				...
				tb.append("<td align=center width=72><font color=" + (page == i+1 ? "80B382" : "9f9f9f") + "><a action=\"bypass -h admin_event_manage event_configs_menu_page " + (i + 1) + "\">Page " + (i+1) + "</a></font></td>");
		...
		html = html.replaceAll("%enableDisable%", EventConfig.getInstance().isEventAllowed(eventType) ? "<font color=74BE85><a action=\"bypass -h admin_event_manage set_available\">Enabled</a></font>" : "<font color=B46F6B><a action=\"bypass -h admin_event_manage set_available\">Disabled</a></font>");
		...
		else
		{
			...
			if(category.equals("General"))
				tb.append("<td align=center><font color=D89C67><a action=\"bypass -h admin_event_manage show_configs_category General\">[General]</a></font></td>");
			else
				tb.append("<td align=center><font color=7f7f7f><a action=\"bypass -h admin_event_manage show_configs_category General\">[General]</a></font></td>");
			...
			for(String cat : event.getCategories())
			{
				...
				if(category.equals(cat))
					tb.append("<td align=center><font color=D89C67><a action=\"bypass -h admin_event_manage show_configs_category " + cat + "\">[" + cat + "]</a></font></td>");
				else 
					tb.append("<td align=center><font color=7f7f7f><a action=\"bypass -h admin_event_manage show_configs_category " + cat + "\">[" + cat + "]</a></font></td>");
	...
	private void showConfig(PlayerEventInfo gm, String key)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_config_show.htm");
	...
	private void showConfigMenu(PlayerEventInfo gm, String type)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mapconfig_menu.htm");
		...
		for(Entry<String, ConfigModel> e : map.getConfigModels().get(event).entrySet())
		{
			...
			tb.append("<td width=45 align=left><font color=9f9f9f>" + (brackets ? "(" : "") + "" + value + "" + (brackets? ")" : "") + "</font></td>");
			tb.append("<td width=50 align=right><button value=\"Expand\" width=55 action=\"bypass -h admin_event_manage expand_configmodel MapConfig " + e.getKey() + "\" height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			...
			if(expanded)
			{
				...
				tb.append("<tr>");
				tb.append("<td>" + e.getValue().getInputHtml(180) + "</td>");
				tb.append("<td align=left><button value=\"" + e.getValue().getAddButtonName() + "\" width=40 action=\"bypass -h admin_event_manage " + e.getValue().getAddButtonAction() + "_map_config " + event.getAltTitle() + " " + e.getKey() + " \\$" + e.getKey() + "\" height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				...
				tb.append("<td align=right><button value=\"" + e.getValue().getUtilButtonName() + "\" action=\"bypass -h admin_event_manage set_map_config " + event.getAltTitle() + " " + e.getKey() + " " + e.getValue().getDefaultVal() + "\" width=" + e.getValue().getUtilButtonWidth() + " height=17 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
				tb.append("</tr>");
				tb.append("</table>");
			}
		...
		/*String[] configs = map.getAvailableConfigs(event);
		if(configs == null)
		{
		...
		for(String key : configs)
		{
			...
			tb.append("<tr>");
			tb.append("<td align=left><font color=ac9887><a action=\"bypass -h admin_event_manage show_map_config false " + event.getAltTitle() + " " + key + "\">" + key + "</a></font></td>");
			tb.append("<td align=right><font color=9f9f9f><a action=\"bypass -h admin_event_manage show_map_config false " + event.getAltTitle() + " " + key + "\">" + value + "</a></font></td>");
			//tb.append("<td align=right><button value=\"Edit\" action=\"bypass -h admin_event_manage show_map_config " + event.getAltTitle() + " " + key + "\" width=35 height=17 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
			tb.append("</tr>");
	...
	private void showMapStatus(PlayerEventInfo gm, int mapId, String event)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_mapstatus.htm");
	...
	private void showMapEditationAddEvent(PlayerEventInfo gm, int mapId)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editmap_add_event.htm");
		...
		for(EventType available : EventType.values())
		{
			if(available.allowEdits() && available != EventType.Unassigned)
			{
				if(!map.getEvents().contains(available))
				{
					if(EventManager.getInstance().getEvent(available) != null)
					{
						counter ++;
						...
						tb.append("<td><button value=\"" + available.getAltTitle() + "\" action=\"bypass -h admin_event_manage add_event_to_map " + available.getAltTitle() + "\" width=85 height=20 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editmap_info.htm");
	...
	private void showMapEditationEvents(PlayerEventInfo gm, int mapId)
	{
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editmap_events.htm");
	...
	private void showMapEditation(PlayerEventInfo gm, int mapId, int page, EventType predefinedEvent)
	{
		...
		if(map == null)
		{
			List<EventType> defaultEvent = new FastTable<EventType>();
		...
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_editmap_spawns.htm");
		
		tb = new TextBuilder();

		tb.append("<table width=275 bgcolor=\"666666\">>");
		tb.append("<tr>" +
				"<td width=20><font color=D5D5D5><a action=\"bypass -h admin_event_manage sort_map_spawns id\">ID</a></font></td>" +
				"<td width=40 align=center><font color=D5D5D5><a action=\"bypass -h admin_event_manage sort_map_spawns type\">Type</a></font></td>" +
				"<td width=70 align=center><font color=D5D5D5><a action=\"bypass -h admin_event_manage sort_map_spawns team\">Team</a></font></td>" +
				"<td width=80 align=right><combobox width=80 height=15 var=filt list=\"" + filters + "\"></td>" +
				"<td width=50 align=right><button value=\"Filter\" action=\"bypass -h admin_event_manage filter_event_spawns \\$filt\" width=50 height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>" +
				"</tr>");
		tb.append("</table>");
		...
		final int maxSpawnsPerPage = NexusLoader.isLimitedHtml() ? 8 : 15;
		...
		for(EventSpawn spawn : map.getSpawns())
		{
			...
			if(count > firstSpawnShowed && count <= lastSpawnShowed)
			{
				...
				//if(spawn.getSpawnType() == SpawnType.Regular || spawn.getSpawnType() == SpawnType.Safe)
				{
					...
					bgcolor = Integer.toHexString(0xFF0000 | color.getRGB()).substring(2);
				}
				
				tb.append("<table width=275 bgcolor=" + bgcolor + ">");
				tb.append("<tr>");
				
				tb.append("<td width=15><a action=\"bypass -h admin_event_manage expand_spawn_info " + spawn.getSpawnId() + "\"><font color=" + spawn.getSpawnType().getHtmlColor() + ">" + spawn.getSpawnId() + "</a></font></td>");
				...
				tb.append("<td width=60 align=right><button value=\"" + spawnUnspawn + "\" action=\"bypass -h admin_event_manage show_spawn " + spawn.getSpawnId() + "\" width=50 height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>"); // show
				tb.append("<td align=right><button value=\"Expand\" action=\"bypass -h admin_event_manage expand_spawn_info " + spawn.getSpawnId() + "\" width=60 height=18 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>"); // expand
				...
				if(_editingInfos.get(gm.getPlayersId()).activeEditingSpawn == spawn.getSpawnId())
				{
					...
					if(team == 0 || team == -1)
						bgcolor = Integer.toHexString(0xFF0000 | color.darker().getRGB()).substring(2);
					else
						bgcolor = Integer.toHexString(0xFF0000 | color.darker().darker().getRGB()).substring(2);
					...
					else
					{
						...
						desc = event.getAvailableSpawnTypes().get(spawn.getSpawnType());
					}
					...
					tb.append("<table width=270 bgcolor=" + bgcolor + ">");
					tb.append("<tr>");
					tb.append("<td><button value=\"Delete\" action=\"bypass -h admin_event_manage remove_spawn " + spawn.getSpawnId() + "\" width=83 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
					tb.append("<td><button value=\"Teleport to\" action=\"bypass -h admin_event_manage teleport_spawn " + spawn.getSpawnId() + "\" width=83 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
					tb.append("<td><button value=\"Edit spawn\" action=\"bypass -h admin_event_manage edit_spawn_menu " + spawn.getSpawnId() + "\" width=83 height=21 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></td>");
					tb.append("</tr>");
					tb.append("</table>");
				}
		...
		if(size > maxSpawnsPerPage)
		{
			...
			for(int x = 0; x < maxPages + 1; x++)
			{
				...
				if(useButtons)
					tb.append("<td><center><button value=\"P " + (x + 1) + "\" action=\"bypass -h admin_event_manage edit_event_map " + mapId + " " + x + "\" width=60 height=17 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center></td>");
				else
					tb.append("<td><center><a action=\"bypass -h admin_event_manage edit_event_map " + mapId + " " + x + "\"><font color=ac9887>P" + (x + 1) + "</font></a></center></td>");
	...
	{
		String html;
		html = EventManager.getInstance().getHtmlManager().getAdminHtml("admin/events/nexus/eventmanage_messagemenu.htm");
