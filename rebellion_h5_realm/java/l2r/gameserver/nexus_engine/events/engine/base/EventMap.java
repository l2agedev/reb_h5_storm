...
{
	private static final Logger _log = LoggerFactory.getLogger(EventMap.class);
	...
	private boolean _beingUsed = false;
	...
	public EventMap(int mapId, String mapName, String mapDesc, List<EventType> events, List<EventSpawn> spawns, String configs)
	{
		...
		_spawns = new FastTable<EventSpawn>();
		...
		if(_events == null)
		{
			...
			_events = new FastTable<EventType>();
		}
	...
	public FastTable<EventSpawn> getSpawns(int teamId, SpawnType type)
	{
		FastTable<EventSpawn> temp = new FastTable<EventSpawn>();
	...
	public FastTable<EventSpawn> getMarkers(int teamId)
	{
	...
	{
		FastTable<EventSpawn> spawns = getSpawns(teamId, type);
	...
	private void initDoors()
	{
		for(EventSpawn spawn : _spawns)
		{
			...
			{
				if(_doorsSpawn == null)
					_doorsSpawn = new FastTable<EventSpawn>();
	...
	public String[] getAvailableConfigs(EventType type)
	{
		if(type != EventType.Unassigned && _events.contains(type))
		{
			...
			if(event == null)
			{
				_log.warn("NexusEngine: EventMap: null event at getAvailableConfigs(EventType)");
				return null;
			}
		...
		_log.warn("NexusEngine: getAvailableConfigs - type " + type.getAltTitle() + " returned null.");
		return null;
	}
