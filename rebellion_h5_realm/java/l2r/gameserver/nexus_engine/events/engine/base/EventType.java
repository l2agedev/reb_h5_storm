...
import l2r.gameserver.nexus_engine.events.engine.main.events.Battlefield;
import l2r.gameserver.nexus_engine.events.engine.main.events.CaptureTheFlag;
import l2r.gameserver.nexus_engine.events.engine.main.events.Commanders;
import l2r.gameserver.nexus_engine.events.engine.main.events.Deathmatch;
import l2r.gameserver.nexus_engine.events.engine.main.events.Domination;
import l2r.gameserver.nexus_engine.events.engine.main.events.HuntingGrounds;
import l2r.gameserver.nexus_engine.events.engine.main.events.LastManStanding;
import l2r.gameserver.nexus_engine.events.engine.main.events.LuckyChests;
import l2r.gameserver.nexus_engine.events.engine.main.events.MassDomination;
import l2r.gameserver.nexus_engine.events.engine.main.events.Mutant;
import l2r.gameserver.nexus_engine.events.engine.main.events.TeamVsTeam;
import l2r.gameserver.nexus_engine.events.engine.main.events.TreasureHunt;
import l2r.gameserver.nexus_engine.events.engine.main.events.TreasureHuntPvp;
import l2r.gameserver.nexus_engine.events.engine.main.events.VIPTeamVsTeam;
import l2r.gameserver.nexus_engine.events.engine.main.events.Zombies;
...
public enum  EventType
{
	...
	Commanders(15, "Commanders", "Commanders", Category.MainTeam, true, false, Commanders.class), 
	
	// Main events to be done
	BombFight(16, "Bomb", "Bomb Fight",  Category.MainTeam, true, false),
	RussianRoulette(17, "Russian", "Russian Roulette",  Category.MainTeam, true, false),
	Simon(18, "Simon", "Simon Says",  Category.MainTeam, true, false),
	//VIP(19, "VIP", "VIP", Category.MainTeam, true, false),
	...
	private static final Logger _log = LoggerFactory.getLogger(EventType.class);
	...
	public Event loadEvent(MainEventManager manager)
	{
		if(eventClass != null)
		{
			...
			{
				_log.error("NexusEngine: error while loading " + getAltTitle() + " event !!!");
				e.printStackTrace();
	...
	{
		List<EventType> types = new FastTable<EventType>();
	...
	public static EventType getNextRegularEvent(int lastId)
	{
		...
		// just return first available event
		for(EventType t : values())
		{
