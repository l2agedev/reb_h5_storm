...
public class EventMapSystem
{
	...
	public List<EventSpawn> loadSpawns(int arenaId)
	{
		...
		EventSpawn spawn;
		List<EventSpawn> spawns = new FastTable<EventSpawn>();
	...
	public EventMap getNextMap(MiniEventManager manager, int lastId, EventMode mode)
	{
		...
		while(limit < _maps.get(type).size() + 99)
		{
			...
			if(map == null) // no next map is available
			{
				nextMapId = 0;
				continue;
			}
		...
		if(map == null)
			NexusLoader.debug("No map available for event " + type.getAltTitle() + " and mode " + mode.getModeName(), Level.WARNING);
