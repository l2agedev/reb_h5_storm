...
import l2r.gameserver.nexus_engine.events.engine.mini.features.LevelFeature;
...
public class EventHtmlManager implements IHtmlManager
{
	...
	public EventHtmlManager()
	{
		...
		//loadAdminHtmls(); THERE ARE NO HTML FILES IN THE JAR!!!
	...
	public void loadAdminHtmls()
	{
		...
		try
		{
			jarFile = new JarFile("./libs/gameserver.jar");
		...
		NexusLoader.debug("Loaded " + _cache.size() + " HTML files from nexus_events.jar", Level.INFO);
	}
	...
	public String getAdminHtml(String name)
	{
		...
		else
		{
			...
			return HtmCache.getInstance().getNotNull(name, null);
		}
	}
	...
	public boolean showNpcHtml(PlayerEventInfo player, NpcData npc)
	{
		...
		if(npc.getNpcId() == EventConfig.getInstance().getGlobalConfigInt("globalEventManagerId"))
		{
			showGlobalEventMenu(player, npc);
			return true;
		}
	...
	protected void showGlobalEventMenu(PlayerEventInfo player, NpcData npc)
	{
		String html;
		
		TextBuilder tb = new TextBuilder();
		
		tb.append("<html><title>Global Event Manager</title><body>");
		...
		{
			tb.append("<table width=280 bgcolor=353535><tr><td align=center width=280><font color=9F9400>No Global event is active at the moment.</font></td></tr></table>");
		}
		else
		{
			String name = EventManager.getInstance().getMainEventManager().getGlobalEventManager().getEventName();
			...
			{
				registerButton = "You are already in another event.";
			}
			else
			{
				registerButton = ("<center><button value=\"Teleport and Help!\" action=\"bypass -h nxs_globalevent_teleport\" width=120 height=20 back=\"rab.2*\" fore=\"rab.2*\"></center>");
			}
			
			tb.append(registerButton);
			
			tb.append("<table width=280 bgcolor=353535>");
			tb.append("<tr><td width=35></td><td align=left width=120><font color=9F9400>Event name:</font></td><td align=center width=135><font color=9F9400>" + name + "</font></td></tr>");
			tb.append("<tr><td width=35></td><td align=left width=120><font color=9F9400>Time limit:</font></td><td align=center width=135><font color=9F9400>" + timeLimit + "</font></td></tr>");
			tb.append("</table>");
		}
	...
	protected boolean showHomePage(PlayerEventInfo player, Object param, Object param2, boolean fullInfo)
	{
		...
		if(EventManager.getInstance().getCurrentMainEvent() != null)
		{
			switch(EventManager.getInstance().getMainEventManager().getState())
			{
				case REGISTERING:
				{
					text = CallBack.getInstance().getOut().getHtml("mods/nexus/cb_registering.htm");
					if(text == null)
					{
				...
				case TELE_BACK:
				{
					text = CallBack.getInstance().getOut().getHtml("mods/nexus/cb_eventactive.htm");
					if(text == null)
					{
		...
		{
			text = CallBack.getInstance().getOut().getHtml("mods/nexus/cb_noevent.htm");
			if(text == null)
			{
	...
	private static void teleportToGlobalEvent(PlayerEventInfo player)
	{
		if(player != null)
		{
			EventManager.getInstance().getMainEventManager().getGlobalEventManager().registerPlayer(player);
		}
	}
	...
	private static void unregisterPvpZone(PlayerEventInfo player)
	{
		...
		NexusEvents.unregisterFromPvpZone(player, true);
	}
	...
	public boolean onBypass(PlayerEventInfo player, String bypass)
	{
		if(bypass.startsWith("mini_"))
		{
			...
			else if(action.startsWith("register"))
			{
				...
				if(!event.isRegularEvent())
				{
					...
					{
						player.sendMessage("Cannot register you!");
					}
				}
		...
		else if(bypass.startsWith("globalevent"))
		{
			String action = bypass.substring(12);
			
			if(action.startsWith("teleport"))
			{
				teleportToGlobalEvent(player);
				return true;
			}
		}
	...
	protected void showEditEventScheme(PlayerEventInfo player, String returnPage, String menuReturnPage, int page, String selectedCategory, String event)
	{
		...
		int id, level;
		i = 0;
		for(Entry<String, Map<Integer, Integer>> e : EventBuffer.getInstance().getAvailableBuffs().entrySet())
		{
			category = e.getKey();
	...
	protected String trimName(String name)
	{
		...
		if(name.equals("Pa'agrio's Emblem"))
			return "P. Emblem";
		
		if(split[0].equals("Improved"))
		{
			for(String s : split)
				if(!s.equalsIgnoreCase("Improved"))
					return s;
		}
	...
	{
		if(hodnota == 65054)
			return "BranchSys2.icon.br_wing_of_sylphide_i00";
		else if(hodnota == 65126)
			return "icon.skill1388";
		else if(hodnota == 65127)
			return "icon.skill1389";
		else if(hodnota == 65128)
			return "icon.skill4283_new";
		else if(hodnota == 65129)
			return "icon.skill1303";
		
		else if(hodnota == 65130)
			return "icon.skill1331";
		else if(hodnota == 65131)
			return "icon.skill1331";
		else if(hodnota == 65132)
			return "icon.skill1332";
		else if(hodnota == 65133)
			return "icon.skill1332";
		
		else if(hodnota == 65173)
			return "icon.skill1259";
		else if(hodnota == 65174)
			return "icon.skill4441";
		else if(hodnota == 65175)
			return "icon.skill1032";
		else if(hodnota == 65176)
			return "icon.skill4285";
		else if(hodnota == 65177)
			return "icon.skill4389";
		
		else if(hodnota == 65134)
			return "icon.skill1062";
	...
	public void showMainEventsMenu(PlayerEventInfo player, NpcData npc, String position, boolean fullDesc, boolean mapInfo)
	{
		...
		if (EventManager.getInstance().getMainEventManager().getState() == State.REGISTERING)
		{
			...
			if(currentEvent.isNewbieOnly())
				tb.append("<td width=205 align=left><font color=EBDF6C>Newbie " + currentEvent.getEventType().getHtmlTitle() + "</font></td>");
			else
				tb.append("<td width=205 align=left><font color=EBDF6C>" + currentEvent.getEventType().getHtmlTitle() + "</font></td>");
			...
			if(player.isRegistered())
			{
				...
				tb.append("<tr>");
					...
					tb.append("<td width=280 align=right><font color=FFFFFF>Type</font><font color=LEVEL> .buff</font> to open buffer in event.</font></td>");
			...
			/*double playerScore = player.getGearScore(false);
			double maxGearScore = currentEvent.getMaxGearScore();
			boolean good = playerScore <= maxGearScore;

			tb.append("<br1><table width=281 bgcolor=363724");
			...
			tb.append("<td width=95 align=right><font color=LEVEL>" + (double) playerScore + "</font></td>");
			...
			if(maxGearScore > 0)
			{
				...
				if(good)
					tb.append("<td width=95 align=right><font color=42D528>" + (double) maxGearScore + "</font></td>");
				else
					tb.append("<td width=95 align=right><font color=E01D1D>" + (double) maxGearScore + "</font></td>");
		...
		else if (EventManager.getInstance().getMainEventManager().getState() == State.RUNNING || EventManager.getInstance().getMainEventManager().getState() == State.TELE_BACK)
		{
			...
			tb.append("<br><br><button value=\"Event status\" action=\"bypass " + BBS_COMMAND + "\" width=95 height=23 back=\"L2UI_ct1.button_df\" fore=\"L2UI_ct1.button_df\"></center>");
		}
		else
		{
	...
	public void showMiniEventsMenu(PlayerEventInfo player)
	{
		...
		for(EventType type : EventType.values())
		{
			...
			tb.append("<font color=EBDF6C>* " + type.getHtmlTitle() + "</font>");
			tb.append("</td>");
			...
			for(Entry<Integer, MiniEventManager> e : EventManager.getInstance().getMiniEvents().get(type).entrySet())
			{
				if(e.getValue().getMode().isAllowed() && e.getValue().getMode().isRunning() && e.getValue().getMode().getNpcId() <= 0 && e.getValue().canRun())
				{
					...
					if(player != null && player.getRegisteredMiniEvent() != null && player.getRegisteredMiniEvent().equals(e.getValue()))
						tb.append("<td width=180><font color=494949>Mode</font><font color=595959> " + e.getValue().getMode().getModeId() + "</font>  <font color=eeeeff>" + e.getValue().getMode().getVisibleName() + "</font>" + "</td>");
					else if(e.getValue().isTournamentActive())
						tb.append("<td width=180><font color=494949>Mode</font><font color=595959> " + e.getValue().getMode().getModeId() + "</font>  <font color=LEVEL>" + e.getValue().getMode().getVisibleName() + " Tournament</font>" + "</td>");
					else
						tb.append("<td width=180><font color=494949>Mode</font><font color=595959> " + e.getValue().getMode().getModeId() + "</font>  <font color=eeeeff>" + e.getValue().getMode().getVisibleName() + "</font>" + "</td>");
					
					tb.append("<td width=70><a action=\"bypass -h nxs_mini_view " + e.getKey() + " " + type.getAltTitle() + "\"><font color=a7501e>View info</font></a></td>");
					...
					try
					{
						...
						if(e.getValue().isTournamentActive())
							maxCount = TournamentManager._maxTeamsCount;
						
						if(maxCount <= 0)
							tb.append("<font color=7A7A7A>" + count + "&nbsp;&nbsp;</font>");
						else
							tb.append("<font color=7A7A7A>" + count + "/" + maxCount + "&nbsp;</font>");
					} 
					catch (Exception ex)
					{
	...
	public void showMiniEventMenu(PlayerEventInfo player, EventType event, int modeId, boolean fullDesc, String position, String returnPage)
	{
		...
		/*String html = CallBack.getInstance().getOut().getEventHtml("mods/nexus/events/" + event.getAltTitle().toLowerCase() + "_" + manager.getMode().getModeName() + ".htm");
		
		// if the mode doesn't have it's own HTML file
		if(html == null)
		{
			html = CallBack.getInstance().getOut().getHtml("mods/nexus/events/" + event.getAltTitle().toLowerCase() + "_Default.htm");
		}
		...
		tb.append("<table width=281 bgcolor=2E2E2E>");
		tb.append("<tr>");
		tb.append("<td width=130><font color=EBDF6C>&nbsp;" + manager.getEventType().getHtmlTitle() + "</font></td>");

		if(manager.isTournamentActive())
			tb.append("<td width=140 align=right><font color=eeeeff>" + manager.getMode().getVisibleName() + " Tournament&nbsp;</font></td>");
		else
			tb.append("<td width=140 align=right><font color=eeeeff>" + manager.getMode().getVisibleName() + "&nbsp;</font></td>");
		...
		{
			boolean meets = false;
			if(player.getParty() != null && player.getParty().size() == partySize)
				meets = true;
		...
		{
			double playerScore = player.getGearScore();
			...
			tb.append("<td width=35 align=right></td>");
			tb.append("<td width=95 align=right><font color=LEVEL>" + playerScore / 100 + "</font></td>");
	...
	protected void showEventMaps(PlayerEventInfo player, EventType event, int modeId, boolean returnPage)
	{
		...
		String html = CallBack.getInstance().getOut().getHtml("mods/nexus/maps.htm");
	...
	public void showSpectateableGames(PlayerEventInfo player, EventType event, int modeId, boolean returnPage)
	{
		...
		String html = CallBack.getInstance().getOut().getHtml("mods/nexus/spectator_menu.htm");
	...
	{
		String html = CallBack.getInstance().getOut().getHtml("mods/nexus/registered.htm");
	...
	{
		String html = CallBack.getInstance().getOut().getHtml("mods/nexus/unregistered.htm");
