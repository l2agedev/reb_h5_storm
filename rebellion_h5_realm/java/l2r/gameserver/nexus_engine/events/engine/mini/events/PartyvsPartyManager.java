...
public class PartyvsPartyManager extends MiniEventManager
{
	...
	private FastTable<RegistrationData> tempTeams = new FastTable<RegistrationData>();
	...
	public void loadConfigs()
	{
		...
		addConfig(new ConfigModel("TeleportChecker", "true", "The count of rounds the event has.", ConfigModel.InputType.Boolean));
			...
			while(true)
			{
				...
				for(RegistrationData team : _parties)
				{
					...
					if(tempTeams.size() >= getTeamsCount())
					{
						...
						launchGame(tempTeams.toArray(new RegistrationData[tempTeams.size()]), chosenMap);
						limit -= 1;
						break;
					}
				}
	...
	{
		FastTable<PlayerEventInfo> partyPlayers = new FastTable<PlayerEventInfo>();
