...
/**
 * @author hNoke
 * - stores information about times when certain EventMode is available to run
 */
public class ScheduleInfo
{
	private static final Logger _log = LoggerFactory.getLogger(ScheduleInfo.class);
	...
	public class RunTime
	{
		...
		private Calendar getNextRun(boolean start)
		{
			...
			List<Calendar> times = new FastTable<Calendar>();
			...
			if(temp != null)
				runTime = temp;
			else // no time available in this week - switch to next week
			{
				for(Calendar time : times)
				{
			...
			if(temp == null)
			{
				_log.error("NexusEngine: No time found!! RunTime ID = " + id + ", from - " + from + ", to " + to);
				return null;
			}
			else
			{
