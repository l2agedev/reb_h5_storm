...
import l2r.gameserver.nexus_engine.events.engine.mini.features.BufferFeature;
import l2r.gameserver.nexus_engine.events.engine.mini.features.EnchantFeature;
import l2r.gameserver.nexus_engine.events.engine.mini.features.ItemGradesFeature;
import l2r.gameserver.nexus_engine.events.engine.mini.features.ItemsFeature;
import l2r.gameserver.nexus_engine.events.engine.mini.features.SkillsFeature;
...
{
	private static final Logger _log = LoggerFactory.getLogger(MiniEventGame.class);
	...
	public abstract int getReflectionId();
	...
	{
		_instanceId = CallBack.getInstance().getOut().createInstance("Game " + getEvent().getEventName() + " ID" + _gameId, getGameTime() + 59000, 0).getId();
		
		handleDoorsAndItems(0);
	...
	protected void checkItems(PlayerEventInfo player)
	{
		if(!player.isOnline())
			return;

		/*if(getEvent().isNewbieOnly())
		{
	...
	protected void loadFences()
	{
		try
		{
			_fences = new FastTable<FenceData>();
			for(EventSpawn spawn : _arena.getSpawns(0, SpawnType.Fence))
			{
				FenceData fence = CallBack.getInstance().getOut().createFence(2, spawn.getFenceWidth(), spawn.getFenceLength(), spawn.getLoc().getX(), spawn.getLoc().getY(), spawn.getLoc().getZ(), _arena.getGlobalId());
	...
	private void loadMapGuards()
	{
		...
		if(!template.exists())
		{
			_log.warn("Missing template for EventMap Guard.");
			return;
		}
		...
		for(EventSpawn spawn : getMap().getSpawns())
		{
			if(spawn.getSpawnType() == SpawnType.MapGuard)
			{
				try
				{
					...
					if(_mapGuards == null)
						_mapGuards = new FastTable<NpcData>();
	...
	protected void loadBuffers()
	{
		try
		{
			...
			if(!template.exists())
			{
				_log.warn("Missing NPC Buffer's template (ID " + bufferId + ") for event system.");
				return;
			}
			...
			for(EventSpawn spawn : _arena.getSpawns())
			{
				if(spawn.getSpawnType() == SpawnType.Buffer)
				{
					...
					if(_buffers == null)
						_buffers = new FastTable<NpcData>();
	 ...
	 */
	protected void handleDoorsAndItems(int state)
	{
		if(!_arena.hasDoor())
			return;
		...
		if(state == 0) // init (after teleported to the event, in wait time start, etc.
		{
			...
			else
			{
				...
				{
					action = DoorAction.getAction(doorSpawn.getNote(), 1);
					door = ZoneHolder.getDoor(doorSpawn.getDoorId());
					
					if(door == null)
						continue;
					
					if(action == DoorAction.Close && door.isOpen())
						door.closeMe();
					else if(action == DoorAction.Open && !door.isOpen())
						door.openMe();
				}
			}
		}
	...
	public boolean onSay(PlayerEventInfo player, String text, int channel)
	{
		...
		// TODO: infern0...
		/*
		if (player.isGM() && (text.equalsIgnoreCase(".voteabort") || text.equalsIgnoreCase(".voteend")))
		{
			voteEnd(player);
			return false;
		}
	...
	{
		if (_voted == null)
			_voted = new FastTable<PlayerEventInfo>();
	...
	public RegistrationData getWinner()
	{
		if(_winner == null)
		{
			for(RegistrationData data : _registeredTeams)
			{
				if(data != null)
				{
					_winner = data;
