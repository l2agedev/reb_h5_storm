...
public final class ItemFunctions
{
	...
	public static ItemInstance createItem(int itemId)
	{
		...
		boolean betaTest = BetaServer.isBetaServerAvailable() && BetaServer.getItemCreationEnchant() > 0;
		if (betaTest &&  item.canBeEnchanted(true))
			item.setEnchantLevel(BetaServer.getItemCreationEnchant());
	...
	{
		if (player.canOverrideCond(PcCondOverride.ITEM_USE_CONDITIONS))
			return null;
