...
public final class PetBabyInstance extends PetInstance
{
	...
	{
		return Util.constrain((getLevel() - getMinLevel()) / ((80 - getMinLevel()) / 12), 1, 12);
	}

	public int getRechargeLevel()
	{
		return Util.constrain((getLevel() - getMinLevel()) / ((80 - getMinLevel()) / 8), 1, 8);
	}

	public int getBuffLevel()
	{
		if(getNpcId() == PetDataTable.FAIRY_PRINCESS_ID)
			return Util.constrain((getLevel() - getMinLevel()) / ((80 - getMinLevel()) / 3), 1, 3);
		return Util.constrain((getLevel() - 55) / 5, 0, 3);
	}
