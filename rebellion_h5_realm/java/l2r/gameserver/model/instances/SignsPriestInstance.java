...
public class SignsPriestInstance extends NpcInstance
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		if(getNpcId() == 31113 || getNpcId() == 31126)
			if(SevenSigns.getInstance().getPlayerCabal(player) == SevenSigns.CABAL_NULL && !player.isGM() && !Config.ALLOW_MAMMON_FOR_ALL)
				return;
		...
		else if(command.startsWith("SevenSigns"))
		{
			...
			switch(val)
			{
				...
				case 21: // Exchange Adena for Ancient Adena - High Five
					if(player.getLevel() < 84) // custom level 85...
					{
	...
	public void showChatWindow(Player player, int val, Object... arg)
	{
		...
		switch(npcId)
		{
			...
			case 31113: // Merchant of Mammon
				if(!player.canOverrideCond(PcCondOverride.HTML_ACTION_CONDITIONS) && !Config.ALLOW_MAMMON_FOR_ALL)
					switch(compWinner)
					{
						case SevenSigns.CABAL_DAWN:
