...
public final class VillageMasterInstance extends NpcInstance
{
			...
			switch(intVal)
			{
				...
				case 4: // Добавление сабкласса - обработка выбора из case 1
					...
					if(allowAddition)
					{
						...
						// Possible fix for animation bug while holding dual daggers and add sub.
	...
	public void createAlly(Player player, String allyName)
	{
		...
		player.broadcastCharInfo();
		player.sendMessage(new CustomMessage("l2r.gameserver.model.instances.VillageMasterInstance.message2", player, allyName));
	}
	...
	public static boolean addNewSubclass(Player player, int classId)
	{
		...
		/*
		 * If you want to quest - continuity check Mimir's Elixir (Path to Subclass) For Kamael quest 236_SeedsOfChaos If the sub first, then check nachil subject, if not the first, then give the subclass. If you do not have subs, then check for a subject.
		 */
		if (!Config.ALT_GAME_SUBCLASS_WITHOUT_QUESTS && !playerClassList.isEmpty() && playerClassList.size() < 2 + Config.ALT_GAME_SUB_ADD)
		{
			if (player.isQuestCompleted("_234_FatesWhisper"))
