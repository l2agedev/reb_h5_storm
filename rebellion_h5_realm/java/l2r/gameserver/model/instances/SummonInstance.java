...
public class SummonInstance extends Summon
{
	...
	class Lifetime extends RunnableImpl
	{
		...
		public void runImpl() throws Exception
		{
			...
			if(!owner.canOverrideCond(PcCondOverride.SKILL_CONDITIONS) && _itemConsumeIdInTime > 0 && _itemConsumeCountInTime > 0 && _consumeCountdown <= 0)
			{
	...
	protected void onDeath(Creature killer)
	{
		...
		if(Config.ENABLE_PLAYER_COUNTERS && killer != null && killer != this && killer.getPlayer() != null)
			killer.getPlayer().getCounters().addPoint("_Summons_Killed");
	...
	public void onAction(Player player, boolean shift)
	{
		...
		if(shift)
		{
			if(player == null || !player.canOverrideCond(PcCondOverride.HTML_ACTION_CONDITIONS))
				return;
