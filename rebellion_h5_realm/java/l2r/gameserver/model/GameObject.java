...
public abstract class GameObject extends EventOwner
{
	...
	public void setReflection(Reflection reflection)
	{
		...
		if (reflection == null)
		{
			_log.error("Setting null reflection at GameObject: " + this, new NullPointerException());
	...
	public void toggleVisible0()
	{
		if (!_state.compareAndSet(DELETED, CREATED))
			_log.warn("state NOT changed from DELETED to created!");
		
		if(!_state.compareAndSet(CREATED, VISIBLE))
			_log.warn("state NOT changed from CREATED to visible");
	...
	/**
	 * Removes an object from the current region, making it invisible.
	 * Not to be confused with deleteMe. Object after decayMe can be restored through spawnMe
