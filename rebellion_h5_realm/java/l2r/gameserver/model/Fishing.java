...
public class Fishing
{
	...
	public void setLureId(LureTemplate lure)
	{
		_lure = lure;
	}

	public LureTemplate getLure()
	{
		return _lure;
	}
	...
	public void startFishing()
	{
		...
		_fisher.broadcastPacket(new ExFishingStart(_fisher, _fish.getGroup(), _fisher.getFishLoc(), _lure.getLureType()));
	...
	protected class LookingForFishTask extends RunnableImpl
	{
		...
		{
			_endTaskTime = System.currentTimeMillis() + _fish.getStartCombatTime() * 1000 + 10000L;
		}
		...
		public void runImpl() throws Exception
		{
			...
			if(!GameTimeController.getInstance().isNowNight() && _lure.getLureType().equals(LureType.NIGHT))
			{
				_fisher.sendPacket(SystemMsg.BAITS_HAVE_BEEN_LOST_BECAUSE_THE_FISH_GOT_AWAY);
				stopFishing();
				return;
		...
		public void runImpl() throws Exception
		{
			...
			else
			{
				...
				if(_stop == 0)
				{
					...
					if(_fish.getGrade().equals(FishGrade.HARD))
						if(Rnd.chance(10))
	...
	private void startFishCombat()
	{
		...
		if (_lure.isHardGrade())
			_deceptiveMode = Rnd.chance(20) ? 1 : 0;
		else
			_deceptiveMode = 0;

		ExFishingStartCombat efsc = new ExFishingStartCombat(_fisher, _time, _fish.getHp(), _combatMode, _lure.getLureType(), _deceptiveMode);
		...
		if(win)
			...
			else
			{
				if (Config.ENABLE_PLAYER_COUNTERS)
					_fisher.getCounters().addPoint("_Fish_Caught");
				
				_fisher.sendPacket(SystemMsg.SUCCEEDED_IN_FISHING);
				ItemFunctions.addItem(_fisher, _fish.getItemId(), 1, true);
