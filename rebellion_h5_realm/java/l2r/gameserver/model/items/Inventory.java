...
public abstract class Inventory extends ItemContainer
{
	...
	protected void onAddItem(ItemInstance item)
	{
		...
		if (getActor() != null)
		{
			if (getActor().isPlayer() && getActor().getPlayer() != null && getActor().getPlayer().getPet() == null)
				Log.LogItem(getActor().getPlayer(), Log.Create, item);
			else
				Log.LogItem(getActor(), Log.Create, item);
		}
	...
	protected void onModifyItem(ItemInstance item)
	{
		...
		{
			if (getActor().isPlayer() && getActor().getPlayer() != null && getActor().getPlayer().getPet() == null)
				Log.LogItem(getActor().getPlayer(), Log.Modifiy, item);
			else
				Log.LogItem(getActor(), Log.Modifiy, item);
		}
	...
	protected void onDestroyItem(ItemInstance item)
	{
		...
		{
			if (getActor().isPlayer() && getActor().getPlayer() != null && getActor().getPlayer().getPet() == null)
				Log.LogItem(getActor().getPlayer(), Log.Delete, item);
			else
				Log.LogItem(getActor(), Log.Delete, item);
		}
	...
	public int getPaperdollItemId(int slot)
	{
		...
		if(player.getVisualItems() != null && 
			...
			|| slot == PAPERDOLL_BACK)) {
			int[] items = player.getVisualItems();
			
			int displayItem = 0;
			if (item != null)
			{
				if (item.getdisplayId() > 0)
					displayItem = item.getdisplayId();
			}
			
			switch(slot)
			{
				case PAPERDOLL_CHEST:
					return items[0] > 0 ? items[0] : displayItem;
				case PAPERDOLL_LEGS:
					return items[1] > 0 ? items[1] : item!= null && item.getBodyPart() == ItemTemplate.SLOT_FULL_ARMOR ? 0 : displayItem;
				case PAPERDOLL_GLOVES:
					return items[2] > 0 ? items[2] : displayItem;
				case PAPERDOLL_FEET:
					return items[3] > 0 ? items[3] : displayItem;
				case PAPERDOLL_HAIR:
					return items[4] > 0 ? items[4] : displayItem;
				case PAPERDOLL_RHAND:
					return items[5] > 0 ? items[5] : displayItem;
				case PAPERDOLL_LHAND:
					return items[6] > 0 ? items[6] : item!= null && item.getTemplate().isShield() ? 0 : displayItem;
				case PAPERDOLL_BACK:
					return items[7] > 0 ? items[7] : displayItem;
			}
		}
	...
	public boolean isSlotForVisuals(int slot)
	{
		return slot == PAPERDOLL_CHEST || slot == PAPERDOLL_LEGS || slot == PAPERDOLL_GLOVES || slot == PAPERDOLL_FEET || slot == PAPERDOLL_HAIR || slot == PAPERDOLL_RHAND || slot == PAPERDOLL_LHAND || slot == PAPERDOLL_BACK;
	}
	...
	public int getPaperdollItemDisplayId(int slot, boolean visuals)
	{
		...
		if (player.getVisualItems() != null && isSlotForVisuals(slot) && visuals)
		{
			int[] items = player.getVisualItems();
			...
			switch(slot)
			{
				case PAPERDOLL_CHEST:
					return items[0] > 0 ? items[0] : displayItem;
				case PAPERDOLL_LEGS:
					return items[1] > 0 ? items[1] : displayItem;
				case PAPERDOLL_GLOVES:
					return items[2] > 0 ? items[2] : displayItem;
				case PAPERDOLL_FEET:
					return items[3] > 0 ? items[3] : displayItem;
