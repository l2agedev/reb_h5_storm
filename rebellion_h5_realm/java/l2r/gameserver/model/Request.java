...
public class Request extends MultiValueSet<String>
{
	...
	public Request(L2RequestType type, Player requestor, Player reciever)
	{
		...
		if (reciever != null)
			...
			_reciever = HardReferences.emptyRef();
	...
	public void timeout()
	{
		Player reciver = getReciever();
		Player reqestor = getRequestor();
		if(reciver != null && reqestor != null)
		{
			if(reciver.getRequest() == this)
				reciver.sendPacket(SystemMsg.TIME_EXPIRED);
			
			if(reqestor.getRequest() == this)
				reqestor.sendMessage("No respond from " + reciver.getName());
		}
