...
public class World
{
	...
	public static int regionX(GameObject obj)
	{
		return (obj.getX() - World.MAP_MIN_X >> 15) + Config.GEO_X_FIRST;
	}

	public static int regionY(GameObject obj)
	{
		return (obj.getY() - World.MAP_MIN_Y >> 15) + Config.GEO_Y_FIRST;
	}

	public static int regionXStatic(int x)
	{
		return (x - World.MAP_MIN_X >> 15) + Config.GEO_X_FIRST;
	}

	public static int regionYStatic(int y)
	{
			...
			for(int y = validY(currentRegion.getY() - 1); y <= validY(currentRegion.getY() + 1); y++)
				for(int z = validZ(currentRegion.getZ() - 1); z <= validZ(currentRegion.getZ() + 1); z++)
					for(GameObject obj : getRegion(x, y, z))
					{
						if(!obj.isCreature())
							continue;
						if (obj.getReflectionId() > ReflectionManager.DEFAULT_ID && loc.r > ReflectionManager.DEFAULT_ID && obj.getReflectionId() != loc.r)
							continue;
						if(Math.abs(obj.getZ() - loc.getZ()) > height)
							continue;
	...
	public static void removeObjectFromPlayer(Player player, GameObject object)
	{
		WorldRegion currentRegion = player.isInObserverMode() ? player.getObserverRegion() : player.getCurrentRegion();
		if(currentRegion == null)
			return;
		...
		for(int x = validX(currentRegion.getX() - 1); x <= validX(currentRegion.getX() + 1); x++)
			for(int y = validY(currentRegion.getY() - 1); y <= validY(currentRegion.getY() + 1); y++)
				for(int z = validZ(currentRegion.getZ() - 1); z <= validZ(currentRegion.getZ() + 1); z++)
					if (object != null && object.getObjectId() != oid && object.getReflectionId() == rid)
						player.sendPacket(player.removeVisibleObject(object, null));
	}
