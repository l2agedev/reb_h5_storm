...
public abstract class GlobalEvent extends LoggerObject
{
	...
	public void spawnAction(String name, boolean spawn)
	{
		...
		if(objects.isEmpty())
		{
			if (!name.equals("siege_minister")) // Infern0, lets this spam end NOW. :)
	...
	public void giveItem(Player player, int itemId, long count)
	{
		...
		FortressSiegeEvent fortsiegeevent = player.getEvent(FortressSiegeEvent.class);
		...
		switch(itemId)
		{
			case ItemTemplate.ITEM_ID_FAME:
				if (fortsiegeevent != null)
					player.setFame(player.getFame() + Config.FAME_REWARD_FORTRESS, toString());
				else if (castlesiegeevent != null)
					player.setFame(player.getFame() + Config.FAME_REWARD_CASTLE, toString());
				else
