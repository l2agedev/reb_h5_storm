...
public abstract class SiegeEvent<R extends Residence, S extends SiegeClanObject> extends GlobalEvent
{
	...
	public void reCalcNextTime(boolean isServerStarted)
	{
		...
		if(isServerStarted)
		{
			...
			if(startSiegeDate.getTimeInMillis() <= System.currentTimeMillis())
			{
				...
				validateSiegeDate(startSiegeDate, Config.PERIOD_CASTLE_SIEGE); // This config should be in how many weeks sieges begin.
				getResidence().setJdbcState(JdbcEntityState.UPDATED);
			}
		}
		else
		{
			startSiegeDate.add(Calendar.WEEK_OF_MONTH, Config.PERIOD_CASTLE_SIEGE); // This config should be in how many weeks sieges begin. (Changed from WEEK_OF_YEAR )
			getResidence().setJdbcState(JdbcEntityState.UPDATED);
		}
	...
	public class KillListener implements OnKillListener
	{
		...
		public void onKill(Creature actor, Creature victim)
		{
			...
			Player loser = victim.getPlayer();

			if(winner == null || victim == null || loser == null/* || winner.isSummon() || winner.isPet()*/)
				return;
			
			if (winner == loser || winner.getLevel() < 40)
				return;
			
			if (winner.getClan() != null && loser.getClan() != null && winner.getClanId() == loser.getClanId())
				return;
			
			if (winner.getAllyId() > 0 && winner.getAllyId() == loser.getAllyId())
				return;
			
			if (winner.getPlayerGroup() == loser.getPlayerGroup()) // Self, Party and Command Channel check.
				return;
			
			if (winner.isDualbox(loser))
				return;
			
			if (victim.getEvent(SiegeEvent.this.getClass()) != SiegeEvent.this || !checkIfInZone(winner) || !checkIfInZone(victim))
				return;

			if (winner.checkIfKillIsFeed(loser))
				return;
			
			winner.setFame(winner.getFame() + Rnd.get(Config.RATE_SIEGE_FAME_MIN, Config.RATE_SIEGE_FAME_MAX), SiegeEvent.this.toString());
			
			
			CastleSiegeEvent castlesiegeevent = winner.getEvent(CastleSiegeEvent.class);
			DominionSiegeEvent dominionsiegeevent = winner.getEvent(DominionSiegeEvent.class);
			FortressSiegeEvent fortsiegeevent = winner.getEvent(FortressSiegeEvent.class);
			
			if (Config.ENABLE_PLAYER_COUNTERS)
			{
				if (castlesiegeevent != null)
					winner.getCounters().addPoint("_Players_Killed_In_Siege");
				if (dominionsiegeevent != null)
					winner.getCounters().addPoint("_Players_Killed_In_Dominion");
				if (fortsiegeevent != null)
					winner.getCounters().addPoint("_Players_Killed_At_Fortress");
			}
		}
