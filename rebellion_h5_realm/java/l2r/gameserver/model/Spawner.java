...
public abstract class Spawner extends EventOwner implements Cloneable
{
			...
			if(mob.isMonster())
			{
				...
				if (ChampionTemplateTable.ENABLE_EXT_CHAMPION_MODE)
				{
					if (!mob.isRaid())
					{
						...
						for (ChampionTemplate ct : ChampionTemplateTable.getChampionTemplates())
						{
							if ((ct.spawnsInInstances || mob.getReflection().getId() == ReflectionManager.DEFAULT_ID) // if template allows to be spawned here
	...
	public void decreaseCount0(NpcTemplate template, NpcInstance spawnedNpc, long deadTime)
	{
		...
		if(_doRespawn && _scheduledCount + _currentCount < _maximumCount)
		{
			...
			long delay = (long) (template.isRaidBoss() ? Config.RAID_RESPAWN_MULTIPLIER * getRespawnDelayWithRnd() : getRespawnDelayWithRnd()) * 1000L;
