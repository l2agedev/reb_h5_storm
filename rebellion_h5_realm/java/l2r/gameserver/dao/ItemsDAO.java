...
public class ItemsDAO implements JdbcDAO<Integer, ItemInstance>
{
	...
	private final static String FIND_ITEM = "SELECT object_id FROM items WHERE item_id = ?";
	...
	public Collection<ItemInstance> getItemsByOwnerId(int ownerId)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("SELECT object_id FROM items WHERE owner_id = ?");
			statement.setInt(1, ownerId);
			rset = statement.executeQuery();
			objectIds = new ArrayList<Integer>();
			while(rset.next())
	...
	public Collection<ItemInstance> getItemsByItemId(int itemId)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement(FIND_ITEM);
			statement.setInt(1, itemId);
			rset = statement.executeQuery();
			items = new ArrayList<Integer>();
			while(rset.next())
				items.add(rset.getInt(1));
		}
		catch(SQLException e)
		{
			_log.error("Error while restore items for id : " + itemId, e);
			items.clear();
		}
		finally
		{
	...
	public long getItemCount(int itemId)
	{
		...
		{
			con = DatabaseFactory.getInstance().getConnection();
			statement = con.prepareStatement("SELECT(SELECT SUM(count) FROM items WHERE item_id=?) AS `count` LIMIT 100");
			statement.setInt(1, itemId);
			rset = statement.executeQuery();
			if(rset.next())
			{
