 ...
 * @author Forsaiken
 */
public final class RequestDeleteInventoryItem extends ReceivableConsolePacket
{
	...
	protected final void runImpl()
	{
		...
		switch (reply.getStatus())
		{
			...
			{
				console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_ITEM_FAILED, AnswereRequestPlayerInfo.MODE_ERROR_PLAYER_NOT_FOUND, null));
				break;
			}
				
			case Console.PLAYER_STATUS_OFFLINE:
			{
				console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_ITEM_FAILED, AnswereRequestPlayerInfo.MODE_ERROR_PLAYER_OFFLINE, null));
				break;
			}
			...
			case Console.PLAYER_STATUS_OFFLINE_MODE:
			{
				...
				if (item != null)
				{
					...
					if (player.getInventory().destroyItem(item, _itemCount))
					{
						ConsoleController.CONSOLE_LOG.log(Level.INFO, console + " changed player: '" + player.getName() + "', deleted item: '" + item.getItemId() + '-' + item.getName() + "-(" + item.getObjectId() + '|' + item.getCount() + '|' + item.getEnchantLevel() + '-' + item.getAugmentationId() + '-' + item.getAttackElement().getId() + '-' + item.getAttackElementValue() + ")'");
						console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_ITEM_SUCCESS, mode, player));
					}
					else
					{
						console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_ITEM_FAILED, mode, player));
					}
				}
				else
				{
