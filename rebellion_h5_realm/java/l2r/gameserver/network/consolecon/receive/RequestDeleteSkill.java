 ...
 * @author Forsaiken
 */
public final class RequestDeleteSkill extends ReceivableConsolePacket
{
	...
	protected final void runImpl()
	{
		...
		switch (reply.getStatus())
		{
			...
			{
				console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_SKILL_FAILED, AnswereRequestPlayerInfo.MODE_ERROR_PLAYER_NOT_FOUND, null));
				break;
			}
				
			case Console.PLAYER_STATUS_OFFLINE:
			{
				console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_SKILL_FAILED, AnswereRequestPlayerInfo.MODE_ERROR_PLAYER_OFFLINE, null));
				break;
			}
			...
			case Console.PLAYER_STATUS_OFFLINE_MODE:
			{
				...
				if (SkillTable.getInstance().getMaxLevel(_skillId) != 0)
				{
					final int skillLvl = player.getSkillLevel(_skillId);
					if (skillLvl > 0)
					{
						final Skill skill = SkillTable.getInstance().getInfo(_skillId, skillLvl);
						if (skill != null)
						{
							player.removeSkill(skill, true);
							player.sendPacket(new SkillList(player));
							ConsoleController.CONSOLE_LOG.log(Level.INFO, console + " changed player: '" + player.getName() + "', deleted skill: '" + skill.getId() + '-' + skill.getLevel() + '\'');
							console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_SKILL_SUCCESS, mode, player));
							break;
						}
					}
					
				}
				console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_DELETE_SKILL_FAILED_UNKNOWN_SKILL, mode, player));
				break;
			}
		}
	}
