 ...
 * @author Forsaiken
 */
public final class RequestModifySkill extends ReceivableConsolePacket
{
	...
	protected final void runImpl()
	{
		...
		switch (reply.getStatus())
		{
			...
			{
				console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_MODIFY_SKILL_FAILED, AnswereRequestPlayerInfo.MODE_ERROR_PLAYER_NOT_FOUND, null));
				break;
			}
				
			case Console.PLAYER_STATUS_OFFLINE:
			{
				console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_MODIFY_SKILL_FAILED, AnswereRequestPlayerInfo.MODE_ERROR_PLAYER_OFFLINE, null));
				break;
			}
			...
			case Console.PLAYER_STATUS_OFFLINE_MODE:
			{
				...
				if (_skillLevel > 100)
				{
					EnchantSkillLearn sl = SkillTreeTable.getSkillEnchant(_skillId, _skillLevel);
					if(sl == null)
						return;

					_skillLevel = SkillTreeTable.convertEnchantLevel(sl.getBaseLevel(), _skillLevel, sl.getMaxLevel());
				}
				...
				if (skill != null)
				{
					...
					ConsoleController.CONSOLE_LOG.log(Level.INFO, console + " changed player: '" + player.getName() + "', modified skill: '" + skill.getId() + '-' + skill.getLevel() + '\'');
					console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_MODIFY_SKILL_SUCCESS, mode, player));
				}
				else
				{
					console.sendPacket(new AnswereRequestPlayerInfo(_requestId, AnswereRequestPlayerInfo.RESPONSE_MODIFY_SKILL_FAILED_UNKNOWN_SKILL, mode, player));
				}
				break;
			}
		}
