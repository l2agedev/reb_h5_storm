...
public class RequestOustPartyMember extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if(party == null || !activeChar.getParty().isLeader(activeChar))
		{
			...
			{
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.clientpackets.RequestOustPartyMember.CantOutOfGroup", activeChar));
				return;
			}
			...
			if(r != null && r instanceof DimensionalRift && member.getReflection().equals(r))
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.clientpackets.RequestOustPartyMember.CantOustInRift", activeChar));
			else if(r != null && !(r instanceof DimensionalRift))
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.clientpackets.RequestOustPartyMember.CantOustInDungeon", activeChar));
			else
