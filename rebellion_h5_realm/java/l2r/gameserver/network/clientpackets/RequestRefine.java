...
public final class RequestRefine extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (Config.SECURITY_ENABLED && Config.SECURITY_ITEM_AUGMENT && activeChar.getSecurity())
		{
		...
		{
			activeChar.sendPacket(ExVariationResult.CLOSE);
			return;
		}
		...
		{
			activeChar.sendPacket(ExVariationResult.FAIL, SystemMsg.AUGMENTATION_FAILED_DUE_TO_INAPPROPRIATE_CONDITIONS);
			return;
		}
		...
		if(TryAugmentItem(activeChar, targetItem, refinerItem, gemstoneItem))
		{
			...
			activeChar.sendPacket(new ExVariationResult(new int[] { stat12, stat34 }), SystemMsg.THE_ITEM_WAS_SUCCESSFULLY_AUGMENTED);
		}
		else
			activeChar.sendPacket(ExVariationResult.FAIL, SystemMsg.AUGMENTATION_FAILED_DUE_TO_INAPPROPRIATE_CONDITIONS);
	}
	...
	boolean TryAugmentItem(Player player, ItemInstance targetItem, ItemInstance refinerItem, ItemInstance gemstoneItem)
	{
		...
		else if (Config.DISABLE_ACCESSORY_AUGMENTATION && isAccessoryLifeStone)
		{
