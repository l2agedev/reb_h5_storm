...
public class SetPrivateStoreBuyList extends L2GameClientPacket
{
	...
	protected void runImpl()
	{
		...
		if (Config.SECURITY_ENABLED && Config.SECURITY_TRADE_ENABLED && buyer.getSecurity())
		{
			buyer.sendChatMessage(0, ChatType.TELL.ordinal(), "SECURITY", (buyer.isLangRus() ? "Для того, чтобы это сделать, идентифицировать себя с помощью .security" : "In order to do this, identify yourself via .security"));
			buyer.sendActionFailed();
			return;
		}
		
		if(!buyer.canOpenPrivateStore(Player.STORE_PRIVATE_BUY))
		{
			buyer.sendActionFailed();
			return;
		}

		if(NexusEvents.isInEvent(buyer))
		{
			buyer.sendActionFailed();
			return;
		}
		...
		try
		{
			loop: for(int i = 0; i < _count; i++)
			{
				...
				{
					buyer.sendMessage(new CustomMessage("l2r.gameserver.clientpackets.SetPrivateStoreBuyList.TooLowPrice", buyer).addItemName(item).addNumber(item.getReferencePrice() / Config.ALT_SELL_PRICE_DIV));
					continue;
				}
		...
		if(!buyList.isEmpty())
		{
			...
			{
				buyer.ask(new ConfirmDlg(SystemMsg.S1, 10000).addString("Do you want to go in offline store mode?"), new OnAnswerListener()
				{
					@Override
					public void sayYes()
					{
						IVoicedCommandHandler vch = VoicedCommandHandler.getInstance().getVoicedCommandHandler("offline");
						if(vch != null && buyer != null)
							vch.useVoicedCommand("offline", buyer, "");
					}
