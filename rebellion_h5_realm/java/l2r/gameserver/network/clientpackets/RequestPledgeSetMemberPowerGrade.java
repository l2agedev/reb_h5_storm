	...
	protected void runImpl()
	{
		...
		if((activeChar.getClanPrivileges() & Clan.CP_CL_MANAGE_RANKS) == Clan.CP_CL_MANAGE_RANKS)
		{
			...
			if(member != null)
			{
				...
				{
					activeChar.sendMessage(activeChar.isLangRus() ? "Вы не можете изменить академии члена класса." : "You cannot change academy member grade.");
					return;
				}
				if(_powerGrade > 5)
				...
				activeChar.sendMessage(new CustomMessage("l2r.gameserver.clientpackets.RequestPledgeSetMemberPowerGrade.NotBelongClan", activeChar));
		}
		else
			activeChar.sendMessage(new CustomMessage("l2r.gameserver.clientpackets.RequestPledgeSetMemberPowerGrade.HaveNotAuthority", activeChar));
	}
}