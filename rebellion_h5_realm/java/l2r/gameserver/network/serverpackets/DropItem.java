...
public class DropItem extends L2GameServerPacket
{
	...
	public DropItem(ItemInstance item, int playerId)
	{
		...
		item_id = item.getdisplayId() > 0 ? item.getdisplayId() : item.getItemId();
		_loc = item.getLoc();
		_stackable = item.isStackable() ? 1 : 0;
		_count = item.getCount();
		//Log.add("Drop Item |" + (player == null ? "NullPlayer" : player.getName()) + "|Drop item|" + item_id + "|" + _count + "|" + item_obj_id, "items");
	}
