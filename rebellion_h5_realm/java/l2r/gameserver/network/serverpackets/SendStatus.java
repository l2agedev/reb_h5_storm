...
public final class SendStatus extends L2GameServerPacket
{
	...
	public SendStatus()
	{
		...
		{
			totalplayers = (totalplayers + (Config.ONLINE_PLUS * totalplayers / 100));
			
			if (!Config.SENDSTATUS_TRADE_JUST_OFFLINE)
				offlinetraders = (offlinetraders + (Config.ONLINE_PLUS * offlinetraders / 100));
		}
		
		online_players = totalplayers;
		online_priv_store = (int) Math.floor(offlinetraders * Config.SENDSTATUS_TRADE_MOD);
