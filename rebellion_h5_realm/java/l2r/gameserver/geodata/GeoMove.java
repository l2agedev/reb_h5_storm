...
{
	public static List<List<Location>> findMovePath(int x, int y, int z, Location target, Creature c, int geoIndex)
	{
		List<Location> path = PathFind.findPath(x, y, z, target, c != null && c.isPlayable(), geoIndex);
		if(path == null)
	...
	public static List<Location> constructMoveList(Location begin, Location end)
	{
		...
		// Removed to fix the clicking bug when you click so fast in terrain and player get back. Thanks Sorin(Prims) for giving me the fix :P
