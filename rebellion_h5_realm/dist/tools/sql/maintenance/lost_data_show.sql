...
SELECT * FROM character_shortcuts WHERE object_id NOT IN (SELECT obj_Id FROM characters);
...
SELECT * FROM items WHERE owner_id NOT IN (SELECT obj_Id FROM characters) AND owner_id NOT IN (SELECT clan_id FROM clan_data) AND owner_id NOT IN (SELECT objId FROM pets);
